...
public class DefaultTreeTableSorter<T extends TreeModel, C extends TreeColumnModel, I>
	...
	public static final List<SortOrder> ASCENDING_DESCENDING = Collections
			.unmodifiableList(Arrays.asList(SortOrder.ASCENDING, SortOrder.DESCENDING));

	public static final List<SortOrder> ASCENDING_DESCENDING_UNSORTED = Collections
			.unmodifiableList(Arrays.asList(SortOrder.ASCENDING, SortOrder.DESCENDING, SortOrder.UNSORTED));
	...
	public void setSortKeys(List<? extends SortKey> keys) {
		...
		if (keys != null && !keys.isEmpty()) {
			sortKeys = Collections.unmodifiableList(new ArrayList<>(keys));
		} else {
	...
	public void toggleSortOrder(int column) {
		...
		if (isSortable(column)) {
			List<SortKey> keys = toggleSortOrder(getSortKeys(), getSortCycle(), column, getMaxSortKeys());
			setSortKeys(keys);
		}
	}

	// adapted from DefaultRowSorter.toggleSortOrder
	static List<SortKey> toggleSortOrder(List<? extends SortKey> sortKeys, List<SortOrder> sortCycle, int column,
			int maxSortKeys) {
		List<SortKey> keys = new ArrayList<>(sortKeys);
		SortKey sortKey;
		int sortIndex;
	...
	public class NodeSorter extends DefaultRowSorter<T, I> implements SortCycle {
		...
		protected Map<Object, NodeSorter> createChildren() {
			return new IdentityHashMap<>(getModel().getChildCount(getNode()));
		}
		...
		protected boolean useToString(int column) {
			if (super.getComparator(column) != null || getMaster().isComparatorSet(column)) {
				return false;
			}
		...
		public void toggleSortOrder(int column) {
			checkColumn(column);
			if (isSortable(column)) {
				List<SortKey> keys = DefaultTreeTableSorter.toggleSortOrder(super.getSortKeys(), getSortCycle(), column,
						getMaxSortKeys());
				setSortKeys(keys);
			}
		}
		...
		private TreePath getPathToRoot() {
			...
			return parent.getPathToRoot().pathByAddingChild(getNode());
		}
		...
		protected class TreeTableWrapper extends ModelWrapper<T, I> {
			...
			/**
			 * The last row count must be cached until this method is called to update it.
			 */
