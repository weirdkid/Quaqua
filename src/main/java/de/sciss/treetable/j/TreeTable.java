...
public class TreeTable extends JComponent implements Scrollable {
	...
	private HashMap<TreePath, SortKey> sortedPaths = new HashMap<>();
...
//		}
	...
	private void updateUIRenderers() {
				...
				if (entry.getValue() instanceof JComponent) {
					...
					for (int col = cm.getColumnCount(); --col >= 0;) {
						if (cls.isAssignableFrom(cm.getColumnClass(col))) {
							addComponents(components, entry.getValue(), convertColumnIndexToView(col));
						}
					}
				}
			}
		...
		if (columnModel != null) {
			for (int col = columnModel.getColumnCount(); --col >= 0;) {
				...
				} else if (!(r instanceof Component) && r instanceof TreeTableCellRenderer) {
		...
		try {
			...
			if (col == adapter.treeColumnModel.getHierarchicalColumn()) {
				...
				if (row < 0) {
					...
					c = r.getTreeTableCellRendererComponent(this, value, false, false, row, col, isExpanded(path),
							isLeaf(path));
				}
	...
	/**
	 * Provides extra information about the mouse event, see TreeTableMouseEvent for
	 * more details. Consuming the TreeTableMouseEvent will also consume the source
	 * MouseEvent. TreeTableMouseListeners are notified before MouseListeners.
	 *
	 ...
	 * Provides extra information about the mouse event, see TreeTableMouseEvent for
	 * more details. Consuming the TreeTableMouseEvent will also consume the source
	 * MouseEvent. TreeTableMouseMotionListeners are notified before
	...
	/**
	 * For variable row heights, sync table row height to corresponding tree row
	 * height.
	 */
	...
	/**
	 * Sync table row heights to corresponding tree row height for rows
	 * <code>fromRow</code> (inclusive) to <code>toRow</code> exclusive.
	 */
	...
	public String getToolTipText(MouseEvent e) {
		return getToolTipMap() == null ? getToolTipText() : getToolTipMap().getToolTipText(this, e);
	}
	...
	/**
	 * Changes to the TreeModel/RowModel as well as row insertion/removal due to
	 * tree expansion/collapse can be listened to in terms of a TableModelListener
	 * by adding the TableModelListener to this TableModel.
	 *
	...
	public TreeTableCellRenderer getDefaultRenderer(Class<?> columnClass) {
		TreeTableCellRenderer r = defaultRenderers == null ? null : getDefault(columnClass, defaultRenderers);
		return r != null ? r : ui == null ? null : getUI().getDefaultRenderer(this, columnClass);
	}
	...
	public TreeTableCellEditor getDefaultEditor(Class<?> columnClass, int column) {
		TreeTableCellEditor e = defaultEditors == null ? null : getDefault(columnClass, defaultEditors);
		return e != null ? e : ui == null ? null : getUI().getDefaultEditor(this, columnClass, column);
	}
	...
	public TreeTableCellRenderer getCellRenderer(int row, int column) {
		if (column >= 0 && column < getColumnModel().getColumnCount()) {
			...
			return getDefaultRenderer(getTreeColumnModel().getColumnClass(convertColumnIndexToModel(column)));
		}
	...
	public TreeTableCellEditor getCellEditor(int row, int column) {
		if (column >= 0 && column < getColumnModel().getColumnCount()) {
			...
			return getDefaultEditor(getTreeColumnModel().getColumnClass(convertColumnIndexToModel(column)), column);
		}
	...
	public Icon getIcon(Object node, boolean expanded, boolean leaf) {
		Icon icon = iconMap == null ? null : iconMap.getIcon(this, node, expanded, leaf);
		if (icon == null) {
			if (leaf) {
				icon = getLeafIcon();
			} else if (expanded) {
	...
	/**
	 * Changes to the TreeSelectionModel can be listened to in terms of a
	 * ListSelectionModel by adding a ListSelectionListener to this
	 * ListSelectionModel.
	 *
	 ...
	 * for vertical scrolls and as a JTable for horizontal scrolls.
	 *
	...
	public int getScrollableBlockIncrement(Rectangle visibleRect, int orientation, int direction) {
		return getScrollable(orientation).getScrollableBlockIncrement(visibleRect, orientation, direction);
	}
	...
	public int getScrollableUnitIncrement(Rectangle visibleRect, int orientation, int direction) {
		return getScrollable(orientation).getScrollableUnitIncrement(visibleRect, orientation, direction);
	}
	...
	protected class Adapter extends AbstractTableModel implements TreeTableModel, TreeModelListener,
			TreeColumnModelListener, ListSelectionModel, PropertyChangeListener, TreeTableSorterListener,
			TreeExpansionListener, TreeSelectionListener, TreeWillExpandListener, ToolTipMap<TreeTable> {
		...
		public void treeColumnChanged(TreeColumnModelEvent e) {
			...
			if (rowSorter != null) {
				RowSorter<?> sorter = path.getParentPath() != null
						? rowSorter.getRowSorter(path.getParentPath().getLastPathComponent())
						: null;
		...
		private int[] getRows(TreePath parent, Object[] childNodes) {
			...
			if (!isExpanded(parent) || (parent.getPathCount() > 1 && getRowForPath(parent) < 0)) {
				return null;
			}
		...
		private void updateTable(int firstRow, int lastRow, int eventType) {
			fireTableChanged(new TableModelEvent(this, firstRow, lastRow, TableModelEvent.ALL_COLUMNS, eventType));
...
//				invalidateRows(firstRow, lastRow);
		...
		public void propertyChange(PropertyChangeEvent evt) {
			...
			if (name.equals(JTree.LEAD_SELECTION_PATH_PROPERTY) || name.equals(JTree.ANCHOR_SELECTION_PATH_PROPERTY)) {
				int col = !isColumnFocusEnabled() ? -1 : getColumnModel().getSelectionModel().getLeadSelectionIndex();
			...
			} else if (name.equals("componentOrientation") || name.equals("enabled") || name.equals("rowSorter")
					|| name.equals("model")) {
				return; // don't fire
			}
		...
		public void invalidateRows(int firstRow, int lastRow) {
			if (firstRow == lastRow) {
				...
				HashMap<TreePath, ArrayList<Object>> map = new HashMap<>();
				int row = firstRow;
		...
		protected void fireTreeNodesChanged(TreePath path, int[] childIndices, Object[] childNodes) {
			AbstractTreeModel.fireNodesChanged(listenerList, this, path, childIndices, childNodes);
		}

		protected void fireTreeNodesInserted(TreePath path, int[] childIndices, Object[] childNodes) {
			AbstractTreeModel.fireNodesInserted(listenerList, this, path, childIndices, childNodes);
		}

		protected void fireTreeNodesRemoved(TreePath path, int[] childIndices, Object[] childNodes) {
			AbstractTreeModel.fireNodesRemoved(listenerList, this, path, childIndices, childNodes);
		}
...
//			System.out.println();
		...
		void updateSorter(TreePath path, boolean visible) {
			if (rowSorter != null) {
				List<TreePath> subPaths = visible ? expandedDescendants(path) : Collections.<TreePath>emptyList();
		...
		public String getToolTipText(TreeTable treeTable, MouseEvent event) {
			...
			if (row >= 0 && col >= 0) {
				...
				boolean foc = treeTable.isColumnFocusEnabled() && col == treeTable.getLeadSelectionColumn()
						&& row == treeTable.getLeadSelectionRow() && treeTable.isFocusOwner();
				Component c = r.getTreeTableCellRendererComponent(treeTable, treeTable.getValueAt(row, col), sel, foc,
						row, col);
				if (c instanceof JComponent) {
					Rectangle cell = treeTable.getCellRect(row, col, false);
					pt.translate(-cell.x, -cell.y);
					MouseEvent e = new MouseEvent(c, event.getID(), event.getWhen(), event.getModifiers(), pt.x, pt.y,
	...
	private static class SorterHelper implements Comparator<SimpleEntry<Integer, Object>> {
		...
		@Override
		public int compare(SimpleEntry<Integer, Object> a, SimpleEntry<Integer, Object> b) {
	...
	/**
	 * Queries the current drop location for this tree table. Currently, a transfer
	 * handler registered with this component will not receive a
	 * `TreeTable.DropLocation`, but only a `JTree.DropLocation`. This is a
	 * limitation of Swing. In order to get the necessary information about the
	 * drop, the handler can then call this method to get a richer description of
	 * the drop.
	 *
	 * TODO: Another future possibility is to modify the `setTransferHandler` method
	 * to wrap the argument in an enrichment handler which provides the full drop
	 * location.
	 *
	...
	private void maybeTogglePath(DropLocation oldLocation, Point pt) {
		...
		if (when == 0) {
			...
			if (path == null) {
				...
				dropLocation.when = isOverTreeHandle(pt) ? System.currentTimeMillis() : -1;
			} else {
				// over path bounds, schedule for expand if expandable
				dropLocation.when = isLeaf(path) || isExpanded(path) ? -1 : System.currentTimeMillis();
			}
		} else {
	...
	private boolean isOverTreeHandle(Point pt) {
		...
		if (col >= 0 && col == getHierarchicalColumn()) {
			...
			if (row >= 0) {
				return getDistanceToTreeHandle(getPathForRow(row), pt.x) == 0;
			}
		}
		return false;
	}
	...
	private DropLocation createDropLocation(Point pt) {
		...
		case INSERT_ROWS:
			...
			if (row < 0) {
				...
				index = path == null ? 0
						: adapter.getIndexOfChild(path.getLastPathComponent(), rowPath.getLastPathComponent());
				if (path == null) {
					path = rowPath;
					if (isRootVisible()) {
						row = 1;
	...
	public static final class DropLocation extends TransferHandler.DropLocation {
		 ...
		 *
		 * @return the index at which a drop occurs within the children of the branch
		 *         denoted by `getPath`. For example, `0` means the drop happens before
		 *         the first child, `1` means it happens after the first child. For `ON`
		 *         drop mode, `-1` indicates that the drop occurs above the parent node.
		 */
		public int getIndex() {
			return index;
		}
		...
		public boolean equals(DropLocation o) {
			return o != null && row == o.row && column == o.column && isInsertRow == o.isInsertRow
					&& isInsertColumn == o.isInsertColumn && index == o.index
	...
	public int getNextMatch(String prefix, int startingRow, int column, Position.Bias bias) {
		if ((prefix == null) || column < 0 || column >= getColumnCount()) {
	...
	/**
	 * Calculates the x distance from the tree handle. If the tree handle isn't
	 * present, it is the distance from the start of the path bounds.
	 * <p>
	 * A return value of 0 means the location is over the tree handle.
	 * <p>
	 * The return value will be negative for x locations that fall in the leading
	 * region and positive for x locations that fall in the trailing region.
	 *
