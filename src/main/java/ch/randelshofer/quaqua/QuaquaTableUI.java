...
public class QuaquaTableUI extends BasicTableUI implements ViewportPainter {
	...
	private void updateStriped() {
		/*
		 * if (isStriped) { table.setIntercellSpacing(new Dimension(1, 1)); } else {
	...
	private void paintCell(Graphics g, Rectangle cellRect, int row, int column, boolean isFocused) {
		...
		if (background instanceof InactivatableColorUIResource) {
			((InactivatableColorUIResource) background)
					.setActive(isFocused && (table.getRowSelectionAllowed() || table.getColumnSelectionAllowed()));
		}
		...
		if (table.isEditing() && table.getEditingRow() == row && table.getEditingColumn() == column) {
			...
			if (table.isCellSelected(row, column)) {
				...
				g.fillRect(cellRect.x - spacing.width, cellRect.y, cellRect.width + spacing.width * 2,
	 ...
	 */
	/**
	 * PropertyChangeListener for the table. Updates the appropriate variable, or
	 * TreeState, based on what changes.
	 */
	protected class Handler implements PropertyChangeListener, ListSelectionListener, //
		...
		/**
		 * This is a reimplementation of the JTable.valueChanged method, with the only
		 * difference, that we repaint the cells _including_ the intercell spacing.
		 *
		 * @param e
		 */
		@Override
		...
		protected boolean shouldIgnore(MouseEvent e) {
			return e.isConsumed() || (!(SwingUtilities.isLeftMouseButton(e) && table.isEnabled())) || e.isPopupTrigger()
					&& (table.rowAtPoint(e.getPoint()) == -1 || table.isRowSelected(table.rowAtPoint(e.getPoint())));
		}
					...
					if (table.isRowSelected(row) && e.isPopupTrigger()) {
						...
						if ((e.getModifiersEx() & (InputEvent.META_DOWN_MASK | InputEvent.BUTTON2_DOWN_MASK
							...
							mouseDragAction = MOUSE_DRAG_TOGGLES_SELECTION;
								...
								| InputEvent.BUTTON3_DOWN_MASK))//
								== InputEvent.SHIFT_DOWN_MASK && anchorIndex != -1 && table.getSelectionModel()
		...
		public void mouseDragged(MouseEvent e) {
			...
			if (editor == null || editor.shouldSelectCell(e)) {
				...
				if (mouseDragAction == MOUSE_DRAG_SELECTS) {
					...
					if (row != -1 && column != -1 && ((!isCellSelection && row != toggledRow)
					...
					if (table.getDragEnabled()) {
						...
						if (action != TransferHandler.NONE) {
							/*
							 * notify the BeforeDrag instance * / if (bd != null) {
