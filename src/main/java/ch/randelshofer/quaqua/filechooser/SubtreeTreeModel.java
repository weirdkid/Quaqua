...
public class SubtreeTreeModel implements TreeModel, TreeModelListener {
	...
	public Object[] toFullPath(Object[] subtreePath) {
		/*
		 * if (subtreeRoot != target.getRoot()) { Object[] components = new
		 * Object[rootPath.length + subtreePath.length - 1]; System.arraycopy(rootPath,
		 * 0, components, 0, rootPath.length); System.arraycopy(subtreePath, 1,
		 * components, rootPath.length, subtreePath.length - 1); return components; }
		 * else { return subtreePath; }
		 */
	...
	/**
	 * Returns true, if the specified path if a descendant path of the path to root
	 * of this subtree model.
	 */
