...
/**
 * Customize the mouse behavior of JLists in the file chooser browser. Some
 * behaviors of NSOpenPanel are quite bizarre, presumably implementation
 * artifacts rather than design choices. This class does not try to imitate
 * bizarre behavior.
 */
...
public class QuaquaFileChooserListMouseBehavior extends QuaquaListMouseBehavior {
	...
	/**
	 * The file selection handler is invoked when the user clicks on an unselectable
	 * item in a Save dialog.
	 */
	...
	protected int getSelectionOperation(MouseEvent e) {
		...
		/*
		 * The behavior of NSSavePanel is somewhat weird and not worth emulating. The
		 * simple model: an unmodified click on a traversable directory selects the
		 * directory in the browser, on anything else it transfers the item name to the
		 * text field without changing the browser selection. Modified clicks do
		 * nothing.
		 */
		...
		/*
		 * Command-clicking a traversable directory that is not an acceptable result has
		 * no effect unless the selection is empty or the item is selected.
		 */
		...
		/*
		 * Command-clicking a file in a files only chooser will deselect a selected
		 * directory.
		 */
	...
	/**
	 * Support the implementation of the behavior that trying to add a file
	 * (acceptable item) to a multiple selection when the selection is a directory
	 * (traversable item) and the file chooser accepts only files should select the
	 * file in place of (rather than in addition to) the directory. The code handles
	 * cases that do not normally arise where the existing selection contains
	 * multiple items of which at least one is a directory.
	 */
	private boolean shouldDeselectDirectory() {
		if (!fc.isMultiSelectionEnabled() || fc.isDirectorySelectionEnabled() || list.isRowSelected(targetIndex)
	...
	public void mouseReleased(MouseEvent e) {
		...
		/*
		 * If after all operations have been performed there are no selected items in
		 * the list, transfer the focus to the parent list.
		 */
	...
	/**
	 * Indicate whether the designated list item is an acceptable file chooser
	 * selection.
	 */
