...
public class FileSystemTreeModel implements TreeModel {
	...
	/**
	 * We need a JFileChooser to determine the user presentable (localized) name of
	 * a file.
	 */
	...
	/**
	 * This comparator is used to compare the user name of two files. The comparator
	 * is able to compare instances of java.io.File and instances of
	 * FileSystemTreeModel.Node.
	 */
	private Comparator nodeComparator;
	/**
	 * When this is true, DirectoryNode's automatically fetch a directory listing
	 * from the file system, if they are invalid and one of the following methods is
	 * called: DirectoryNode.getChildCount(), DirectoryNode.getChildAt(),
	...
	/**
	 * The collator used for sorting files. Note: We use a static variable here,
	 * because creating a collator is very expensive.
	 */
	 ...
	 *
	 * @param fileChooser The JFileChooser is used to determine the user presentable
	 *                    (localized) names of the files.
	 */
	 ...
	 * The comparator is able to compare instances of java.io.File and instances of
	 * FileSystemTreeModel.Node. Both kinds can by mixed freely.
	 */
	private Comparator getNodeComparator() {
		if (nodeComparator == null) {
			nodeComparator = UIManager.getBoolean("FileChooser.orderByType") ? (Comparator) new FoldersFirstComparator()
	 ...
	 */
	private void insertNodeInto(FileSystemTreeModel.Node newChild, FileSystemTreeModel.Node parent, int index) {
	 ...
	 *
	 * This is used to create nodes depending on their type (file, directory, alias
	 * to file, or alias to directory) and depending on the "resolveAliases"
	 * property.
	 */
	...
	/**
	 * Invalidates the provided path. This should be used to invalidateChildren the
	 * tree model when there are significant changes in the JFileChooser. Such as
	 * showing the JFileChooser and changing the FileFilters of the JFileChooser. To
	 * actually get a refresh of the tree, validatePath must be called.
	 */
	...
	/**
	 * Invalidates all nodes in the tree. This should be used to invalidate the tree
	 * model when there are significant changes in the JFileChooser. To actually get
	 * a refresh of the tree, validatePath must be called.
	 */
	...
	/**
	 * Stalls validation. This should be used to stop validation of the tree model
	...
	/**
	 * Lazily invalidates the provided path. This should be used to trigger lazy
	 * refreshes of the tree model when the user navigates through the tree.
	 */
	...
	/**
	 * Validates (refreshes) the nodes specified by the provided path. Validation is
	 * done for nodes only, which have been marked as invalid. The validation is
	 * done asynchronously in worker threads.
	 */
	...
	/**
	 * Returns true, if the file may be selected as a valid output selection of the
	 * file chooser. (Selection of a traversable item is a separate concept.)
	 */
	private boolean accept(FileInfo info) {
		File f = info.getFile();

		if ((fileChooser.getDialogType() == JFileChooser.SAVE_DIALOG) || !fileChooser.accept(f)) {
			return false;
		}
	...
	public class Node implements MutableTreeNode, FileInfo {
		...
		/**
		 * Holds a Finder label for the file represented by this node. The label is a
		 * value in the interval from 0 through 7. The value -1 is used, if the label
		 * has not (yet) been retrieved, or if it couldn't be determined due to the lack
		 * of native support.
		 */
		protected int fileLabel = -1;
		/**
		 * Holds the icon of the file. The value null is used, if the icon has not (yet)
		 * been retrieved, or if it couldn't be determined due to the lack of native
		 * support.
		 */
		protected Icon icon;
		/**
		...
		/**
		 * Contains Boolean.TRUE or Boolean.FALSE, if the file has been accepted or
		 * rejected by the FileFilter of the JFileChooser. Contains null, if the
		 * acceptance has not been determined yet.
		 */
		...
		/**
		 * Holds the tag names for the file represented by this node. The value null is
		 * used, if the tag names have not (yet) been retrieved, or if it couldn't be
		 * determined due to the lack of native support.
		 */
		...
		/**
		 * Lazily resolves the file. This method returns null, if the File has not been
		 * resolved yet.
		 */
		@Override
		...
		public Icon getIcon() {
			validateInfo();
			if (icon == null) {
				return (isLeaf()) ? UIManager.getIcon("FileView.fileIcon")
		...
		/**
		 * Changes the traversability of a directory node. This method has no effect on
		 * non-directory nodes.
		 */
		...
		public void invalidateInfo() {
			...
			if (isMonitoringInfoValidation(file) && infoState != INVALID) {
				System.out.println(
						"Invaliding info for " + file + (infoState == VALIDATING ? " (validation in progress)" : ""));
			}
		...
		/**
		 * Invoke the runnable when validation is complete, either now or later. This
		...
		/**
		 * Marks this node as invalid. If the node denotes not a directory, nothing
		 * happens.
		 */
		...
		/**
		 * Marks this node as invalid if the node is not currently being updated. If the
		 * node denotes not a directory, nothing happens.
		 */
		...
		/**
		 * Validates this node if it is invalid. If this node is invalid, a worker
		 * thread is launched, which reads the directory denoted by this node and merges
		 * it with the existing children of this node. If the node denotes not a
	...
	public class DirectoryNode extends Node {
		...
		private class DirectoryValidator implements Runnable {
			...
			/**
			 * This method is called by a worker thread. It reads the directory represented
			 * by this tree node and updates the children of this node accordingly. Since
			 * this runs asynchronously with the AWT event dispatcher thread, the updating
			 * may become obsolete. i.e, because the user chooses a different file filter.
			 * If this is detected, this method does nothing.
			 */
			@Override
			public void run() {
				if (this != validator) {
				...
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						...
						Node[] oldNodes = (children == null) ? new Node[0]
								: (Node[]) children.toArray(new Node[children.size()]);
						...
						for (int i = 0; i < count; i++) {
							if (freshIndex >= freshNodes.length) {
								...
								// or vice versa.
								if (comparison == 0) {
									if (freshNodes[freshIndex].getAllowsChildren() != oldNodes[oldIndex]
											.getAllowsChildren()) {
										comparison = -1;
									}
								}
							...
							if (comparison < 0) {
								...
								deletedChildIndices[deletedChildren.size()] = mergeIndex + deletedChildren.size()
										- newChildren.size();
						...
						if (newChildren.size() > 0 || deletedChildren.size() > 0) {
							...
							// iteration.
							/*
							 * for (Iterator i = newChildren.iterator(); i.hasNext(); ) { Node n = (Node)
							 * i.next(); n.parent = Node.this; }
							 */
							...
							if (newChildren.size() > 0 && deletedChildren.size() == 0) {
								...
								fireTreeNodesInserted(FileSystemTreeModel.this, getPath(),
										ArrayUtil.truncate(newChildIndices, 0, newChildren.size()),
										newChildren.toArray());
							} else if (newChildren.size() == 0 && deletedChildren.size() > 0) {
								children = mergedChildren;
								fireTreeNodesRemoved(FileSystemTreeModel.this, getPath(),
										ArrayUtil.truncate(deletedChildIndices, 0, deletedChildren.size()),
										deletedChildren.toArray());
							} else if (newChildren.size() > 0 && deletedChildren.size() > 0) {
						...
						/*
						 * // Validate the info of the first 16 nodes for (int i=0, n = Math.min(16,
						 * mergedChildren.size()); i < n; i++) { Node node = (Node)
						 * mergedChildren.get(i); node.validateInfo(); }
						 */
		...
		/**
		 * The time until the directory is considered to stay valid. The bestBeforeDate
		 * is a timestamp in milliseconds, that we can compare with the curent time
		...
		public DirectoryNode(File file, boolean isHidden) {
			...
			/*
			 * if (file != null && file.exists() && ! fileChooser.isTraversable(file)) {
		...
		/**
		 * Marks the children of this node as invalid. This has only an effect, if this
		 * node denotes a directory.
		 */
		@Override
		public void lazyInvalidateChildren() {
			if (validator == null && bestBeforeTimeMillis < System.currentTimeMillis()) {
				if (DEBUG) {
					System.out.println("FileSystemTreeModel.lazyInvalidateChildren YES  validator=" + validator
							+ " good for " + (bestBeforeTimeMillis - System.currentTimeMillis()) + " millis "
							+ lazyGetResolvedFile());
				}
				childrenState = INVALID;
			} else {
				if (DEBUG) {
					System.out.println("FileSystemTreeModel.lazyInvalidateChildren NO  validator=" + validator
							+ " good for " + (bestBeforeTimeMillis - System.currentTimeMillis()) + " millis "
							+ lazyGetResolvedFile());
				}
			}
		...
		public void invalidateTree() {
			...
			// invalidateChildren does not discard the nodes, and the nodes can contain
			// stale data
		...
		protected File[] getFiles() {
			...
			File[] files = getFileSystemView().getFiles(lazyGetResolvedFile(), fileChooser.isFileHidingEnabled());
			if (DEBUG) {
				System.out
						.println("FileSystemTreeModel getFiles " + lazyGetResolvedFile() + " returns " + files.length);
			}
			return files;
		}
...
//            return (File[]) list.toArray(new File[list.size()]);
		...
		public void validateChildren() {
			if (DEBUG) {
				System.out.println("FileSystemTreeModel.validateChildren of ROOT " + (childrenState == INVALID) + " "
						+ lazyGetResolvedFile());
			}
		...
		/*
		 * Don't hardcode "Computer" public String getUserName() { return "Computer"; }
		 */
		@Override
		public Icon getIcon() {
			validateInfo();
