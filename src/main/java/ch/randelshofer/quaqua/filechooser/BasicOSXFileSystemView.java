 ...
 *
 * A common base class for OS X file system views. The key feature of an OS X
 * file system view is that the Unix root directory is mapped to the system
 * volume. Also, there is no OS X file that corresponds exactly to the Computer
 * node in the file chooser tree model, but the Volumes folder is the closest,
 * so we use that.
 *
...
public abstract class BasicOSXFileSystemView extends QuaquaFileSystemView {
	 ...
	 * FileDialog when they are at the top directory level of a volume. The file
	 * names are wrongly treated as visible by Apple's implementation
	 * FileSystemView, so we use this HashSet here, to hide them 'manually'.
	 */
	...
	protected final static Set<String> hiddenVolumeNames = new HashSet<>();
	...
	/**
	 * Convert a file (path) to canonical form. A reference to the Unix root
	 * directory is mapped to the system volume.
	 */
	@Override
	...
	/**
	 * Returns the parent directory of dir. This method returns the system volume
	...
	/**
	 * Returns all root partitions on this system. This method returns the contents
	 * of the volumes folder. The root folder ("/") is considered as hidden and not
	...
	protected List<File> getRootList() {
		...
		/*
		 * Even if /Network is hidden when viewing /, it may be visible in the sidebar
		 * or under the Computer node.
		 */
	...
	public boolean isRoot(File aFile) {
		return aFile.equals(unixRoot) || aFile.equals(networkFolder)
	 ...
	 *
	 * @param folder a <code>File</code> object representing a directory or special
	 *               folder
	...
	/**
	 *
	 * @param parent   a <code>File</code> object representing a directory or
	 *                 special folder
