 ...
 * <p>
 * As of Quaqua 5.5, JSheets under Java 5 and lower are natively shown by
 * {@link OSXSheetSupport}. To activate that behavior, set the UIManager
 * property {@code "Sheet.experimentalSheet"} to {@code Boolean.TRUE}.
 * 
...
public class JSheet extends JDialog {
	...
	/**
	 * This handler is used to handle movements of the owner. If the owner moves, we
	 * have to change the location of the sheet as well.
	 */
	...
	/**
	 * We need to keep track of the old owner position, in order to avoid processing
	 * duplicate owner moved events.
	 */
	...
	/**
	 * This is set to true, when the listeners for the JSheet are installed on the
	 * parent component.
	 */
	private boolean isInstalled;
	/**
	 * If this is set to true, the JSheet uses a transition effect when shown and
	 * when hidden.
	 */
	...
	/**
	 * If this is set to true, the JSheet tries to use an experimental JNI method to
	 * be shown as a native window.
	 **/
	private boolean isExperimentalSheet;
	/**
	 * This variable is only used in Java 1.5 and previous versions. In order to
	 * make the sheet document modal, we have to block events on the owner window.
	 * We do this by setting a JPanel as the glass pane on the owner window. Before
	 * we do this, we store the glass pane of the owner window here, and restore it
		...
		ownerMovementHandler = new ComponentAdapter() {
			...
			public void componentMoved(ComponentEvent evt) {
				...
				if (!newLocation.equals(oldLocation)) {
					setLocation(newLocation.x + (owner.getWidth() - getWidth()) / 2,
			...
			if (owner != null) {
				...
				if (isShowAsSheet()) {
					if (owner instanceof JFrame) {
						sheetLoc = new Point(ownerLoc.x + (owner.getWidth() - getWidth()) / 2, ownerLoc.y
								+ owner.getInsets().top + ((JFrame) owner).getRootPane().getContentPane().getY());
					} else if (owner instanceof JDialog) {
						sheetLoc = new Point(ownerLoc.x + (owner.getWidth() - getWidth()) / 2, ownerLoc.y
								+ owner.getInsets().top + ((JDialog) owner).getRootPane().getContentPane().getY());
					} else {
						sheetLoc = new Point(ownerLoc.x + (owner.getWidth() - getWidth()) / 2,
	...
	/**
	 * Uninstalls the sheet on the owner. This method is invoked immediately after
	 * the JSheet is hidden.
	 */
	...
	/**
	 * If this returns true, the JSheet uses a transition effect when shown and when
	 * hidden.
	 * 
	 * @return the value
	 */
		...
		if (isExperimentalSheet()) {
			...
			final Rectangle endBounds = new Rectangle(
					(parentWidth < startBounds.width) ? startBounds.x + (startBounds.width - parentWidth) / 2
							: startBounds.x,
					startBounds.y, Math.min(startBounds.width, parentWidth), 0);
			...
			timer.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent evt) {
					...
					if (now > endTime) {
						...
						setBounds((int) (startBounds.x * (1f - ratio) + endBounds.x * ratio),
	...
	public void show() {
		...
		if (isAnimated() && isShowAsSheet() && !isNativeSheetSupported()) {
			...
			final Rectangle startBounds = new Rectangle(
					...
					endBounds.y, Math.min(endBounds.width, parentWidth), 0);
			setBounds(startBounds);
			if (!isDocumentModalitySupported()) {
	 ...
	 *
	 * @param parentComponent determines the {@code Frame} in which the sheet is
	 *                        displayed; if {@code null}, or if the
	 *                        {@code parentComponent} has no {@code Frame}, the
	 ...
	 *                        dialog: {@code YES_NO_OPTION}, or
	 *                        {@code YES_NO_CANCEL_OPTION}
	 * @param listener        The listener for SheetEvents.
	 */
	public static void showConfirmSheet(Component parentComponent, Object message, int optionType,
	...
	/**
	 * Brings up a sheet where the number of choices is determined by the
	 * {@code optionType} parameter, where the {@code messageType} parameter
	 * determines the icon to display. The {@code messageType} parameter is
	 * primarily used to supply a default icon from the Look and Feel.
	 *
	 * @param parentComponent determines the {@code Frame} in which the dialog is
	 *                        displayed; if {@code null}, or if the
	 *                        {@code parentComponent} has no {@code Frame}, the
	 *                        sheet is displayed as a dialog.
	 * @param message         the {@code Object} to display
	 ...
	 *                        {@code JOptionPane.QUESTION_MESSAGE}, or
	 *                        {@code JOptionPane.PLAIN_MESSAGE}
	 * @param listener        The listener for SheetEvents.
	 */
	public static void showConfirmSheet(Component parentComponent, Object message, int optionType, int messageType,
			SheetListener listener) {
	 ...
	 * determined by the {@code optionType} parameter. The {@code messageType}
	 ...
	 */
	public static void showConfirmSheet(Component parentComponent, Object message, int optionType, int messageType,
			Icon icon, SheetListener listener) {
	...
	/**
	 * Shows a question-message sheet requesting input from the user and parented to
	 * {@code parentComponent}. The input value will be initialized to
	 * {@code initialSelectionValue}.
	 *
	 * @param parentComponent       the parent {@code Component} for the dialog
	 * @param message               the {@code Object} to display
	 ...
	 * @param listener              The listener for SheetEvents.
	 */
	public static void showInputSheet(Component parentComponent, Object message, Object initialSelectionValue,
			SheetListener listener) {
		showInputSheet(parentComponent, message, JOptionPane.QUESTION_MESSAGE, null, null, initialSelectionValue,
				listener);
	}
	 ...
	 * @param listener        The listener for SheetEvents.
	 */
	public static void showInputSheet(Component parentComponent, Object message, int messageType,
			SheetListener listener) {
		showInputSheet(parentComponent, message, messageType, null, null, null, listener);
	}

	/**
	 * Prompts the user for input in a sheet where the initial selection, possible
	 * selections, and all other options can be specified. The user will able to
	 * choose from {@code selectionValues}, where {@code null} implies the user can
	 * input whatever they wish, usually by means of a {@code JTextField}.
	 * {@code initialSelectionValue} is the initial value to prompt the user with.
	 * It is up to the UI to decide how best to represent the
	 * {@code selectionValues}, but usually a {@code JComboBox}, {@code JList}, or
	 ...
	 * @param selectionValues       an array of {@code Object}s that gives the
	 *                              possible selections
	 * @param initialSelectionValue the value used to initialize the input field
	 * @param listener              The listener for SheetEvents.
	 */
	public static void showInputSheet(Component parentComponent, Object message, int messageType, Icon icon,
		...
		/*
		 * sheet.addWindowListener(new WindowAdapter() { public void
	...
	/**
	 * Brings up a sheet that displays a message using a default icon determined by
	 * the {@code messageType} parameter.
	 *
	 * @param parentComponent determines the {@code Frame} in which the dialog is
	 *                        displayed; if {@code null}, or if the
	 *                        {@code parentComponent} has no {@code Frame}, the
	 ...
	 * @param listener        This listener is notified when the sheet is dismissed.
	 */
	public static void showMessageSheet(Component parentComponent, Object message, int messageType, Icon icon,
			SheetListener listener) {
	 ...
	 * determined by the {@code initialValue} parameter and the number of choices is
	 * determined by the {@code optionType} parameter.
	 * <p>
	 * If {@code optionType} is {@code YES_NO_OPTION}, or
	 * {@code YES_NO_CANCEL_OPTION} and the {@code options} parameter is
	 * {@code null}, then the options are supplied by the look and feel.
	 * <p>
	 * The {@code messageType} parameter is primarily used to supply a default icon
	 ...
	 *                        are rendered properly; non-{@code String} objects are
	 *                        rendered using their {@code toString} methods; if this
	 *                        parameter is {@code null}, the options are determined
	 *                        by the Look and Feel
	 * @param initialValue    the object that represents the default selection for
	 *                        the dialog; only meaningful if {@code options} is
	 *                        used; can be {@code null}
	 * @param listener        The listener for SheetEvents.
	 */
	public static void showOptionSheet(Component parentComponent, Object message, int optionType, int messageType,
	 ...
	 *
	 * @param parentComponent the {@code Component} to check for a {@code Frame} or
	 *                        {@code Dialog}
	 * @return the {@code Frame} or {@code Dialog} that contains the component, or
	 *         the default frame if the component is {@code null}, or does not have
	 *         a valid {@code Frame} or {@code Dialog} parent
	 */
	...
	/**
	 * Displays a "Save File" file chooser sheet. Note that the text that appears in
	 * the approve button is determined by the L&amp;F.
	 *
	...
	/**
	 * Displays an "Open File" file chooser sheet. Note that the text that appears
	 ...
	 * @param listener          The listener for SheetEvents.
	 */
	public static void showSheet(final JFileChooser chooser, Component parent, String approveButtonText,
			SheetListener listener) {
