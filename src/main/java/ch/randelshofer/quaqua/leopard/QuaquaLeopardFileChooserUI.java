 ...
 * the one provided with the native Aqua user interface on Mac OS X 10.5
 * (Leopard).
 *
...
public class QuaquaLeopardFileChooserUI extends BasicFileChooserUI implements SubtreeFileChooserUI {
	...
	/**
	 * Values greater zero indicate that the UI is adjusting. This is required to
	 * prevent the UI from changing the FileChooser's state while processing a
	 * PropertyChangeEvent fired from the FileChooser.
	 */
	...
	private KeyStroke[] KEYSTROKES = {
			...
			KeyStroke.getKeyStroke(KeyEvent.VK_BACK_QUOTE, 0), KeyStroke.getKeyStroke(KeyEvent.VK_SLASH, 0) };
	private AbstractAction keyListenerAction = new AbstractAction() {
		...
		public void actionPerformed(ActionEvent ae) {
			...
			case 'a':
				...
				break;
			/*
			 * case 'u': dir = new File( "/Applications/Utilities" ); break;
			 */
			case 'g':
				/* case '`': */
			case '/':
	...
	public void installComponents(JFileChooser fc) {
		...
		// previousButton.setIcon(new
		// javax.swing.ImageIcon(getClass().getResource("/ch/randelshofer/quaqua/panther/images/FileChooser.previousIcon.png")));
		// navigationButtonsPanel.add(previousButton, new
		// java.awt.GridBagConstraints());

		// nextButton.setIcon(new
		// javax.swing.ImageIcon(getClass().getResource("/ch/randelshofer/quaqua/panther/images/FileChooser.nextIcon.png")));
		// navigationButtonsPanel.add(nextButton, new java.awt.GridBagConstraints());

		// viewGroup.add(tableToggleButton);
		// tableToggleButton.setIcon(new
		// javax.swing.ImageIcon(getClass().getResource("/ch/randelshofer/quaqua/panther/images/FileChooser.tableIcon.png")));
		gridBagConstraints = new java.awt.GridBagConstraints();
		gridBagConstraints.insets = new java.awt.Insets(0, 5, 0, 0);
		// navigationButtonsPanel.add(tableToggleButton, gridBagConstraints);

		// viewGroup.add(browserToggleButton);
		// browserToggleButton.setIcon(new
		// javax.swing.ImageIcon(getClass().getResource("/ch/randelshofer/quaqua/panther/images/FileChooser.browserIcon.png")));
		...
		// tableScrollPane.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0,
		// 0, 0));
		...
		// Configure JBrowser
		LeopardFileRenderer fileRenderer = new LeopardFileRenderer(fc, UIManager.getIcon("Browser.expandingIcon"),
		...
		sidebarTree.setModel(sidebarTreeModel = new SidebarTreeModel(fc,
				new TreePath(getFileSystemTreeModel().getRoot()), getFileSystemTreeModel()));
		...
		Component[] dropComponents = { fc, accessoryPanel, approveButton, browser, browserScrollPane,
				// browserToggleButton,
				buttonsPanel, cancelButton, controlsPanel, directoryComboBox, fileNameLabel, fileNamePanel,
				fileNameSpringPanel, fileNameTextField, filesOfTypeLabel, filterComboBox, formatPanel,
				formatSpringPanel, mainPanel, navigationButtonsPanel, navigationPanel, navigationSpringPanel,
				newFolderButton,
				// nextButton,
				// previousButton,
				separator, splitPane,
				...
				viewsPanel, sidebarTree, sidebarScrollPane };
	...
	/**
	 * The array contains the selected dir(s) of the JFileChooser. All files have an
	 * absolute path. If no dir is selected, the length of the array is 0. Always
	 * returns a non-null value. All array elements are non-null.
	 */
	...
	/**
	 * Updates the selection in the JBrowser, to match the selected dir/s of the
	 * JFileChooser.
	 */
	private void updateSelection() {
		...
		if (fullPath != null && fullPath.getPathCount() > 0) {
			...
			directoryComboBoxModel.setPath((node.isLeaf()) ? fullPath.getParentPath() : fullPath);
		}
	...
	private void doDirectoryChanged(PropertyChangeEvent e) {
		...
		TreePath selectionPath = (browser.getSelectionPath() == null) ? null
		...
		if (!subtreeModel.getPathToRoot().isDescendant(dirPath)) {
			...
			for (Enumeration i = ((DefaultMutableTreeNode) sidebarTree.getModel().getRoot()).preorderEnumeration(); i
					.hasMoreElements();) {
...
//->the update did not occur from the browser        updateFileChooserFromBrowser();
		/*
		 * updateSelection(); updateApproveButtonState();
		 */
...
//->the update did not occur from the browser        updateFileChooserFromBrowser();
	...
	public void setFileName(String filename) {
		if (fileNameTextField != null && !fileNameTextField.hasFocus()
	...
	static class DirectoryComboBoxRenderer extends DefaultListCellRenderer {
		...
		public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected,
			...
			if (node == null) {
				return separator;
				/*
				 * File root = new File("/"); setText(getFileChooser().getName(root)); ii.icon =
...
//        
	 ...
	 * the tree model: the dir system root (aka the comouter).
	 */
	...
	/**
	 * This method is called, when the user double clicks the JBrowser, or when she
	...
	private void maybeApproveSelection(boolean allowDirectories) {
		...
		if (fc.isMultiSelectionEnabled()) {
			...
			if (filename != null) {
				File f = new File(((FileSystemTreeModel.Node) selectedPaths[0].getLastPathComponent()).getResolvedFile()
						.getParent(), filename);
			...
			FileSystemTreeModel.Node node = (FileSystemTreeModel.Node) browser.getSelectionPath()
...
//                }
	...
	public void ensureFileIsVisible(JFileChooser fc, final File f) {
		if (browser.getSelectionPaths() != null) {
			...
			TreePath fullPath = getFileSystemTreeModel().toPath(f, subtreeModel.getPathToRoot());
			TreePath subPath = getTreeModel().toSubPath(fullPath);
			if (subPath == null) {
				isAdjusting++;
		...
		public void valueChanged(TreeSelectionEvent e) {
			...
			if (sidebarTree != null && sidebarTree.getSelectionPath() != null) {
				if (sidebarTree.getSelectionPath().getLastPathComponent() instanceof SidebarTreeFileNode) {
					SidebarTreeFileNode info = (SidebarTreeFileNode) sidebarTree.getSelectionPath()
							.getLastPathComponent();
					File file = info.getResolvedFile();
					if (file == null) {
