...
//	}
	...
	private void updatePlacesNode() {
		...
		if (oldUserItemsSize > 0) {
			...
			fireTreeNodesRemoved(SidebarTreeModel.this, placesNode.getPath(), removedIndices, removedChildren);
		}
		...
		if (freshUserItems.size() > 0) {
			...
			for (int i = 0; i < freshUserItems.size(); i++) {
				...
				if (freshUserItems.get(i) == null) {
					...
					placesNode.add(freshUserItems.get(i));
				}
			}
			fireTreeNodesInserted(SidebarTreeModel.this, placesNode.getPath(), insertedIndices, insertedChildren);
		}
	}
		...
		for (int i = 0, n = modelDevicesNode.getChildCount(); i < n; i++) {
			...
			if (!modelNode.isLeaf() && !(deviceName != null && deviceName.startsWith(backupPrefix))) {
				...
				if (!isInView) {
					...
					while (insertionIndex < devicesNode.getChildCount() && comparator
							.compare((SidebarViewToModelNode) devicesNode.getChildAt(insertionIndex), newNode) < 0) {
						insertionIndex++;
					}
				...
				if (!UIManager.getBoolean("FileChooser.speed")) {
					dispatcher.dispatch(new Worker<Icon>() {
						...
						public void done(Icon value) {
							...
							SidebarTreeModel.this.fireTreeNodesChanged(SidebarTreeModel.this, placesNode.getPath(),
