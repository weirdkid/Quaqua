...
public class QuaquaComboPopup extends BasicComboPopup {
	...
	public void setVisible(boolean newValue) {
		...
		if (newValue) {
			if (!comboBox.isEditable()
					&& comboBox != KeyboardFocusManager.getCurrentKeyboardFocusManager().getFocusOwner()) {
				//
				...
				if (invokerRootPane != null) {
					 ...
					 * addUIInputMap(invokerRootPane, menuInputMap); addUIActionMap(invokerRootPane,
					 * menuActionMap);
					 */
				}
			}
	...
	private void updateCellRenderer(boolean isTableCellEditor) {
		list.setCellRenderer(
				new QuaquaComboBoxCellRenderer(comboBox.getRenderer(), isTableCellEditor, comboBox.isEditable()));
	}
	...
	/**
	 * Sets the list selection index to the selectedIndex. This method is used to
	 * synchronize the list selection with the combo box selection.
	 *
	 ...
	 * on the combo box location and the enclosing screen bounds. If no
	 * transformations are required, then the returned rectangle will have the same
	...
	protected Rectangle computePopupBounds(int px, int py, int pw, int ph) {
		...
		if (screen.contains(rect)) {
			...
			// popup intersects screen bounds but fits into screen => nudge into screen
			// bounds.
			if (rect.x < screen.x) {
				rect.x = screen.x;
	...
	protected void configurePopup() {
		...
		// setBorder( new CompoundBorder(UIManager.getBorder("PopupMenu.border"), new
		// EmptyBorder(0,0,0,0)));
		setBorder(UIManager.getBorder("ComboBox.popupBorder"));
		setFocusable(true);
	}
	 ...
	 * combo box.
	 *
	 * <strong>Warning:</strong> When overriding this method, make sure to maintain
	 * the existing behavior.
	 *
	 * @return a <code>MouseListener</code> which will be added to the combo box or
	 *         null
	 */
	@Override
	 ...
	 * the existing behavior.
	 *
	 * @return a <code>MouseMotionListener</code> which will be added to the combo
	 *         box or null
	 */
	@Override
	...
	private class Handler implements ItemListener, MouseListener, MouseMotionListener, PropertyChangeListener,
		...
		public void mousePressed(MouseEvent e) {
			if ((e.getSource() == list) || !SwingUtilities.isLeftMouseButton(e) || !comboBox.isEnabled()) {
				return;
			}
		...
		private boolean processKeyEvent(KeyEvent e, boolean pressed) {
			...
			if (!e.isConsumed() && !comboBox.isEditable() && !comboBox.isFocusOwner()) {
				...
				// System.out.println("QuaquaComboPopup@"+QuaquaComboPopup.this.hashCode()+".processKeyEvent
				// " + ks);
				if (map != null && am != null && isEnabled()) {
					...
					if (action != null) {
						e.consume();
						return SwingUtilities.notifyAction(action, ks, e, comboBox, e.getModifiers());
					}
				}
			}
			return false;
