 ...
 * @version $Id$
 */
public class QuaquaNativeToolBarBorder extends AbstractBorder
	...
	private static String getStyle(Component c) {
		...
		if (style == null || !(style instanceof String)) {
			/*
			 * boolean isTextured = QuaquaUtilities.isOnTexturedWindow(c); if (isTextured) {
			 * JRootPane rootPane = SwingUtilities.getRootPane(c); int xOffset = 0, yOffset
			 * = 0; for (Component cc = c; cc != rootPane; cc = cc.getParent()) { xOffset +=
			 * cc.getX(); yOffset += cc.getY(); } style = (yOffset == 0) ? "title" :
			 * "plain"; } else { style = "plain"; }
			 */
			style = "plain";
		}
		return (String) style;
	...
	private static Border getGradientBorder() {
		if (gradientBorder == null) {
			gradientBorder = new QuaquaNativeButtonStateBorder(Widget.buttonBevelInset, new Insets(-1, 0, -1, 0),
					new Insets(0, 0, 0, 0));
		}
		...
		if (bottomBorder == null) {
			bottomBorder = new Border() {
				...
				public void paintBorder(Component c, Graphics gr, int x, int y, int width, int height) {
					...
					Color[] gradient = (Color[]) UIManager
							.get(isActive ? "ToolBar.bottom.gradient" : "ToolBar.bottom.gradientInactive");
					if (gradient == null) {
						g.setPaint(PaintableColor.getPaint(c.getBackground(), c));
					} else if (gradient.length == 2) {
						g.setPaint(new LinearGradientPaint(new Point2D.Float(x, y + 1),
								new Point2D.Float(x, y + height - 2), new float[] { 0f, 1f }, gradient));
					} else if (gradient.length == 3) {
						g.setPaint(new LinearGradientPaint(new Point2D.Float(x, y + 1),
								new Point2D.Float(x, y + height - 2), new float[] { 0f, 0.25f, 1f }, gradient));
					} else if (gradient.length == 4) {
						g.setPaint(new LinearGradientPaint(new Point2D.Float(x, y + 1),
								new Point2D.Float(x, y + height - 2), new float[] { 0f, 0.25f, 0.5f, 1f }, gradient));
					}
