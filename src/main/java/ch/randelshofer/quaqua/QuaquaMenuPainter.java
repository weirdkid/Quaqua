...
public class QuaquaMenuPainter {
	...
	protected void paintMenuItem(QuaquaMenuPainterClient client, Graphics g, JComponent c, Icon checkIcon,
			Icon arrowIcon, Color background, Color foreground, Color disabledForeground, Color selectionForeground,
		...
		String clippedText = layoutMenuItem(menuItem, textFM, menuItem.getText(), acceleratorFM, acceleratorKeyText,
				modifiersText, menuItem.getIcon(), checkIcon, arrowIcon, menuItem.getVerticalAlignment(),
				menuItem.getHorizontalAlignment(), menuItem.getVerticalTextPosition(),
				menuItem.getHorizontalTextPosition(), viewRect, iconRect, textRect, acceleratorRect, checkIconRect,
				arrowIconRect, menuItem.getText() == null ? 0 : textIconGap, textIconGap);

		java.awt.Container container;
		for (container = menuItem.getParent(); container != null
				&& !(container instanceof JPopupMenu); container = container.getParent()) {
			// empty for-loop body
		}

		boolean isEnabled = (buttonModel.isEnabled() && (container == null || container.isVisible()));
		...
		if (acceleratorKeyText != null && !acceleratorKeyText.equals("")) {
			...
			if (modifiersText.equals("")) {
				...
				int acceleratorKeyWidth = Math.max(textFM.charWidth('M'),
						SwingUtilities.computeStringWidth(textFM, acceleratorKeyText));
				if (isLeftToRight) {
					g.setFont(acceleratorFont);
					drawString(client, g, modifiersText, mnemonicChar, acceleratorRect.x, baseline, isEnabled,
							isSelected);
					g.setFont(textFont);
					g.drawString(acceleratorKeyText,
							(acceleratorRect.x + acceleratorRect.width - kAcceleratorArrowSpace - acceleratorKeyWidth),
							baseline);
				} else {
					int x = acceleratorRect.x + kAcceleratorArrowSpace + acceleratorKeyWidth;
					g.setFont(acceleratorFont);
					drawString(client, g, modifiersText, mnemonicChar, x, baseline, isEnabled, isSelected);
		...
		if (clippedText != null && !clippedText.equals("")) {
			...
			if (view != null) {
				...
				drawString(client, g, clippedText, buttonModel.getMnemonic(), textRect.x,
						textRect.y + textFM.getAscent(), isEnabled, isSelected);
			}
		}
		if (arrowIcon != null) {
	...
	protected Dimension getPreferredMenuItemSize(JComponent c, Icon checkIcon, Icon arrowIcon, int textIconGap,
		...
		layoutMenuItem(menuItem, textFM, text, acceleratorFM, acceleratorKeyText, modifiersText, icon, checkIcon,
				arrowIcon, menuItem.getVerticalAlignment(), menuItem.getHorizontalAlignment(),
				menuItem.getVerticalTextPosition(), menuItem.getHorizontalTextPosition(), viewRect, iconRect, textRect,
				acceleratorRect, checkIconRect, arrowIconRect, text == null ? 0 : textIconGap, textIconGap);
	...
	public void drawString(QuaquaMenuPainterClient client, Graphics g, String text, int mnemonicChar, int x, int y,
	 ...
	 */
	private String layoutMenuItem(JMenuItem menuItem, FontMetrics textFM, String text, FontMetrics acceleratorFM,
			String acceleratorKeyText, String modifiersText, Icon icon, Icon checkIcon, Icon arrowIcon,
			int verticalAlignment, int horizontalAlignment, int verticalTextPosition, int horizontalTextPosition,
			Rectangle viewRect, Rectangle iconRect, Rectangle textRect, Rectangle acceleratorRect,
			Rectangle checkIconRect, Rectangle arrowIconRect, int textIconGap, int textCheckIconGap) {
		if (menuItem.getParent() != null) {
			placeholderIcon.dim = getMinimumIconSize(menuItem.getParent());
			icon = (placeholderIcon.getIconWidth() == 0) ? null : placeholderIcon;
		}

		SwingUtilities.layoutCompoundLabel(menuItem, textFM, text, icon, SwingConstants.TOP, SwingConstants.LEFT,
		...
		if (hasNoAccelerator) {
			...
			acceleratorRect.width += Math.max(textFM.charWidth('M'),
					SwingUtilities.computeStringWidth(textFM, acceleratorKeyText));
		...
		// acceleratorRect.x += viewRect.width - arrowIconRect.width -
		// acceleratorRect.width;
		acceleratorRect.x = viewRect.width - acceleratorRect.width;// - kAcceleratorArrowMargin;
		// acceleratorRect.y = viewRect.y + viewRect.height / 2 - acceleratorRect.height
		// / 2;
		acceleratorRect.y = textRect.y;
		if (isUseCheckAndArrow) {
			// arrowIconRect.x = acceleratorRect.x + acceleratorRect.width -
			// arrowIconRect.width;
