...
/**
 * QuaquaPopupFactory to work around a bug with heavy weight popups on Java 1.4
 * in full screen mode.
 * <p>
...
public class QuaquaPopupFactory extends PopupFactory {
	...
	private Popup getPopup(Component owner, Component contents, int ownerX, int ownerY, int popupType) {
		/*
		 * if (GraphicsEnvironment.isHeadless()) { return getHeadlessPopup(owner,
		 * contents, ownerX, ownerY); }
		 */
		switch (popupType) {
...
//       return null;
	...
	/**
	 * ContainerPopup consolidates the common code used in the light/medium weight
	 * implementations of <code>Popup</code>.
	 */
...
////            super.reset(owner, contents, ownerX, ownerY);
	...
	/**
	 * Converts the location <code>x</code> <code>y</code> to the parents coordinate
	 * system, returning the location.
	 */
	...
	private static class LightWeightPopup extends ContainerPopup {
		 ...
		 * <code>Popup</code> needs more space that in available in <code>owner</code>,
		...
		static Popup getLightWeightPopup(Component owner, Component contents, int ownerX, int ownerY) {
			...
			/*
			 * if (!popup.fitsOnScreen() || popup.overlappedByOwnedWindow()) { popup.hide();
			 * return null; }
			 */
			return popup;
		}
	...
	private static class MediumWeightPopup extends ContainerPopup {
		...
		public void show() {
			...
			/*
			 * Find the top level window, if it has a layered pane, add to that, otherwise
			 * add to the window.
			 */
	...
	private static class HeavyWeightPopup extends ContainerPopup {
		...
		/**
		 * Returns either a new or recycled <code>Popup</code> containing the specified
		 * children.
		 */
		...
		Component createComponent(Component owner) {
			...
			wnd.addComponentListener(new ComponentListener() {
				...
				private void updateShadow() {
					...
					wnd.getRootPane().putClientProperty("apple.awt.windowShadow.revalidateNow",
							(oldValue instanceof Integer) ? ((Integer) oldValue) + 1 : 1);
				}
