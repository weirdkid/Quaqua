...
/**
 * Mouse behavior for list-like components. This class simulates Mavericks
 * NSTableView behavior.
 */
...
public class QuaquaGenericListMouseBehavior extends MouseInputAdapter {
	...
	public void mouseDragged(MouseEvent e) {
		...
		/*
		 * NSTableView allows an unselected item to be dragged. Alas, Swing/AWT defers
		 * this decision to the transfer handler, which can (and often must) be
		 * redefined by the application. To make things look reasonable, we try to
			...
			if (mouseDragOp != 0) {
				...
				if (targetIndex != -1 && targetIndex != mouseDragOpPreviousIndex) {
					...
					if (!isMultipleSelection) {
						...
						/*
						 * Drag selection operates on an interval between the anchor index and the drag
						 * index. The effect is different if we are moving away from the anchor or
						 * towards the anchor.
						 */
						...
						if (targetIndex > anchorIndex && targetIndex > mouseDragOpPreviousIndex) {
							...
							performDragSelectionOperation(mouseDragOp, targetIndex + 1, mouseDragOpPreviousIndex,
									false);
						} else if (targetIndex <= anchorIndex && targetIndex > mouseDragOpPreviousIndex) {
							// moving toward the anchor
							performDragSelectionOperation(mouseDragOp, targetIndex - 1, mouseDragOpPreviousIndex,
	...
	/**
	 * Interpret the mouse location represented by a mouse pressed event. Set
	 * targetIndex to the index of the target item (if any). Set dragWouldTransfer
	 * to true if a drag operation started here would perform a data transfer (as
	 * opposed to a drag selection). The default behavior is that any drag gesture
	 * represents a data transfer if drag is enabled on the list. However, there are
	 * situations where data transfers happen only if the drag gesture starts in an
	 * active area of the list cell (such as Finder).
	 */
	...
	/**
	 * Identify the target index corresponding to a mouse event. Locations outside
	 * of any list cell are excluded.
	 */
	...
	/**
	 * The mouse has been pressed while outside any list cell. Perform appropriate
	 * actions.
	 */
	...
	protected void performDragSelectionOperation(int op, int index1, int index2, boolean isExtending) {
		...
		/*
		 * It is surprising that extending a shift-click by dragging deselects items
		 * that are not contiguous with the interval. I wonder if this was intentional
		 * on Apple's part. Seems like a bug.
		 */
	...
	/**
	 * Extend and/or retract a contiguous interval of selection. Do not alter the
	 * anchor index. Do not alter selected items that are not contiguous with the
	 * interval.
	 */
	...
	/**
	 * Get the anchor index. The anchor index is always valid unless the list is
	 * empty.
	 */
