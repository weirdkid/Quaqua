 ...
 * <p>
 * Images are read using the Cocoa class <a href=
...
public class OSXImageIO {
	...
	/**
	 * Reads a <code>BufferedImage</code> from the supplied <code>File</code> using
	 * the Cocoa NSImage API.
	 */
	...
	/**
	 * Reads a <code>BufferedImage</code> scaled to the specified size from the
	 * supplied <code>File</code> using the TwelveMonkeys ImageIO
	 *
...
//    	}
		...
		BufferedImageOp resampler = new ResampleOp(width, height, ResampleOp.FILTER_LANCZOS); // A good default filter,
																								// see class
																								// documentation for
																								// more info
	...
	/**
	 * Reads a <code>BufferedImage</code> from the supplied array using the Cocoa
	 * NSImage API.
	 */
	...
	/**
	 * Returns a <code>BufferedImage</code> as the result of decoding the data in
	 * the Mac OS X system clipboard using the Cocoa NSImage API.
	 */
	...
	/**
	 * Reads an image from the specified path and returns it in a TIFF encoded byte
	 * array.
	 */
	private native static byte[] nativeRead(String path);

	/**
	 * Reads an image from the specified path, scales it to the specified size and
	 * returns it in a TIFF encoded byte array.
	 * <p>
	 * A preferred size is used if it is stored in the image file, but you cannot
	 * expect the resulting Image to have these dimensions.<br>
	 * If your application needs a specific size, you should check the dimension.
	 * <p>
	 * If JNI fails to load the image (e.g. the native code could not be loaded),
	 * <code>null</code> is returned.
	 *
	 * @param path   The file containing the image.
	 * @param width  The preferred width.
	 * @param height The preferred height.
	 * @return The image loaded. <code>null</code>, if no image could be loaded by
	 *         JNI.
	 */
	 ...
	 *
	 * @return The image loaded. <code>null</code>, if no image could be loaded from
