...
public class OSXFile {
	...
	private static final int kLSItemInfoAppPrefersNative = 0x00000200; /*
																		 * Carbon app that prefers to be launched
																		 * natively
																		 */
	private static final int kLSItemInfoAppPrefersClassic = 0x00000400; /*
																		 * Carbon app that prefers to be launched in
																		 * Classic
																		 */
	...
	/**
	 * Converts the path name denoted by the file to an absolute path. Relative
	 * paths are always resolved against the home directory of the user and not
	 * against the current user.dir directory. The returned file objects represents
	 * an absolute path containing no '.' and '..' relative path components. This
	 * method acts solely on the textual representation of the file and therefore
	 * does does not necessarily canonicalize the path nor does it resolve aliases.
	 *
	 ...
	 * @param noUI Set this to true, if the alias should be resolved without user
	 *             interaction.
	...
	public static File resolveAlias(File f, boolean noUI) {
		if (isNativeCodeAvailable()) {
			...
			/*
			 * Cocoa path resolution refuses to follow certain top level links. If the
			 * result is an alias, we may have one of those top level links. The JDK support
			 * for canonical files works better!
			 */
	 ...
	 * 
	 * @return A File or null, if the serialized Alias could not be resolved.
	 * @param noUI Set this to true, if the alias should be resolved without user
	 *             interaction.
	 */
	...
	/**
	 * Returns the label of the specified file. The label is a value in the interval
	 * from 0 through 7. Returns -1 if the label could not be determined, e.g. if
	...
	/**
	 * Returns the tag names of the specified file. R
	 */
	...
	/**
	 * Returns the color of the specified label. Returns null, if the label does not
	 * have a color.
	 *
	...
	public static Color getLabelColor(int label, int type) {
		if (labelColors == null) {
			synchronized (OSXFile.class) {
				if (labelColors == null) {
					/*
					 * We use constants here, because the color values returned by the Carbon API do
					 * not match the colors used by the Finder.
					 */
	...
	/**
	 * Returns the color of the specified tag name. Returns null, if the tag does
	 * not have a color.
	 *
	 * @param tagName a tag name
	 * @param type    0=dark enabled,1=bright enabled,2=dark disabled,3=bright
	 *                enabled
	 * @return
	 */
	...
	/**
	 * Returns the icon image for the specified file. If the file does not exist, a
	 * generic image is returned. XXX - Returns null if it was not possible to get
	 * the icon image. We should return a generic image in this case!
	 */
		...
		if (isNativeCodeAvailable() && file != null) {
			try {
				...
				if (rImg instanceof BufferedImage) {
					...
					rImg.copyData(wr);
					image = new BufferedImage(rImg.getColorModel(), wr, rImg.getColorModel().isAlphaPremultiplied(),
							null);
	...
	/**
	 * Returns the QuickLook thumbnail image for the specified file. If it could not
	 * be created, native code fetches the file's icon instead.
	 * <p>
	 * Please only call this method on Mac OS X 10.6 Snow Leopard and above. Altough
	 * the native API is also present on Mac OS X 10.5 our code does not run stable
	 * there.
	 */
	...
	/**
	 * Returns the icon for the specified file. If the file does not exist, a
	 * generic icon is returned.
	 */
	...
	/**
	 * Returns the kind string of the specified file. The description is localized
	 * in the current Locale of the Finder.
	 *
	...
	/**
	 * Indicate whether the specified file is a directory that is normally viewed by
	 * the user as an ordinary file, i.e., something that can be opened in an
	 * application.
	 */
	...
	public static boolean isTraversable(File file, boolean isPackageTraversable, boolean isApplicationTraversable) {
		if (file == null) {
			...
			if ((flags & (kLSItemInfoIsPlainFile | kLSItemInfoIsContainer)) == kLSItemInfoIsContainer) {
				...
				// All applications are packages, so the application option takes priority for
				// applications.
				if (isApplication) {
	...
	/**
	 * Resolves an alias to a path String. Returns the same path if the provided
	 * path is not an alias.
	 *
	 * @param aliasPath the path to the alias to be resolved.
	 * @param noUI      Set this to true, if the alias should be resolved without
	 *                  user interaction.
	...
	/**
	 * Resolves a serialized Alias to a path String. Returns null if the resolution
	 * failed.
	 *
	...
	/**
	 * Returns the label of the file specified by the given path. The label is a
	 * value in the interval from 0 through 7. Returns -1 if the label could not be
	 * determined, e.g. if the file does not exist.
	 *
	...
	/**
	 * Returns the kind of the file specified by the given path. The kind is
	...
	/**
	 * Returns the icon image of a file as a byte array containing TIFF image data.
	 * This method may return an image of a different size, if no icon in the
	...
	/**
	 * Returns the QuickLook thumbnail image of a file as a byte array containing
	 * TIFF image data.
	 *
	 * @param path the path to the file.
	 * @param size the desired size of the icon in pixels (width and height).
	...
	/**
	 * Returns the basic item-information flags of the file specified by the given
	 * path.
	 * <p>
	 * Requests all item-information flags that are not application-specific: that
	 * is, all except kLSItemInfoIsNativeApp through kLSItemInfoAppIsScriptable.
	 * <p>
	...
	/**
	 * Return the time of last use of a file, as recorded by Launch Services. Called
	 * the "Date Last Opened" by Finder.
	 */
	 ...
	 * @return an array containing the files that satisfy the query or null if the
	...
	/**
	 * Returns the name of the file or directory at a given path in a localized form
	...
	/**
	 * Return the time of last use, as recorded by Launch Services. Called the "Date
	 * Last Opened" by Finder.
	 */
	 ...
	 * @return an array containing the paths of the files that satisfy the query or
	 *         null if the query could not be performed.
	 */
