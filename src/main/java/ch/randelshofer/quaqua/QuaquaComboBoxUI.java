...
public class QuaquaComboBoxUI extends BasicComboBoxUI implements VisuallyLayoutable {
	...
	/**
	 * Optional: if specified, these insets act as padding around the cell renderer
	 * when laying out and painting the "selected" item in the combo box.
	 * BasicComboBoxUI uses a single combo box renderer for rendering both the main
	 * combo box item and also all the items in the dropdown for the combo box.
	 * padding allows you to specify addition insets in addition to those specified
	 * by the cell renderer.
	 */
...
// Flag for calculating the display size
	...
	/**
	 * Preferred spacing between combo boxes and other components. / private final
	 * static Insets regularSpacing = new Insets(12,12,12,12); private final static
	 * Insets smallSpacing = new Insets(10,10,10,10); private final static Insets
	 * miniSpacing = new Insets(8,8,8,8);
	 */
	...
	public void installUI(JComponent c) {
		...
		if (arrowButton != null) {
			arrowButton.putClientProperty("JComponent.sizeVariant",
					comboBox.getClientProperty("JComponent.sizeVariant"));
		}
	...
	/**
	 * Create and install the listeners for the combo box and its model. This method
	...
	/**
	 * Remove the installed listeners from the combo box and its model. The number
	...
	protected void uninstallListeners() {
		 ...
		 * comboBox.removeHierarchyListener(hierarchyListener); hierarchyListener =
		 * null; }
		 */
	}
	...
	/*
	 * protected HierarchyListener createHierarchyListener() { return new
	 * ComponentActivationHandler(comboBox); }
	 */
	...
	protected JButton createArrowButton() {
		JButton button = new QuaquaComboBoxButton(this, comboBox, getArrowIcon(), comboBox.isEditable(),
	...
	public void paint(Graphics g, JComponent c) {
		if (editor != null && UIManager.getBoolean("ComboBox.changeEditorForeground")) {
	...
	//
	private class QuaquaComboBoxUIHandler implements ActionListener, FocusListener, KeyListener, LayoutManager,
		...
		private void superPropertyChange(PropertyChangeEvent e) {
			...
			if (e.getSource() == editor) {
				...
				if (propertyName == "model") {
					...
					isTableCellEditor = inTable ? true : false;
		...
		public void propertyChange(PropertyChangeEvent e) {
			...
			if (name == null) {
				...
				Boolean inTable = (Boolean) e.getNewValue();
				setTableCellEditor(inTable ? true : false);
			...
			if (arrowButton != null) {
				if (QuaquaUtilities.isLeftToRight(cb)) {
					...
					Insets buttonInsets = UIManager
							.getInsets("ComboBox.button.insets." + QuaquaUtilities.getSizeVariant(comboBox));
					...
					arrowButton.setBounds(width - getArrowWidth() - insets.right,
							insets.top + buttonInsets.top /* + margin.top - 3 */, getArrowWidth(),
							buttonSize /*- margin.top - margin.bottom*/ - (buttonInsets.top + buttonInsets.bottom));
				} else {
					arrowButton.setBounds(insets.left, insets.top, getArrowWidth(), buttonSize);
				}
			}
			if (editor != null) {
	...
	public void layoutComboBox(Container parent, QuaquaComboBoxUIHandler manager) {
		if (comboBox.isEditable()) {
			...
			if (arrowButton != null) {
				...
				if (buttonInsets != null) {
					insets = new Insets(insets.top + buttonInsets.top, insets.left + buttonInsets.left,
							insets.bottom + buttonInsets.bottom, insets.right + buttonInsets.right);
				}
	...
	protected Icon getArrowIcon() {
		if (isTableCellEditor()) {
			...
			/*
			 * The following does not work as expected: if (comboBox.isEditable()) { return
			 * UIManager.getIcon("ComboBox.smallDropDownIcon"); } else { return
			 * UIManager.getIcon("ComboBox.smallPopupIcon"); }
			 */
		} else {
			if (comboBox.isEditable()) {
	...
	protected Rectangle rectangleForCurrentValue(int width, int height) {
		...
		if (comboBox.isEditable()) {
			if (!isTableCellEditor()) {
				...
				/*
				 * insets.left--; insets.top--; insets.bottom--;
				 */
		...
		return new Rectangle(insets.left, insets.top, width - getArrowWidth() - insets.right - insets.left,
				height - insets.top - insets.bottom);
	}
			...
			if (modelSize > 0) {
				for (int i = 0; i < modelSize; i++) {
					...
					if (sameBaseline && value != null && (!(value instanceof String) || !"".equals(value))) {
						...
						try {
							newBaseline = (Integer) Methods.invoke(c, "getBaseline",
									new Class[] { Integer.TYPE, Integer.TYPE }, new Object[] { d.width, d.height });
	 ...
	 * simplified for the next major release. adding/removing the component to the
	 * currentValuePane and changing the font may be redundant operations.
	 */
	@Override
	...
	/**
	 * Note: Changes in this method also require changes in method
	 * rectangelForCurrentValue.
	 */
	@Override
	public Dimension getMinimumSize(JComponent c) {
		...
		Dimension size = null;
		if (!comboBox.isEditable() && arrowButton != null && arrowButton instanceof QuaquaComboBoxButton) {
			...
			/*
			 * TBD: This seems wrong. How do we know that the combo box height must be
			 * increased to properly contain the arrow button?
			 */
	...
	public Rectangle getVisualBounds(JComponent c, int layoutType, int width, int height) {
		...
		if (arrowButton != null) {
			if (QuaquaUtilities.isLeftToRight(comboBox)) {
				...
				buttonRect.setBounds(width - getArrowWidth() - insets.right, insets.top + margin.top - 2,
						getArrowWidth(), buttonSize - margin.top - margin.bottom + plusHeight);
			} else {
				buttonRect.setBounds(insets.left, insets.top, getArrowWidth(), buttonSize);
			}
		}
		...
		case VisuallyLayoutable.TEXT_BOUNDS:
			Object renderer = (editor == null) ? (Object) comboBox.getRenderer().getListCellRendererComponent(listBox,
					comboBox.getSelectedItem(), comboBox.getSelectedIndex(), false, comboBox.hasFocus())
					: (Object) editor;
			if ((renderer instanceof JComponent)
					&& (Methods.invokeGetter(renderer, "getUI", null) instanceof VisuallyLayoutable)) {
				bounds = ((VisuallyLayoutable) Methods.invokeGetter(renderer, "getUI", null))
						.getVisualBounds((JComponent) renderer, layoutType, editorRect.width, editorRect.height);
	 ...
	 * within subclasses of <code>BasicComboBoxUI</code>.
	 */
