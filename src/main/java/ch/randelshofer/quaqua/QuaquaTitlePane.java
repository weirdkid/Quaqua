...
public class QuaquaTitlePane extends JComponent {
	...
	private void updateIconsAndTextures() {
		...
		activeBackground = UIManager
				.getColor(prefix + ((isVertical) ? "vTitlePaneBackground" : "titlePaneBackground") + suffix);
	...
	private void setState(int state, boolean updateRegardless) {
		...
		if (frame != null && !isPalette) {
			...
			toggleButton.setEnabled(
					frame.isResizable() && Toolkit.getDefaultToolkit().isFrameStateSupported(Frame.MAXIMIZED_BOTH));
			/*
			 * } else { maximizeAction.setEnabled(false); restoreAction.setEnabled(false);
			 * if (toggleButton.getParent() != null) { remove(toggleButton); revalidate();
			 * repaint(); }
			 */
			// }
		} else {
	...
	private void updateToggleButton(Action action, Icon icon) {
		...
		toggleButton.setText(null);
		toggleButton.setEnabled(Toolkit.getDefaultToolkit().isFrameStateSupported(Frame.MAXIMIZED_BOTH));
	}
	...
	public Font getFont() {
		...
		/*
		 * Window window = getWindow(); return (window != null) ? window.getFont() :
		 * rootPane.getFont();
		 */
	}
	...
	public void paintComponent(Graphics gr) {
		...
		if (/* isPalette || */titleBorder == null) {
			g.setPaint(PaintableColor.getPaint(background, rootPane));
			g.fillRect(0, 0, width, height);
			if (darkShadow != null) {
				g.setPaint(PaintableColor.getPaint(darkShadow, rootPane));
				if (isVertical) {
		...
		if (theTitle != null) {
			...
			if (isVertical) {
				...
				int y = Math.max(Math.max(closeRect.y + closeRect.height, toggleRect.y + toggleRect.height) + 5,
						(getHeight() - titleLength) / 2) + titleLength;
				...
				g.setPaint(PaintableColor.getPaint(foreground, window));
				...
				if (UIManager.getColor("Frame.titlePaneEmbossForeground") != null) {
					g.setPaint(UIManager.getColor("Frame.titlePaneEmbossForeground"));
					g.drawString(theTitle,
							Math.max(Math.max(closeRect.x + closeRect.width, toggleRect.x + toggleRect.width) + 5,
									(getWidth() - titleLength) / 2),
							yOffset + 1);
				}
				g.setPaint(PaintableColor.getPaint(foreground, window));
				g.drawString(theTitle,
		 ...
		 * g.setColor(new Color(0x00ff00,true)); g.fill(new Polygon(new int[] {0,10,0},
		 * new int[] {0,0,10}, 3)); g.setComposite(savedComposite);
		 */
	...
	private class CloseAction extends AbstractAction {
		...
		public CloseAction() {
			super(UIManager.getString("QuaquaTitlePane.closeTitle", getLocale()));
		}
	...
	private class IconifyAction extends AbstractAction {
		...
		public IconifyAction() {
			super(UIManager.getString("QuaquaTitlePane.iconifyTitle", getLocale()));
		}
	...
	private class MaximizeAction extends AbstractAction {
		...
		public MaximizeAction() {
			super(UIManager.getString("QuaquaTitlePane.maximizeTitle", getLocale()));
		}
	...
	private class SystemMenuBar extends JMenuBar {
		...
		public void paint(Graphics gr) {
			...
			if (isOpaque()) {
				g.setPaint(PaintableColor.getPaint(getBackground(), this));
				g.fillRect(0, 0, getWidth(), getHeight());
			}
	...
	private class TitlePaneLayout implements LayoutManager {
		...
		private int computeHeight() {
			...
			/*
			 * FontMetrics fm = Toolkit.getDefaultToolkit().getFontMetrics(getFont()); int
			 * fontHeight = fm.getHeight();
			 * 
			 * 
			 * fontHeight += 2; int iconHeight = 0; // if (getWindowDecorationStyle() ==
			 * JRootPane.FRAME) { iconHeight = IMAGE_HEIGHT; // }
			 * 
			 * int finalHeight = Math.max( fontHeight, iconHeight ); return finalHeight;
			 */
		}
