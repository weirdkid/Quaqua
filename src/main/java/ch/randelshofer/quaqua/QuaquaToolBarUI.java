...
public class QuaquaToolBarUI extends BasicToolBarUI {
			...
			protected JRootPane createRootPane() {
				JRootPane rootPane = new JRootPane() {
					...
					public void validate() {
						putClientProperty("Quaqua.RootPane.isVertical",
								toolBar.getOrientation() == SwingConstants.VERTICAL ? Boolean.FALSE : Boolean.TRUE);
						super.validate();
						if (!packing) {
							packing = true;
							pack();
	...
	private String getDockingConstraint(Component c, Point p) {
		...
		if (c.contains(p)) {
			dockingSensitivity = (toolBar.getOrientation() == SwingConstants.HORIZONTAL) ? toolBar.getSize().height
	...
	private boolean isBlocked(Component comp, Object constraint) {
		if (comp instanceof Container) {
			...
			if (lm instanceof BorderLayout) {
				...
				Component c = null;
				try {
					c = (Component) Methods.invoke(blm, "getLayoutComponent",
							new Class[] { Container.class, Object.class }, new Object[] { cont, constraint });
				} catch (Throwable ex) {
					// ex.printStackTrace();
				}
	...
	protected void dragTo(Point position, Point origin) {
		if (toolBar.isFloatable()) {
			...
			if (ancestorWindow != null) {
				...
				ancestorWindow.setLocation(loc.x + position.x - dragWindowOffset.x,
						loc.y + position.y - dragWindowOffset.y);
			}
		}
	}
	...
	private String calculateConstraint() {
		...
		if (lm instanceof BorderLayout) {
			...
			try {
				constraint = (String) Methods.invoke(bl, "getConstraints", new Class[] { Component.class },
						new Object[] { toolBar });
			} catch (Throwable ex) {
				// Suppress silently
				// ex.printStackTrace();
	...
	/**
	 * Returns true, if the parent window shall be moved, when the mouse is dragged
	 * over the toolbar.
	 */
