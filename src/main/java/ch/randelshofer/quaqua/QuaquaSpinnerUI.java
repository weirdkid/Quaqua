...
public class QuaquaSpinnerUI extends BasicSpinnerUI implements VisuallyLayoutable {
	 ...
	 * returned by <code>spinner.getPreviousValue</code>. By default the
	 * <code>previousButton</code> is a JButton who's <code>ActionListener</code>
	 * updates it's <code>JSpinner</code> ancestors model. If a previousButton isn't
	 * needed (in a subclass) then override this method to return null.
	 *
	 * @return a component that will replace the spinners model with the next value
	...
	/**
	 * Create a component that will replace the spinner models value with the object
	 * returned by <code>spinner.getNextValue</code>. By default the
	...
	/**
	 * Updates the font of the children Components based on the font of the
	 * <code>JSpinner</code>.
	 */
	...
	private void installEditorFocusInputMap(JComponent editor) {
		...
		if (editor != null) {
			...
			SwingUtilities.replaceUIInputMap(editor, JComponent.WHEN_FOCUSED, iMap);
	...
	public Rectangle getVisualBounds(JComponent c, int layoutType, int width, int height) {
		...
		case VisuallyLayoutable.TEXT_BOUNDS:
			...
			if (ui instanceof VisuallyLayoutable) {
				Rectangle editorBounds = ((VisuallyLayoutable) ui).getVisualBounds(editor, layoutType, editorWidth,
						editorHeight);
