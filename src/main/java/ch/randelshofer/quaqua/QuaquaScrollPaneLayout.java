 ...
 * scroll bar of a JScrollPane do not intersect with the grow-box of a JFrame or
 * a JDialog.
 * <p>
...
public class QuaquaScrollPaneLayout extends ScrollPaneLayout {
	...
	public void layoutContainer(Container parent) {
		...
		if (fontSize >= 13 && vsbVisible != hsbVisible || fontSize < 13 && (vsbVisible || hsbVisible)) {
			...
			if (rootPane != null && (rootPane.getParent() instanceof Frame
					&& ((Frame) rootPane.getParent()).isResizable()
				...
				if (x + parent.getWidth() > rootPane.getWidth() - growBox.width
					...
					if (vsbVisible) {
						vsb.setSize(vsb.getWidth(),
								parent.getHeight() - insets.top - Math.max(insets.bottom, growBox.height));
					}
					if (hsbVisible) {
						hsb.setSize(parent.getWidth() - insets.left - Math.max(insets.right, growBox.width),
								hsb.getHeight());
					}
				}
			}
