...
public class Debug {
	...
	public static void paint(Graphics gr, JComponent c, ComponentUI ui) {
		...
		if (UIManager.getBoolean("Quaqua.Debug.showVisualBounds") && ui instanceof VisuallyLayoutable) {

			VisuallyLayoutable layoutable = (VisuallyLayoutable) ui;
			g.setColor(UIManager.getColor("Quaqua.Debug.componentBoundsForeground"));
			Rectangle rect = layoutable.getVisualBounds(c, VisuallyLayoutable.COMPONENT_BOUNDS, c.getWidth(),
					c.getHeight());
		...
		/*
		 * if (QuaquaManager.getBoolean("Quaqua.Debug.colorizePaintEvents")) { if
		 * (EventQueue.getCurrentEvent() != previousEvent) { previousEvent =
		 * EventQueue.getCurrentEvent(); rainbow = (rainbow + 30) % 360; } g.setColor(
		 * new Color(0x22ffffff & Color.HSBtoRGB((float) (rainbow / 360f), 0.4f,
		 * 1.0f),true) ); g.fillRect(0,0,c.getWidth(),c.getHeight()); }
		 */
