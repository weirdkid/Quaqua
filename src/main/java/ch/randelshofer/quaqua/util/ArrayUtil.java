...
public class ArrayUtil {
	...
	/*
	 * public static Vector asVector(int[] a) { Vector list = new Vector(a.length);
	 * for (int i=0; i < a.length; i++) { list.addElement(new Integer(a[i])); }
	 * return list; } public static List asList(Object[] a) { return
	 * Arrays.asList(a); }
	 */
	public static ArrayList asList(int[] a) {
		 ...
		 * public static ArrayList asList(int[] a, int off, int len) { ArrayList list =
		 * new ArrayList(a.length); int end = off + len; for (; off < end; off++) {
		 * list.add(new Integer(a[off])); } return list; }
		 */
