...
public class Fonts {
	...
	/**
	 * Default font render context for Mac OS X. XXX - We should create the default
	 * depending on properties of the current Look and Feel.
	 */
	private static FontRenderContext defaultFontRenderContext = new FontRenderContext(new AffineTransform(), true,
			true);
	...
	/**
	 * The dialog font is used for text in menus, modeless dialogs, and titles of
	 * document windows.
	 */
	...
	/**
	 * Use emphasized dialog fonts sparingly. Emphasized (bold) dialog font is used
	 * in only two places in the interface: the application name in an About window
	 * and the message text in an option pane.
	 */
	...
	/**
	 * The small dialog font is used for informative text in alerts. It is also the
	 * default font for headings in lists, for help tags, and for text in the small
	 * versions of many controls. You can also use it to provide additional
	 * information about settings in various windows.
	 */
	...
	/**
	 * You might use emphasized small dialog font to title a group of settings that
	 * appear without a group box, or for brief informative text below a text field.
	 */
	...
	/**
	 * If your application creates text documents, use the application font as the
	 * default for user-created content.
	 */
	 ...
	 * bevel buttons. You should rarely need to use this font in dialogs, but may
	 * find it useful in utility windows when space is at a premium.
	 */
	...
	/**
	 * If necessary, the mini dialog font can be used for utility window labels and
	 * text.
	 */
	...
	/**
	 * Puts an HTML font tag for the Emphasized Dialog Font around the specified
	 * text.
	 */
	...
	/**
	 * Returns the perceived bounds of the specified string, if it is rendered using
	 * the specified font on the provided component.
	 */
	public static Rectangle getPerceivedBounds(String str, Font f, Component c) {
		if (str == null) {
			...
			int scriptSystem = (str == null || str.length() == 0) ? ScriptSystem.LATIN
					: ScriptSystem.getScriptSystemOf(str.charAt(0));
	...
	/**
	 * Returns the perceived ascent of the specified font, if text is written using
	 * the specified script system and font render context.
	 * <p>
	 * The perceived ascent is a distance above the baseline, chosen by the font
	 * designer and the same for all glyphs in a script system, that often
	 * corresponds approximately to the tops of the uppercase letters in a Latin
	 * script system. Uppercase letters are chosen because, among the regularly used
	 * glyphs in a font, they are generally the tallest.
	 * <p>
	 * The value returned by this method differs from the ascent returned by
	 * java.awt.font.LineMetrics.getAscent(), in that the perceived ascent does not
	 * include diacritical marks of the font. Therefore the perceived ascent is
	 * useful for visually north alignig text with graphical elements.
	 */
	...
	/**
	 * Returns the perceived descent of the specified font, if text is written using
	 * the specified script system and font render context.
	 * <p>
	 ...
	 * whithout the descenders of lower case characters. In a Latin script system
	 * the perceived descent is usually 0. The descent is the same distance from the
	 * baseline for all glyphs in the script system.
	 * <p>
	 * The value returned by this method differs from the one that would be returned
	 * by java.awt.font.LineMetrics.getAscent(), in that the perceived descent does
	 * not include the descender of lower case characters and diacritical marks
	 * below them. Therefore the perceived descent is useful for visully south
	 * aligning text with graphical elements.
	 */
