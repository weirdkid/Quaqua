...
public class BinaryPListParser {
	 ...
	 *
	 * objectTable ::= { null | bool | fill | number | date | data | string | uid |
	 * array | dict }
	 *
	 * null ::= 0b0000 0b0000
	 *
	 ...
	 * 0b1111 then a int // variable-sized object follows // to indicate the number
	 * of bytes
	 *
	 * string ::= asciiString | unicodeString asciiString ::= 0b0101 0bnnnn [int]
	 * byte* unicodeString ::= 0b0110 0bnnnn [int] short* // nnnn is number of bytes
	 ...
	 * unless 0b1111 then a int // variable-sized object follows // to indicate the
	 * number of objref
	 *
	 * dict ::= 0b1010 0bnnnn [int] keyref* objref* // nnnn is number of keyref and
	 * // objref pairs // unless 0b1111 then a int // variable-sized object follows
	 * // to indicate the number of pairs
	 *
	 * objref = byte | short // if refCount // is less than 256 then objref is // an
	 * unsigned byte, otherwise it // is an unsigned big-endian short
	 *
	 * keyref = byte | short // if refCount // is less than 256 then objref is // an
	 * unsigned byte, otherwise it // is an unsigned big-endian short
	 *
	 ...
	 *
	 *
	 * offsetTable ::= { int } // List of ints, byte size of which // is given in
	 * trailer // These are the byte offsets into // the file. // The number of the
	 * ffsets is given // in the trailer.
	 *
	 * trailer ::= refCount offsetCount objectCount topLevelOffset
	 *
	 * refCount ::= byte*8 // unsigned big-endian long offsetCount ::= byte*8 //
	 * unsigned big-endian long objectCount ::= byte*8 // unsigned big-endian long
	 * topLevelOffset ::= byte*8 // unsigned big-endian long </pre>
	 */
	/**
	...
	/**
	 * Object table. We gradually fill in objects from the binary PList object table
	 * into this list.
	 */
	...
	/**
	 * Parses a binary PList file and turns it into a XMLElement. The XMLElement is
	 * equivalent with a XML PList file parsed using NanoXML.
	 *
	...
	/**
	 * Object Formats (marker byte followed by additional info in some cases) null
	 * 0000 0000 bool 0000 1000 // false bool 0000 1001 // true fill 0000 1111 //
	 * fill byte int 0001 nnnn ... // # of bytes is 2^nnnn, big-endian bytes real
	 * 0010 nnnn ... // # of bytes is 2^nnnn, big-endian bytes date 0011 0011 ... //
	 * 8 byte float follows, big-endian bytes data 0100 nnnn [int] ... // nnnn is
	 * number of bytes unless 1111 then int count follows, followed by bytes string
	 * 0101 nnnn [int] ... // ASCII string, nnnn is # of chars, else 1111 then int
	 * count, then bytes string 0110 nnnn [int] ... // Unicode string, nnnn is # of
	 * chars, else 1111 then int count, then big-endian 2-byte shorts 0111 xxxx //
	 * unused uid 1000 nnnn ... // nnnn+1 is # of bytes 1001 xxxx // unused array
	 * 1010 nnnn [int] objref* // nnnn is count, unless '1111', then int count
	 * follows 1011 xxxx // unused 1100 xxxx // unused dict 1101 nnnn [int] keyref*
	 * objref* // nnnn is count, unless '1111', then int count follows 1110 xxxx //
	 * unused 1111 xxxx // unused
	 */
	private void parseObjectTable(DataInputStream in) throws IOException {
		...
		while ((marker = in.read()) != -1) {
			...
			// Integer.toBinaryString(marker)+" 0x"+Integer.toHexString(marker)+"
			// @0x"+Long.toHexString(getPosition()));
	...
	/**
	 * Reads a count value from the object table. Count values are encoded using the
	 * following scheme:
	 *
	...
	/**
	 * null 0000 0000 bool 0000 1000 // false bool 0000 1001 // true fill 0000 1111
	 * // fill byte
	 */
	...
	/**
	 * array 1010 nnnn [int] objref* // nnnn is count, unless '1111', then int count
	 * follows
	 */
	...
	/**
	 * string 0110 nnnn [int] ... // Unicode string, nnnn is # of chars, else 1111
	 * then int count, then big-endian 2-byte shorts
	 */
