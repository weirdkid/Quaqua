...
public class Images {
	...
	public static BufferedImage toBufferedImage(Image image) {
		 ...
		 * BufferedImage bufImg; Frame f = new Frame(); f.pack(); MediaTracker t = new
		 * MediaTracker(f); t.addImage(image, 0); try { t.waitForAll(); } catch
		 * (InterruptedException e) {}
		 * 
		 * // Workaround for Java 1.4.1 on Mac OS X. if
		 * (System.getProperty("java.version").startsWith("1.4.1_")) { bufImg = new
		 * BufferedImage(image.getWidth(f), image.getHeight(f),
		 * BufferedImage.TYPE_INT_ARGB); } else { bufImg = GraphicsEnvironment
		 * .getLocalGraphicsEnvironment() .getDefaultScreenDevice()
		 * .getDefaultConfiguration() .createCompatibleImage(image.getWidth(null),
		 * image.getHeight(null), Transparency.TRANSLUCENT); } Graphics2D imgGraphics =
		 * bufImg.createGraphics(); imgGraphics.drawImage(image, 0, 0, f);
		 * imgGraphics.dispose(); f.dispose(); return bufImg; }
		 */
	}
