...
public class QuaquaMenuItemUI extends BasicMenuItemUI implements QuaquaMenuPainterClient {
	 ...
	 * createMouseInputListener(menuItem)) != null) {
	 * menuItem.addMouseListener(mouseInputListener);
	 * menuItem.addMouseMotionListener(mouseInputListener); } if
	 * ((menuDragMouseListener = createMenuDragMouseListener(menuItem)) != null) {
	 * menuItem.addMenuDragMouseListener(menuDragMouseListener); } if
	 * ((menuKeyListener = createMenuKeyListener(menuItem)) != null) {
	 * menuItem.addMenuKeyListener(menuKeyListener); if (menuKeyListener instanceof
	...
	protected void paintMenuItem(Graphics g, JComponent c, Icon checkIcon, Icon arrowIcon, Color background,
		...
		QuaquaMenuPainter.getInstance().paintMenuItem(this, g, c, checkIcon, arrowIcon, background, foreground,
				disabledForeground, selectionForeground, defaultTextIconGap, acceleratorFont);
	}

	@Override
	protected Dimension getPreferredMenuItemSize(JComponent c, Icon checkIcon, Icon arrowIcon, int defaultTextIconGap) {
		return QuaquaMenuPainter.getInstance().getPreferredMenuItemSize(c, checkIcon, arrowIcon, defaultTextIconGap,
				acceleratorFont);
	}
	...
	public void paintBackground(Graphics g, JComponent component, int menuWidth, int menuHeight) {
		...
		/*
		 * if (component.getBorder() instanceof BackgroundBorder) { Border b =
		 * ((BackgroundBorder) component.getBorder()).getBackgroundBorder();
		 * b.paintBorder(component, g, 0, 0, component.getWidth(),
		 * component.getHeight()); }
		 */
	}
}
