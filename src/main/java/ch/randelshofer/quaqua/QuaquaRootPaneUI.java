...
public class QuaquaRootPaneUI extends BasicRootPaneUI {
	...
	/**
	 * This variable is set to true if we determine that the platform supports the
	 * Window.documentModified property.
	 */
	private static boolean isDocumentModifiedSupported = false;
	/**
	 * This method is used to access the non-API peer methods of Apple's Window
	 * peers. The method is different for the different MRJ versions.
	 */
	...
	/**
	 * Since method Window.getWindows() is only available since Java 1.6, we
	 * indirectly keep track of all windows by ourselves by storing all JRootPanes
	 * in this weak hash map. We add a JRootPane to this map, upon installUI, and
	 * remove a JRootPane from this upoin deinstallUI.
	 */
	...
	public void paint(Graphics g, JComponent c) {
		...
		if (style != JRootPane.NONE) {
			...
			if (needsResizeIcon) {
				...
				resizeIcon.paintIcon(c, g, w - resizeIcon.getIconWidth() - insets.right,
	...
	public void update(Graphics gr, final JComponent c) {
		...
		/*
		 * root.putClientProperty( "apple.awt.windowShadow.revalidateNow", Boolean.TRUE
		 * );
		 */
	...
	private void installClientDecorations(JRootPane root) {
		...
		/*
		 * window = SwingUtilities.getWindowAncestor(root); if (window != null) {
...
// Reset the cursor, as we may have changed it to a resize cursor
	 ...
	 * <code>defaultButton</code> has changed, this will reinstall the keyboard
	 * actions.
	 */
	@Override
	...
	private static class RootPaneAncestorListener implements AncestorListener, WindowListener {
		...
		public void ancestorAdded(AncestorEvent evt) {
			...
			Window window = (ancestor instanceof Window) ? (Window) ancestor
		...
		public void ancestorRemoved(AncestorEvent evt) {
			...
			if (ancestorParent != null) {
				Window window = (ancestorParent instanceof Window) ? (Window) ancestorParent
...
//       would force this to be non-static.
	private static class QuaquaRootLayout implements LayoutManager2 {
		...
		public Dimension preferredLayoutSize(Container parent) {
			...
			if (root.getWindowDecorationStyle() != JRootPane.NONE && (root.getUI() instanceof QuaquaRootPaneUI)) {
			...
			if (isVertical) {
				return new Dimension(Math.max(cpWidth, mbWidth) + tpWidth + i.left + i.right,
	...
	private class MouseInputHandler implements MouseInputListener {
		...
		/**
		 * We cache the screen bounds here, so that we don't have to retrieve them for
		 * each mouseDragged event. We clear the cache on mouseReleased.
		 */
		...
		public void mousePressed(MouseEvent ev) {
			...
			if (getTitlePane() != null && getTitlePane().contains(convertedDragWindowOffset)) {
				...
				dragCursor = (dragWindowOffset.x >= dragWidth - BORDER_DRAG_THICKNESS
		...
		public void mouseDragged(MouseEvent ev) {
			...
			if (isMovingWindow) {
				...
				if (w.isShowing()) {
					...
					boolean isOnDefaultScreen = w.getGraphicsConfiguration().getDevice() == GraphicsEnvironment
						...
						for (Iterator i = snapBounds.iterator(); i.hasNext();) {
							...
							if (snapper.intersects(windowRect)) {
								...
								if (windowPt.y > r.y - snap && windowPt.y < r.y + snap) {
									...
									windowPt.y = r.y;
								} else if (windowPt.y > r.y + r.height - snap && windowPt.y < r.y + r.height + snap) {
					...
					if (isOnDefaultScreen) {
						...
						if (isVertical(getRootPane())) {
							...
							windowPt.x = Math.min(cachedScreenBounds.x + cachedScreenBounds.width - titlePaneBounds.x
									- titlePaneBounds.width, windowPt.x);
							...
							windowPt.y = Math.min(cachedScreenBounds.y + cachedScreenBounds.height - titlePaneBounds.y
									- titlePaneBounds.height, windowPt.y);
