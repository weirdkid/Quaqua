 ...
 *          <br>
 *          4.0 2007-07-26 Add support for client property
 *          "Quaqua.TextField.style" "search", "plain". <br>
 *          3.1 2007-04-12 Honour margin of JTextComponent. <br>
 *          3.0.1 2006-01-04 Non-editable text field border must be same as
 *          disabled text field border. <br>
 *          3.0 2005-12-08 Rewritten to create border images lazily and to not
 *          hardcode the URL to the border images. <br>
 *          2.0 2005-10-01 Turned into a background border. <br>
 *          1.3 2005-06-20 1.2 Changed border insets to achieve baseline
 *          alignment with other components. <br>
 *          1.2 2005-04-10 1.1.1 Alignemnt changed for small size text fields.
 *          <br>
 *          1.1 2005-03-27 1.1 Updated to take account of larger focus ring and
 *          of margins. <br>
...
public class QuaquaTextFieldBorder extends VisualMarginBorder implements BackgroundBorder {
	...
	private static class BgBorder implements Border, Serializable {
		...
		public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
			...
			if (isSearchField(c)) {
				...
				g.fillRoundRect(insets.left + 4, insets.bottom + 3, width - insets.left - insets.right - 8,
						height - insets.top - insets.bottom - 7, arc, arc);
			} else {
				g.fillRect(insets.left + 3, insets.bottom + 3, width - insets.left - insets.right - 6,
	...
	public QuaquaTextFieldBorder(String imagesLocation, Insets imageInsets, String searchImagesLocation,
			Insets searchImageInsets, String smallSearchImagesLocation, Insets smallSearchImageInsets) {
		super(3, 3, 2, 3);
	...
	public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
		...
		border.paintBorder(c, g, x + margin.left, y + margin.top, width - margin.left - margin.right,
		...
		if (isSearchField(c)) {
			...
			if (isSmall) {
				if (smallSearchBorders == null) {
					smallSearchBorders = (Border[]) QuaquaBorderFactory.create(smallSearchImagesLocation,
							smallSearchImageInsets, 3, true);
				}
				borders = smallSearchBorders;
			} else {
				if (searchBorders == null) {
					searchBorders = (Border[]) QuaquaBorderFactory.create(searchImagesLocation, searchImageInsets, 3,
							true);
				}
	...
	public static class UIResource extends QuaquaTextFieldBorder implements javax.swing.plaf.UIResource {
		...
		public UIResource(String imagesLocation, String searchImagesLocation, String smallSearchImagesLocation) {
			this(imagesLocation, new Insets(6, 6, 5, 6), searchImagesLocation, new Insets(13, 13, 13, 13),
					smallSearchImagesLocation, new Insets(11, 13, 11, 13));
		}

		public UIResource(String imagesLocation, Insets imageInsets, String searchImagesLocation,
				Insets searchImageInsets, String smallSearchImagesLocation, Insets smallSearchImageInsets) {
			super(imagesLocation, imageInsets, searchImagesLocation, searchImageInsets, smallSearchImagesLocation,
