...
public class QuaquaSliderUI extends BasicSliderUI implements VisuallyLayoutable {
	...
	protected void calculateThumbLocation() {
		...
		if (slider.getOrientation() == SwingConstants.HORIZONTAL) {
			...
			/*
			 * if (slider.getPaintTicks()) { if (QuaquaUtilities.isLeftToRight(slider)) {
			 * thumbRect.y += 3; } else { thumbRect.y -= 3; } }
			 */
		} else {
			...
			/*
			 * if (slider.getPaintTicks()) { if (QuaquaUtilities.isLeftToRight(slider)) {
			 * thumbRect.x += 3; } else { thumbRect.x -= 3; } }
			 */
		}
	}
	...
	public void paintTrack(Graphics g) {
		...
		if (slider.getOrientation() == SwingConstants.HORIZONTAL) {
			...
			border.paintBorder(slider, g, trackBounds.x - thumbRect.width / 2 + 3,
					trackBounds.y + (trackBounds.height - insets.top - insets.bottom) / 2 + offset,
					trackBounds.width + thumbRect.width - 6, insets.top + insets.bottom);
		} else {
			int index = slider.isEnabled() ? 0 : 1;
			...
			border.paintBorder(slider, g, x + offset, trackBounds.y - thumbRect.height / 2 + 3,
					insets.left + insets.right, trackBounds.height + thumbRect.height - 6);
		}
	}
	...
	private class Handler implements ChangeListener, ComponentListener, FocusListener, PropertyChangeListener {
		...
		public void propertyChange(PropertyChangeEvent e) {
			...
			if (name == "orientation" || name == "inverted" || name == "labelTable" || name == "majorTickSpacing"
					|| name == "minorTickSpacing" || name == "paintTicks" || name == "paintTrack"
					|| name == "paintLabels") {
				calculateGeometry();
				slider.repaint();
