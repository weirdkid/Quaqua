...
public class QuaquaListUI extends BasicListUI {
	...
	protected void paintCell(Graphics g, int row, Rectangle rowBounds, ListCellRenderer cellRenderer,
		...
		if (list.isSelectedIndex(row)) {
			((Graphics2D) g).setPaint(PaintableColor.getPaint(
					UIManager.getColor(isComboPopup ? "ComboBox.selectionBackground" : "List.selectionBackground"),
					rendererComponent, cx, cy, cw, ch));
	 ...
	 * calls paintCell as necessary. Subclasses may want to override these methods.
	 *
	 * @see #paintCell
	 */
	@Override
	public void paint(Graphics g, JComponent c) {
		...
		Color selectionBackground = UIManager
				.getColor(isComboPopup ? "ComboBox.selectionBackground" : "List.selectionBackground");
		Color selectionForeground = UIManager
				.getColor(isComboPopup ? "ComboBox.selectionForeground" : "List.selectionForeground");
	 ...
	 * list.addMouseMotionListener(defaultDragRecognizer); super.installListeners();
	 * 
	 * // Remove the dreaded BasicDragGestureRecognizer from the list boolean
	 * removalSuccessful = false; MouseListener[] ml = list.getMouseListeners(); for
	 * (int i = 0; i < ml.length; i++) { if (ml[i].getClass().getName().equals(
	 * "javax.swing.plaf.basic.BasicListUI$ListDragGestureRecognizer")) {
	 * list.removeMouseListener(ml[i]); removalSuccessful = true; } }
	 * MouseMotionListener[] mml = list.getMouseMotionListeners(); for (int i = 0; i
	 * < mml.length; i++) { if (mml[i].getClass().getName().equals(
	 * "javax.swing.plaf.basic.BasicListUI$ListDragGestureRecognizer")) {
	 * list.removeMouseMotionListener(mml[i]); } } if (!removalSuccessful) {
	...
	/**
	 * Returns a new instance of QuaquaListUI. QuaquaListUI delegates are allocated
	 * one per JList.
	 *
	 ...
	 * updateLayoutStateNeeded. If the JLists model changes we additionally remove
	 * our listeners from the old model. Likewise for the JList selectionModel.
	 * <p>
	...
	protected class ListDataHandler implements ListDataListener {
		...
		public void intervalAdded(ListDataEvent e) {
			...
			/*
			 * Repaint the entire list, from the origin of the first added cell, to the
		...
		public void intervalRemoved(ListDataEvent e) {
			...
			/*
			 * Repaint the entire list, from the origin of the first removed cell, to the
			 * bottom of the component.
			 */
	...
	/*
	 * private static final ListDragGestureRecognizer defaultDragRecognizer = new
	 * ListDragGestureRecognizer();
	 * 
	 * /** Drag gesture recognizer for JList components / static class
	 * ListDragGestureRecognizer extends QuaquaDragGestureRecognizer {
	 * 
	 * /** Determines if the following are true: <ul> <li>the press event is located
	 * over a selection <li>the dragEnabled property is true <li>A TranferHandler is
	 * installed </ul> <p> This is implemented to perform the superclass behavior
	 * followed by a check if the dragEnabled property is set and if the location
	 * picked is selected. / protected boolean isDragPossible(MouseEvent e) { if
	 * (super.isDragPossible(e)) { JList list = (JList) this.getComponent(e); if
	 * (list.getDragEnabled()) { QuaquaListUI ui = (QuaquaListUI) list.getUI(); int
	 * row = ui.locationToIndex(list, e.getPoint()); if ((row != -1) &&
	 * list.isSelectedIndex(row)) { return true; /* } else if (row != -1 &&
	 * list.getCellBounds(row, row).contains(e.getPoint())) { return true; * / } } }
	 * return false; } }
	 */
}
