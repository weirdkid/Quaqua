...
public class QuaquaPasswordView extends FieldView {
	...
	protected char getEchoChar(JPasswordField field) {
		return field.getEchoChar() == '*' ? '\u2022' : // Unicode BULLET
				field.getEchoChar();
	}

	/**
	 * Renders the given range in the model as normal unselected text. This sets the
	 * foreground color and echos the characters using the value returned by
	 * getEchoChar().
	 *
	 ...
	 * render the text in the color specified in the hosting component. It assumes
	 * the highlighter will render the selected background. Uses the result of
	 * getEchoChar() to display the characters.
	 *
	 * @param g  the graphics context
	 * @param x  the starting X coordinate &gt;= 0
	 * @param y  the starting Y coordinate &gt;= 0
	...
	/**
	 * Renders the echo character, or whatever graphic should be used to display the
	 * password characters. The color in the Graphics object is set to the
