...
public class QuaquaScrollPaneUI extends BasicScrollPaneUI implements VisuallyLayoutable {
	 ...
	 * before the value of "extent" was set in right-to-left component orientation.
	 */
	...
	/**
	 * Creates an instance of MouseWheelListener, which is added to the JScrollPane
	 * by installUI(). The returned MouseWheelListener is used to handle mouse
	...
	public void update(Graphics g, JComponent c) {
		if (c.isOpaque()) {
			...
			g.fillRect(margin.left, margin.top, c.getWidth() - margin.left - margin.right,
		...
		if (viewport != null) {
			...
			if (hsb != null) {
				...
				if (ltr) {
					...
					/*
					 * Use a particular formula to calculate "value" until effective x coordinate is
					 * calculated.
					 */
					if (setValueCalled && ((max - currentValue) == viewPosition.x)) {
						...
						if (extent > max) {
							...
							/*
							 * The following line can't handle a small value of viewPosition.x like
							 * Integer.MIN_VALUE correctly because (max - extent - viewPositoiin.x) causes
							 * an overflow. As a result, value becomes zero. (e.g.
							 * setViewPosition(Integer.MAX_VALUE, ...) in a user program causes a overflow.
							 * Its expected value is (max - extent).) However, this seems a trivial bug and
							 * adding a fix makes this often-called method slow, so I'll leave it until
							 * someone claims.
							 */
	...
	class Handler implements ChangeListener, PropertyChangeListener, MouseWheelListener {
		...
		public void mouseWheelMoved(MouseWheelEvent e) {
			if (scrollpane.isWheelScrollingEnabled() && e.getWheelRotation() != 0) {
				...
				int orientation = (e.getModifiersEx() & InputEvent.SHIFT_DOWN_MASK) == 0 ? SwingConstants.VERTICAL //
						: SwingConstants.HORIZONTAL;
				JScrollBar toScroll = orientation == SwingConstants.VERTICAL ? scrollpane.getVerticalScrollBar()
				...
				if (orientation == SwingConstants.VERTICAL
						...
						|| orientation == SwingConstants.HORIZONTAL && scrollpane
								.getHorizontalScrollBarPolicy() == ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER) {
					JScrollPane parentPane = (JScrollPane) SwingUtilities.getAncestorOfClass(JScrollPane.class,
							scrollpane);
					if (parentPane != null) {
		...
		private void hsbStateChanged(JViewport viewport, ChangeEvent e) {
			...
			if (scrollpane.getComponentOrientation().isLeftToRight()) {
				...
				/*
				 * If setValue() was called before "extent" was fixed, turn setValueCalled flag
				 * on.
				 */
				if ((extent == 0) && (value != 0) && (oldX == max)) {
					...
					/*
					 * When a pane without a horizontal scroll bar was reduced and the bar appeared,
					 * the viewport should show the right side of the view.
					 */
