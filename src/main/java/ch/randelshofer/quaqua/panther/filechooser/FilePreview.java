...
public class FilePreview extends JPanel implements BrowserPreviewRenderer {
	...
	public Component getPreviewRendererComponent(JBrowser browser, TreePath[] paths) {
		...
		if (paths.length == 1) {
			...
			nameText.setText(MessageFormat.format(UIManager.getString("FileChooser.items"),
			...
			for (Iterator i = kinds.entrySet().iterator(); i.hasNext();) {
				...
				buf.append(MessageFormat.format(UIManager.getString("FileChooser." + entry.getKey() + "Count"),
		...
		sizeText.setText(MessageFormat.format(UIManager.getString(label),
				new Object[] { scaledLength, fileLength, paths.length }));
		updatePreviewIcon();
		return this;
	}
