...
/**
 * A replacement for Apple's AquaTabbedPaneUI for Mac OS X 10.3 Panther. All
 * tabs are placed in one run. Tabs on the left and on the right are rotated by
 * 90 degrees. If the tabs don't fit into one run, they are shortened. Tabs on
 * the top and on the bottom are only shortend as at least one tab can be shown
 * fully. If this isn't possible, a combo box is shown instead of the tabs.
 * <p>
 * Supports the following client properties on the JTabbedPane:
 * <ul>
 * <li><code>Quaqua.TabbedPane.contentBorderPainted</code> specifies whether the
 * content border should be painted.</li>
 * </ul>
 * <p>
 * Supports the following client properties on the children of the JTabbedPane:
 * <ul>
 * <li><code>Quaqua.TabbedPaneChild.contentBackground</code> specifies the
 * background Color to be used to fill the content border.</li>
 * <li><code>Quaqua.TabbedPaneChild.contentInsets</code> specifies the insets to
 * be used to lay out the child component inside the JTabbedPane.</li>
 * </ul>
 *
 *
...
public class QuaquaPantherScrollTabbedPaneUI extends BasicTabbedPaneUI implements NavigatableTabbedPaneUI {
	...
	/**
	 * Visual margin of the tabs. This margin depends on the artwork used by the tab
	 * background.
	 */
	...
	/**
	 * Changeable UIManager property prefix. The value is changed by
	...
	protected void installDefaults() {
		...
		LookAndFeel.installColorsAndFont(tabPane, prefix + "background", prefix + "foreground", prefix + "font");
	...
	protected Insets getTabInsets(int tabPlacement, int tabIndex) {
		...
		if (tCount == 1) {
			...
			insets = isSmall
					? new Insets(smallOuterTabInsets.top, smallOuterTabInsets.left, smallTabInsets.bottom,
							smallTabInsets.right)
					: new Insets(outerTabInsets.top, outerTabInsets.left, tabInsets.bottom, tabInsets.right);
		} else if (tabIndex == tCount - 1) {
			insets = isSmall
					? new Insets(smallTabInsets.top, smallTabInsets.left, smallOuterTabInsets.bottom,
							smallOuterTabInsets.right)
	...
	protected void paintContentBorder(Graphics gr, int tabPlacement, int selectedIndex) {
		...
		if (contentBackground != null) {
			g.setPaint(PaintableColor.getPaint(contentBackground, tabPane));
	...
	protected String layoutTabLabel(int tabPlacement, FontMetrics metrics, int tabIndex, String title, Icon icon,
		...
		String croppedLabel = SwingUtilities.layoutCompoundLabel(tabPane, metrics, title, icon, SwingConstants.CENTER,
				SwingConstants.CENTER, SwingConstants.CENTER, SwingConstants.TRAILING, tabRect, iconRect, textRect,
				textIconGap);
	...
	@Override
	protected void paintTab(Graphics gr, int tabPlacement, Rectangle[] rects, int tabIndex, Rectangle iconRect,
			Rectangle textRect) {
		Graphics2D g = (Graphics2D) gr;

		Rectangle tabRect = rects[tabIndex];
		if (tabPlacement == LEFT) {
			tabRect = new Rectangle(tabPane.getHeight() - tabRect.y - tabRect.height, tabRect.x, tabRect.height,
					tabRect.width);
		} else if (tabPlacement == RIGHT) {
			tabRect = new Rectangle(tabRect.y, tabPane.getWidth() - tabRect.x - tabRect.width, tabRect.height,
					tabRect.width);
		}
		int selectedIndex = tabPane.getSelectedIndex();
		...
		innerTabRect = new Rectangle(tabRect.x + insets.left, tabRect.y, // + insets.top,
				tabRect.width - insets.left - insets.right, tabRect.height// - insets.top - insets.bottom
		);

		title = layoutTabLabel(tabPlacement, metrics, tabIndex, title, icon, innerTabRect, iconRect, textRect,
				isSelected);
		/*
		 * g.setColor(Color.red); g.draw(textRect); g.setColor(Color.green);
		 * g.draw(innerTabRect); g.setColor(Color.blue); g.draw(iconRect);
		 */
	...
	private void ensureCurrentLayout() {
		...
		/*
		 * If tabPane doesn't have a peer yet, the validate() call will silently fail.
		 * We handle that by forcing a layout if tabPane is still invalid. See bug
		 * 4237677.
		 */
		if (!tabPane.isValid()) {
	...
	protected int calculateTabAreaHeight(int tabPlacement, int horizRunCount, int maxTabHeight) {
		...
		return (horizRunCount > 0
				? horizRunCount * (maxTabHeight - tRunOverlay) + tRunOverlay + tAreaInsets.top + tAreaInsets.bottom
				: 0);
	}
	...
	protected int calculateTabAreaWidth(int tabPlacement, int vertRunCount, int maxTabWidth) {
		...
		int result = (vertRunCount > 0
				? vertRunCount * (maxTabWidth - tRunOverlay) + tRunOverlay + tAreaInsets.left + tAreaInsets.right
				: 0);
		return result;
	}
	...
	protected void repaintTabArea() {
		...
		switch (tabPlacement) {
		case LEFT:
			clipRect.setBounds(insets.left, insets.top, calculateTabAreaWidth(tabPlacement, runCount, maxTabWidth) + 6,
					size.height - insets.bottom - insets.top);
			break;
		case BOTTOM:
			int totalTabHeight = calculateTabAreaHeight(tabPlacement, runCount, maxTabHeight);
			clipRect.setBounds(insets.left, size.height - insets.bottom - totalTabHeight - 6,
					size.width - insets.left - insets.right, totalTabHeight + 6);
			break;
		case RIGHT:
			int totalTabWidth = calculateTabAreaWidth(tabPlacement, runCount, maxTabWidth);
			clipRect.setBounds(size.width - insets.right - totalTabWidth - 6, insets.top, totalTabWidth + 6,
					size.height - insets.top - insets.bottom);
			break;
		case TOP:
		default:
			clipRect.setBounds(insets.left, insets.top, size.width - insets.right - insets.left,
	 ...
	 * htmlViews Vector, which is only allocated after the first time we run into an
	 * HTML tab. Note that this vector is kept in step with the number of pages, and
	...
	public void navigateSelectedTab(int direction) {
		...
		/*
		 * int current = QuaquaManager.getBoolean( "TabbedPane.selectionFollowsFocus") ?
		...
		case TOP:
			...
			case NORTH:
				 ...
				 * selectAdjacentRunTab(tabPlacement, current, offset); break;
				 */
			case SOUTH:
				/*
	 ...
	 *
	 * FIXME - Actually, this listener is not needed. The combo box model already
	 * does the job for us.
	 */
	...
	public class QuaquaTabbedPaneLayout // implements LayoutManager {
				...
				if (numChildren > 0) {
					...
					for (int i = 0; i < numChildren; i++) {
						...
						if (child instanceof TabsComboBox) {
							...
							preferredSize.width = Math.min(tw - tabAreaInsets.left - tabAreaInsets.right,
									preferredSize.width);

							Integer propertyValue = (Integer) tabPane
									.getClientProperty("Quaqua.TabbedPane.tabAlignment");
							int tabAlignment = (propertyValue != null
									&& propertyValue.intValue() == SwingConstants.LEADING) ? SwingConstants.LEADING
											: SwingConstants.CENTER;
							if (tabAlignment == SwingConstants.CENTER) {
								child.setBounds(tx + (tw - preferredSize.width) / 2,
										ty + (th - preferredSize.height) / 2, preferredSize.width,
										preferredSize.height);
							} else {
								child.setBounds(tx + tabAreaInsets.left, ty + (th - preferredSize.height) / 2,
		...
		protected void calculateTabRects(int tabPlacement, int tabCount) {
			...
			if (verticalTabRuns || totalTabSize <= availableTabSize || redistributableSize > 0
					&& tabPane.getClientProperty("Quaqua.TabbedPane.shortenTabs") != Boolean.FALSE) {
			...
			int tabAlignment = (propertyValue != null && propertyValue.intValue() == SwingConstants.LEADING)
			...
			if (tabAlignment == SwingConstants.CENTER) {
				...
				case LEFT:
				case RIGHT: {
					int availableTabAreaHeight = size.height - insets.top - insets.bottom - tabAreaInsets.top
				...
				case TOP:
				default: {
					int availableTabAreaWidth = size.width - insets.left - insets.right - tabAreaInsets.left
							- tabAreaInsets.right;
					for (int run = 0; run < runCount; run++) {
						int firstIndex = tabRuns[run];
						int lastIndex = lastTabInRun(tabCount, run);
	...
	private static class TabsComboBoxModel extends AbstractListModel
			implements ComboBoxModel, ContainerListener, ChangeListener {
