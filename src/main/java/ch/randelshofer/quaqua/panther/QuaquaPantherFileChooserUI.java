...
public class QuaquaPantherFileChooserUI extends BasicFileChooserUI implements SubtreeFileChooserUI {
	...
	private AbstractAction keyListenerAction = new AbstractAction() {
		...
		public void actionPerformed(ActionEvent ae) {
			...
			case 'a':
				...
				break;
			/*
			 * case 'u': file = new File( "/Applications/Utilities" ); break;
			 */
			case 'g':
				/* case '`': */
			case '/':
	...
	public void installComponents(JFileChooser fc) {
		 ...
		 * java.awt.GridBagConstraints()); nextButton.setIcon(new
		 ...
		 * tableToggleButton.setIcon(new javax.swing.ImageIcon(getClass().getResource(
		 * "/ch/randelshofer/quaqua/panther/images/FileChooser.tableIcon.png")));
		 * viewGroup.add(tableToggleButton); gridBagConstraints = new
		 * java.awt.GridBagConstraints(); gridBagConstraints.insets = new
		 * java.awt.Insets(0, 5, 0, 0); navigationButtonsPanel.add(tableToggleButton,
		 * gridBagConstraints); / browserToggleButton.setIcon(new
		 * javax.swing.ImageIcon(getClass().getResource(
		 * "/ch/randelshofer/quaqua/panther/images/FileChooser.browserIcon.png")));
		 * browserToggleButton.setSelected(true); viewGroup.add(browserToggleButton);
		 * gridBagConstraints = new java.awt.GridBagConstraints();
		...
		/*
		 * tableScrollPane.setViewportView(table); viewsPanel.add(tableScrollPane,
		 * "table");
		 */
		...
		// Configure JBrowser
		browser.setColumnCellRenderer(new FileRenderer(fc, UIManager.getIcon("Browser.expandingIcon"),
	...
	/**
	 * The array contains the selected file(s) of the JFileChooser. All files have
	 * an absolute path. If no file is selected, the length of the array is 0.
	 * Always returns a non-null value. All array elements are non-null.
	 */
	...
	/**
	 * Updates the selection in the JBrowser, to match the selected file/s of the
	 * JFileChooser.
	 */
	private void updateSelection() {
		...
		if (files.length != 0) {
			...
			for (int i = 0; i < files.length; i++) {
				...
				/*
				 * fullPath = subtreeModel.toPath( file,
			...
			if (list.size() == 0 && files.length > 0) {
				...
				/*
				 * // Set the file name, if it does not exist if (! files[0].exists()) {
...
//->The Update did not occur from the browser        updateFileChooserFromBrowser();
	...
	private static class SidebarRenderer extends DefaultListCellRenderer {
		...
		protected void paintComponent(Graphics gr) {
			...
			case 1:
				...
				g.setPaint(new LinearGradientPaint(0f, 1f, new Color(62, 155, 228), 0f, height - 2,
						new Color(0, 96, 255)));
			...
			case 2:
				...
				g.fillRect(0, 0, width, 1);
				g.setPaint(new LinearGradientPaint(0f, 1f, new Color(151, 151, 151), 0f, height - 2,
	 ...
	 * the tree model: the file system root (aka the comouter).
	 */
