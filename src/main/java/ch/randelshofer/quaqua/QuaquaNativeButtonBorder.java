...
public class QuaquaNativeButtonBorder extends VisualMarginBorder implements Border, PressedCueBorder, BackgroundBorder {
	...
	/**
	 * Each border type needs to be tweaked in a different way. We collect all the
	 * tweaks here.
	 */
	private static class WidgetConfig {
		...
		/**
		 * Border insets. regular,small,mini
		 */
		Insets[] borderInsets = new Insets[3];
		/**
		 * Image insets are used when rendering the native widget onto an image, so that
		 * native widget's visual bounds are rendered with a fixed inset of 3,3,3,3 into
		 * the image. The fixed inset allows for cast shadows in the image.
		 *
		 * only regular,small,mini, first regular,small,mini, middle regular,small,mini,
	...
	static {
		Object[][] a = { //
				...
				{ "square", false, Widget.buttonBevel, //
						...
						new Insets[] { new Insets(3, 3, 3, 3), new Insets(3, 3, 3, 3), new Insets(3, 3, 3, 3), // only
				...
				{ "gradient", "placard", false, Widget.buttonBevelInset, //
						new Insets[] { new Insets(1, 1, 1, 1), new Insets(1, 1, 1, 1), new Insets(1, 1, 1, 1) }, //
						new Insets[] { new Insets(2, 8, 2, 8), new Insets(1, 5, 1, 5), new Insets(1, 4, 1, 4) }, //
						new Insets[] { new Insets(2, 3, 2, 3), new Insets(2, 3, 2, 3), new Insets(2, 3, 2, 3), // only
				...
				{ "segmented", "toggle", "toggleEast", "toggleCenter", "toggleWest", true, Widget.buttonSegmented, //
						...
						new Insets[] { new Insets(1, 4, 1, 4), new Insets(1, 2, 1, 2), new Insets(1, 2, 1, 2) }, //
						new Insets[] { new Insets(0, 1, 0, 1), new Insets(1, 1, 0, 1), new Insets(0, 2, 0, 2), // only
																												// regular,small,mini
				...
				{ "segmentedRoundRect", "segmentedCapsule", "segmentedTextured", true, Widget.buttonSegmentedInset, //
						new Insets[] { new Insets(2, 13, 3, 13), new Insets(2, 8, 2, 8), new Insets(2, 7, 2, 7) }, //
						new Insets[] { new Insets(1, 4, 1, 4), new Insets(1, 2, 1, 2), new Insets(1, 2, 1, 2) }, //
						new Insets[] { new Insets(0, 1, 0, 1), new Insets(0, 1, 0, 1), new Insets(0, 2, 0, 2), // only
																												// regular,small,mini
								new Insets(0, 1, 0, 0), new Insets(1, 1, 0, 0), new Insets(0, 2, 0, 0), // first
								new Insets(0, 0, 0, 0), new Insets(1, 0, 0, 1), new Insets(0, 0, 0, 0), // middle
								new Insets(0, 0, 0, 1), new Insets(1, 1, 0, 1), new Insets(0, 0, 0, 2) } },// last
	...
	/**
	 * The background border. This border delegates the actual painting to different
	 * borders, because {@link OSXAquaPainter} can not render all borders that we
	 * need.
	 */
	private class BGBorder implements Border, PressedCueBorder, VisualMargin {
		...
		private PressedCueBorder getBevelBorder() {
			if (bevelBorder == null) {
				bevelBorder = new FocusedBorder(new CompositeVisualMarginBorder(new ButtonStateBorder(
						Images.createImage(QuaquaButtonBorder.class.getResource("images/RoundedBevel.borders.png")), 10,
						true, new Insets(10, 9, 10, 8), new Insets(0, 0, 0, 0), true), 3, 2, 2, 2));
			}
			return bevelBorder;
		}
		...
		private PressedCueBorder getPlacardBorder() {
			if (placardBorder == null) {
				...
				placardBorder = new FocusedBorder(
						new CompositeVisualMarginBorder(new QuaquaPlacardButtonBorder(), 1, 0, 1, 0));
		...
		private PressedCueBorder getToolBarBorder() {
			if (toolBarBorder == null) {
				...
				toolBarBorder = new VisualMarginBorder(0, 4, 0, 4) {
		...
		private PressedCueBorder getColorWellBorder() {
			if (colorWellBorder == null) {
				colorWellBorder = new FocusedBorder(
						new CompositeVisualMarginBorder(new QuaquaColorWellBorder(), 0, 0, 0, 0));
	...
	private class NativeBGBorder extends CachedPainter implements PressedCueBorder, VisualMargin {
		...
		/**
		 * Returns fake insets since this is just a background border. The real insets
		 * are returned by {@link QuaquaNativeButtonBorder#getBorderInsets}.
		 */
		@Override
		public Insets getBorderInsets(Component c) {
	...
	/*
	 * private Widget getWidget(Component c) { String s = getStyle(c);
	 * WidgetProperty wp = wps.get(s); return (wp == null) ? null : wp.widget; }
	 */
