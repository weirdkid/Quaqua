...
public class QuaquaSplitPaneDivider extends BasicSplitPaneDivider {
	...
	/**
	 * Width or height of the divider based on orientation BasicSplitPaneUI adds two
	 * to this.
	 */
	...
	public void paint(Graphics g) {
		...
		if (style.equals("bar")) {
			...
			if (border != null) {
				border.paintBorder(splitPane, g, insets.left, insets.top, size.width - insets.right - insets.left,
		...
		if (style.equals("thumb")) {
			...
			drawDimple = size.width >= dimple.getIconWidth() && size.height >= dimple.getIconHeight();
		}
	...
	protected class QuaquaDividerLayout implements LayoutManager {
		...
		public void layoutContainer(Container c) {
			if (leftButton != null && rightButton != null && c == QuaquaSplitPaneDivider.this) {
				if (splitPane.isOneTouchExpandable()) {
					...
					if (orientation == JSplitPane.VERTICAL_SPLIT) {
						...
						int extraX = c.getSize().width - insets.right - ONE_TOUCH_OFFSET_ - blockSize * 4;

						int y = (c.getSize().height - blockSize) / 2;

						leftButton.setBounds(extraX, y, blockSize * 2, blockSize);
						rightButton.setBounds(extraX + blockSize * 2, y, blockSize * 2, blockSize);
					} else {
						...
						blockSize = Math.max(blockSize, 0);
						blockSize = Math.min(blockSize, ONE_TOUCH_SIZE_);
						int extraY = c.getSize().height - insets.bottom - ONE_TOUCH_OFFSET_ - blockSize * 4;

						int x = (c.getSize().width - blockSize) / 2;

						leftButton.setBounds(x, extraY, blockSize, blockSize * 2);
						rightButton.setBounds(x, extraY + blockSize * 2, blockSize, blockSize * 2);
					}
				} else {
	...
	protected class QuaquaMouseHandler extends MouseHandler {
		...
		/**
		 * Double click on the split bar moves it to the bottom or to the left. If it is
		 * already at the bottom most or leftmost position, it is moved to its last
		 * location.
		 */
		@Override
		public void mouseClicked(MouseEvent evt) {
			if (evt.getClickCount() == 2 && splitPane.isOneTouchExpandable()) {
				...
				if (isHorizontal) {
					if (leftC.isVisible()) {
						minLoc = leftC.getMinimumSize().width + insets.left;
					} else {
						minLoc = insets.left;
					}
					if (rightC.isVisible()) {
						maxLoc = splitPane.getWidth() - rightC.getMinimumSize().width - insets.right - getSize().width;
					} else {
						maxLoc = splitPane.getWidth() - insets.right;
					}
				} else {
					if (leftC.isVisible()) {
						minLoc = leftC.getMinimumSize().height + insets.top;
					} else {
						minLoc = insets.top;
					}
					if (rightC.isVisible()) {
						maxLoc = splitPane.getHeight() - rightC.getMinimumSize().height - insets.bottom
								- getSize().height;
					} else {
