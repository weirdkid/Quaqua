...
public class QuaquaTextFieldUI extends BasicTextFieldUI implements VisuallyLayoutable {
	...
	/**
	 * Preferred spacing between text fields and other components. / private final
	 * static Insets regularSpacing = new Insets(10,10,10,10); private final static
	 * Insets smallSpacing = new Insets(8,8,8,8); private final static Insets
	...
	/**
	 * Paints a background for the view. This method is empty, because we already
	 * paint the background in method paintSafely.
	 *
	...
	public Dimension getPreferredSize(JComponent c) {
		...
		try {
			...
			d.width = (int) Math.min((long) rootView.getPreferredSpan(View.X_AXIS) + i.left + i.right,
					Integer.MAX_VALUE);
			d.height = (int) Math.min((long) rootView.getPreferredSpan(View.Y_AXIS) + i.top + i.bottom,
					Integer.MAX_VALUE);
		} finally {
			if (doc instanceof AbstractDocument) {
