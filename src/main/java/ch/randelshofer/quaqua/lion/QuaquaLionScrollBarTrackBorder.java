...
public class QuaquaLionScrollBarTrackBorder implements Border, UIResource {
	...
	private final static Color[] rrectGradientColors = { new Color(0xdbe3e3e3, true), new Color(0xdbe7e7e7, true),
			new Color(0xdbeaeaea, true), };
	private final static float[] rrectGradientFractions = { 0f, 0.5f, 1f, };
	private final static Color topColor = new Color(0xe4e4e4);
	private final static Color bottomColor = new Color(0xefefef);
	private final static Color[] gradientColors = { new Color(0xf2f2f2), new Color(0xfcfcfc), new Color(0xf8f8f8), };
	private final static float[] gradientFractions = { 0f, 0.5f, 1f, };
		...
		if (false && sp != null) {
			if (sb.getValueIsAdjusting()) {
				if (sb.getOrientation() == SwingConstants.HORIZONTAL) {
					...
					int tw = width - 4;
					int th = height - 4;
					g.setPaint(new LinearGradientPaint(tx, ty, tx, ty + th - 1, rrectGradientFractions,
							rrectGradientColors));
					...
					int tw = width - 4;
					int th = height - 4;
					g.setPaint(new LinearGradientPaint(tx, ty, tx + width - 1, ty, rrectGradientFractions,
							rrectGradientColors));
