...
public class QuaquaLionSideBarSelectionBorder implements Border, UIResource {
	...
	public void paintBorder(Component c, Graphics gr, int x, int y, int width, int height) {
		...
		if (QuaquaUtilities.isFocused(c)) {
			...
			// gradient
			g.setPaint(new LinearGradientPaint(x, y + 2, new Color(0x6db1e3), x, y + height - 3, new Color(0x3b89d0)));
			...
			if (QuaquaUtilities.isOnActiveWindow(c, true)) {
				...
				// gradient
				g.setPaint(
						new LinearGradientPaint(x, y + 2, new Color(0xbdc7dc), x, y + height - 3, new Color(0x9dabc4)));
				g.fillRect(x, y + 2, width, height - 3);
