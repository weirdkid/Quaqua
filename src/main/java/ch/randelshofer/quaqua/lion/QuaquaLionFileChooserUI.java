...
import ch.randelshofer.quaqua.QuaquaNativeTextFieldBorder;
...
/**
 * A replacement for the AquaFileChooserUI. Provides a list view and a column
 * view similar to the one provided with the native Aqua user interface on Mac
 * OS X 10.7 (Lion).
 *
...
public class QuaquaLionFileChooserUI extends BasicFileChooserUI {
	...
	/*
	 * In general, this class makes changes to the user interface directly, rather
	 * than making changes to the file chooser and responding to events from the
	 * file chooser. This design is needed to support smart folders (saved
	 * searches). With saved searches, a file can appear in multiple models using
	 * different paths. Using this design, tree paths are generally used instead of
	 * files.
	 * 
	 * In general, events are used only to receive notification of user actions and
	 * changes made by the application to the file chooser (e.g. from an accessory).
	 * Events are not used internally to achieve consistency. The isAdjusting
	 * variable is used to prevent updates made by this class from being processed
	 * by event listeners in this class.
	 * 
	 * There are a few minor exceptions where this class changes an attribute of the
	 * file chooser with the intent that an event will be received and processed.
	 * These exceptions are cases where a user action is translated directly into a
	 * change on the file chooser, which is processed exactly the same as if the
	 * application had made the change.
	 */
	...
	/**
	 * Values greater than zero indicate that a change is being made by this class
	 * to state that is monitored using events and that processing of any events by
	 * this class should be inhibited. (This is the normal mode of operation, see
	 * comment above.) Examples of state monitored using events: file chooser
	 * attributes, file name text field, view root, view selection, sidebar
	 * selection.
	 */
	...
	private static Border textFieldBorder = new QuaquaNativeTextFieldBorder(new Insets(0, 0, 0, 0),
			new Insets(6, 8, 6, 8), true);
	...
	protected CellRenderer createFileRenderer(JFileChooser fc) {
		return new LeopardFileRenderer(fc, UIManager.getIcon("Browser.expandingIcon"),
				UIManager.getIcon("Browser.expandedIcon"), UIManager.getIcon("Browser.selectedExpandingIcon"),
	...
	private class KeyListenerAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent ae) {
			...
			if (file != null) {
				...
				// FIXME - Implement me
				/*
				 * for (int i=0, n = sidebarTreeModel.getSize(); i < n; i++) { FileInfo
				 * sidebarFile = (FileInfo) sidebarTreeModel.getElementAt(i); if (sidebarFile !=
				 * null && sidebarFile.getFile().equals(dir)) { sidebarTree.setSelectedIndex(i);
				 * return; } }
				 */
	 ...
	 * By handling the key typed event, we (hopefully) avoid a race condition
	 * observed in Java 1.6 when the shortcut was implemented using an input map.
	 * The input map processes key pressed events. On occasion, the subsequent key
	 * typed event would be processed by the sheet, resulting in a doubling of the
	 * shortcut character in the text field.
	 */
	...
	protected void requestFileSelectionPath(String initialText) {
		...
		final RequestFileSelectionPathPane pane = new RequestFileSelectionPathPane(sheet, initialText, prompt,
				cancelLabel, acceptLabel, errorText) {
			private static final long serialVersionUID = 1L;
	...
	/*
	 * Not using a JOptionPane because we do not want to display an icon, we want to
	 * disable the accept button when text field is empty, we want to display an
	 * error message.
	 */
	...
	protected class RequestFileSelectionPathPane extends JPanel implements DocumentListener {
		...
		public RequestFileSelectionPathPane(final JDialog parent, String initialText, String message,
				String cancelLabel, String acceptLabel, final String errorText) {
	...
	public void installComponents(JFileChooser fc) {
		...
		/*
		 * The native side bar default width is 134, but that assumes a floating scroll
		 * bar. Until Quaqua implements a floating scroll bar, we need to allocate extra
		 * space.
		 */
		...
		// The following does not work, QuaquaPanelUI has limited special cases for
		// background borders
		// accessoryViewPanel.setBorder(new
		// CompoundBorder(javax.swing.BorderFactory.createEmptyBorder(15, 13, 0 13), new
		// TitledBorder("")));
		{
		...
		Component[] dropComponents = { fc, accessoryPanel, approveButton, columnView, buttonsPanel, cancelButton,
				controlsPanel, directoryComboBox, fileNameLabel, fileNamePanel, fileNameSpringPanel, fileNameTextField,
				filesOfTypeLabel, filterComboBox, formatPanel, formatSpringPanel, listView, mainPanel, navigationPanel,
				newFolderButton,
				// nextButton,
				// previousButton,
				separator, splitPane, viewModeControl, viewsPanel, sidebarTree, sidebarScrollPane };
		...
		// do not register handlers for ordinary characters, use key typed event
		// handlers instead

		ActionMap am = mainPanel.getActionMap();
		InputMap globalInputMap = mainPanel.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW);
		for (KeyStroke ks : KEYSTROKES) {
			if ((ks.getModifiers() & InputEvent.META_MASK) != 0) {
	...
	/**
	 * Create a Quaqua text field. This method configures the text field without
	 * contaminating ordinary text fields with UIDefaults when the file chooser only
	 * JAR is used.
	 */
	...
	/**
	 * Create a Quaqua button. This method configures the button without
	 * contaminating ordinary buttons with UIDefaults when the file chooser only JAR
	 * is used.
	 */
	...
	/**
	 * Installs listeners. We install the same listeners as BasicFileChooserUI plus
	 * an AncestorListener and a property change listener.
	 */
	@Override
	...
	protected void installStrings(JFileChooser fc) {
		...
		// FIXME - There is no "FileChooser.newFolderButtonText", so we use the
		// newFolderTitleText.
		...
		/// newFolderAccessibleName = getString("FileChooser.newFolderAccessibleName",
		/// l, newFolderTitleText);
	...
	/**
	 * Creates an AncestorListener. The AncestorListener is used to take an action
	 * when the JFileChooser becomes showing on screen.
	 */
	...
	/**
	 * Update the selection in the active view to match the selected files of the
	 * JFileChooser.
	 *
	 * This method maps the (application provided) file chooser selection to a
	 * (possibly new) view selection and a (possibly new) view root.
	 *
	 * In a column view, all elements of a multiple selection must reside in the
	 * same parent directory. Although not mandatory for a list view, we enforce the
	 * common parent rule in all cases to ensure that the file selection is
	 * viewable. The file chooser selection is updated as needed to be consistent
	 * with what we display in the view.
	 *
	 * If the view is displaying a smart folder (saved search), we display the file
	 * selection in the the smart folder if possible. Otherwise, we switch to the
	 * file system model.
	 */
	private void updateSelection(final int source) {
		...
		/*
		 * Special case for save dialogs: Ordinary files are not normally acceptable in
		 * a save dialog view: the user may click on a file but not actually select it.
		 * However, an application may set the selected file of the file chooser as a
		 * way of selecting a directory and a default file name.
		 */
		...
		if (directoryPath != null) {
			...
			/*
			 * Because this method may be called from a property change listener, it is
			 * better to defer any change to the file chooser state, otherwise other
			 * property change listeners may be invoked in the wrong order.
			 */
			...
			// Update the view selection based on the current directory. We can not assume
			// that a change event will
	...
	private void finishUpdateSelection(List<File> originalFiles, List<TreePath> newSelection) {
		if (originalFiles.size() != newSelection.size()) {
			// We filtered out some selected files because they did not share a common
			// parent
	...
	private boolean computeApproveButtonEnabled() {
		...
		if (isFileNameFieldVisible() && isFileNameFieldValid()
	...
	private boolean isAcceptableSaveDialogState() {
		...
		if (dir == null || !dir.isDirectory() || !isFileNameFieldValid()) {
			return false;
		}
	...
	/**
	 * Update the file chooser current directory, selected files, and the approve
	 * button enabled state based on the selection in the active view. The active
	 * view selection may be updated if it is not consistent with the file chooser
	 * configuration. If the file chooser current directory is changed, update the
	 * directory combo box to be consistent.
	 */
	...
	private void respondToUISelectionChange() {
		...
		/*
		 * Update the file chooser current directory based on the normalized view
		 * selection (column view only).
		 */
		...
		/*
		 * Update the file chooser selected files based on the normalized view
		 * selection. The current directory is preserved.
		 */
	...
	/**
	 * Determine the (possibly new) current directory based on the selected files in
	 * the column view.
	 */
	...
	/**
	 * Determine the (possibly new) selected files based on the selected files in
	 * the active view. The special case is where a single unacceptable, traversable
	...
	/**
	 * Set the selected files in the file chooser. The file chooser may change its
	 * current directory.
	 */
	...
	/**
	 * Set the selected files in the file chooser. The file chooser current
	 * directory is restored if needed.
	 */
	private void installSelectedFiles(List<File> fs) {
		if (isAdjusting == 0) {
			++isAdjusting;
			// Setting the selected file(s) will probably reset the current directory. We do
			// not want that.
			File currentDirectory = fc.getCurrentDirectory();
	...
	/**
	 * Respond to double click on a item in the view that is either traversable or
	 * acceptable (or both) or to a single click on a file in a Save dialog.
	 *
	 * A double click on a traversable item installs the item as the current
	 * directory (list view only), which may update the display subtree root and the
	 * sidebar selection. (Nothing special happens in a column view.)
	 *
	 * A double click on an acceptable (but not traversable) item selects that item
	 * as the selected file (possibly adding it to the existing selection) and
	 * dismisses the dialog.
	 *
	 * A single click on a file in a Save dialog installs the file name in the file
	 * name text field.
	 */
	protected class ViewSelectListener implements FileChooserView.SelectListener {
		...
		public void select(TreePath path) {
			...
			if (info.isTraversable()) {
				...
				/*
				 * If the item is a member of the selection, the selection is not changed.
				 * Otherwise, the item is selected.
				 */
				if (!isSelected(f)) {
	...
	private void doDirectoryChanged(PropertyChangeEvent e) {
		...
		/*
		 * The application has changed the file chooser current directory. We map that
		 * to a change in our selected directory. Programmatic selection of a directory
		 * in NSOpenPanel always chooses the best sidebar root.
		 */
	...
	/**
	 * Update a model after a change in the file chooser configuration that might
	 * impact the model.
	 */
	...
	private void updateModel() {
		...
		if (fc.isShowing()) {
			...
			/*
			 * TBD: would like to make sure that the current selection is still visible
			 * after the model change. The issue is that validation is asynchronous, so we
			 * do not know when to update the scroll pane.
			 */
		}
	}
	...
	/*
	 * Listen for filechooser property changes, such as the selected dir changing,
		...
		return new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent e) {
				...
				if (s.equals(JFileChooser.SELECTED_FILE_CHANGED_PROPERTY)) {
					...
					/*
					 * FIXME - This needs JDK 1.4 to work. ComponentOrientation o =
					 * (ComponentOrientation)e.getNewValue(); JFileChooser fc =
					 * (JFileChooser)e.getSource(); if (o != (ComponentOrientation)e.getOldValue())
...
//                    }
	...
	/*
	 * Sources for selecting a directory by means other than the user changing the
	...
	/**
	 * Update the current view to display the specified directory. A new view root
	 * may be chosen and the sidebar selection may be changed. The file chooser
	 * current directory will be changed if needed and the combo box model updated.
	 * The view selection will be updated as needed. The approve button enabled
	 * state is updated.
	 *
	 * This method may perform some updates asynchronously.
	 *
	 * @param f      The traversable file to become the current directory. If
	 *               {@code f} is not a traversable file, the nearest traversable
	 *               ancestor is used (should be the parent).
	...
	protected void selectDirectory(File f, final int source, final Runnable r) {
		...
		/*
		 * Smart folders are displayed when they are selected using the sidebar. If a
		 * smart folder is already displayed in a column view and a directory in the
		 * smart folder tree is selected by means other than the sidebar, then the
		 * directory is displayed as part of the smart folder. In all other cases, the
		 * selected directory is displayed in a file system tree.
		 */

		if ((source == SELECT_DIRECTORY_FROM_SIDEBAR || source == SELECT_DIRECTORY_TO_INITIALIZE
				|| source == SELECT_DIRECTORY_NEW_VIEW) && OSXFile.isSavedSearch(f)) {
			installModel(getSavedSearchTreeModel(f));
		} else {
			f = toTraversableFile(f);
			if (viewMode != ViewModeControl.COLUMN_VIEW || source == SELECT_DIRECTORY_FROM_SIDEBAR
					|| model.toPath(f, null) == null) {
				ensureFileSystemModel();
			}
		}

		/*
		 * If the selection was made using the sidebar, the sidebar item becomes the
		 * view root.
		 * 
		 * Otherwise, if not displaying an element of a smart folder:
		 * 
		 * If displaying a list view, the specified directory becomes the view root and
		 * directory is selected in the sidebar if it is present (otherwise the sidebar
		 * selection is cleared).
		 * 
		 * If displaying a column view, a new view root is determined by searching for
		 * the best matching sidebar item and that sidebar item (if found) is selected.
		 */
		...
		if (source == SELECT_DIRECTORY_FROM_SIDEBAR) {
			...
			sidebarTreeModel.invokeWhenValid(new Runnable() {
				...
				public void run() {
					...
					finishSelectDirectory(source, fullPath, r);
					/*
					 * Seems to be a race condition causing QuaquaTreeUI to get confused about which
					 * row is selected when new items have been added to the sidebar tree model but
					 * not yet displayed.
					 */
					SwingUtilities.invokeLater(new Runnable() {
						@Override
	...
	/**
	 * Complete the selection of a specified directory. This method is called after
	 * any update to the view root.
	 */
	private void finishSelectDirectory(int source, TreePath fullPath, Runnable r) {
		...
		if (source != SELECT_DIRECTORY_FROM_API && source != SELECT_DIRECTORY_TO_INITIALIZE
				&& source != SELECT_DIRECTORY_NEW_VIEW) {
	...
	/**
	 * Select a subtree of the model to display that contains the specified file and
	 * identify the matching sidebar item, if any.
	 *
	 * @param f                     The (resolved) file.
	 * @param isRestrictedToSidebar If true, new subtree root will be the best
	 *                              matching sidebar item, where a sidebar item
	 *                              matches if the file is contained in the subtree
	 *                              defined by that sidebar item. (If there is no
	 *                              matching sidebar item, the entire file system
	 *                              tree model is displayed.) If false, the
	 *                              specified file becomes the new subtree root, and
	 *                              the sidebar item for that (exact) file is
	 *                              returned (if any).
	 *
	...
	protected void updateComboBoxModel(TreePath subtreePath) {
		...
		/*
		 * The special case here is selecting a directory via a displayed smart folder
		 * (saved search). The items in the combo box should spell out the full file
		 * system path of the directory.
		 */
	...
	private void ensurePathIsVisible(final TreePath path) {
		...
		/*
		 * Make sure the nodes are valid otherwise the scroll positions may be
		 * incorrect.
		 */
	...
	static class DirectoryComboBoxRenderer extends DefaultListCellRenderer {
		...
		public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected,
			...
			if (node == null) {
				return separator;
				/*
				 * File root = new File("/"); setText(fc.getName(root)); ii.icon =
				 * fc.getIcon(root);
				 */
			} else {
	...
	/**
	 * Selection model for the sidebar. Prevents category headings from being
	 * selected.
	 */
	...
	//
	// CHANGE This class is rewritten after adding support for the Quaqua.Tree.style
	// property
	...
	private static class SidebarRenderer extends DefaultTreeCellRenderer {
		...
		public Component getTreeCellRendererComponent(JTree tree, Object value, boolean isSelected, boolean isExpanded,
				boolean isLeaf, int row, boolean cellHasFocus) {
		...
		/**
		 * Gets whether the the {@link FileInfo} represents a "special" folder - a
		 * folder which is visually different in the side bar than in the browser view
		 * of a file chooser.
		 *
		 * @param info The {@link FileInfo}.
		...
		private boolean isSpecialFolder(SidebarTreeFileNode info) {
			return true;
			/*
			 * // Only allow this for Mac OS X as directory structures are different on
			 * other OSs. if (!QuaquaManager.isOSX()) { return false; }
			 * 
			 * File file = info.getFile(); // Only directories can have special icons. if
			 * (file == null || file.isFile()) { return false; }
			 * 
			 * if (file.getParentFile() != null) { String parentFile =
			 * file.getParentFile().getAbsolutePath(); if
			 * (parentFile.equals(System.getProperty("user.home"))) { // Look for user's
			 * home special folders String name = file.getName(); return
			 * name.equals("Applications") || name.equals("Desktop") // ||
			 * name.equals("Documents") || name.equals("Downloads")// ||
			 * name.equals("Library") || name.equals("Movies") // || name.equals("Music") ||
			 * name.equals("Pictures") // || name.equals("Public") || name.equals("Sites");
			 * } else if (parentFile.equals(computer.getAbsolutePath())) { // Look for
			 * computer's special folders String name = file.getName(); return
			 * name.equals("Applications") || name.equals("Library"); } else if
			 * (parentFile.equals(new File(computer, "Applications").getAbsolutePath())) {
			 * // Look for Utility folder in the /Applications folder return
			 * file.getName().equals("Utilities"); } } // Nothing found - return null return
			 * false;
			 */
		}
	}
	...
	/**
	 * Data model for a directory selection combo-box. There is always one node in
	 * the tree model: the dir system root (aka the computer).
	 */
	protected class DirectoryComboBoxModel extends AbstractListModel implements ComboBoxModel {
		...
		/**
		 * Sets the path of the directory combo box. TreePath<FileSystemTreeModel.Node>
		 */
	...
	protected class FilterComboBoxModel extends AbstractListModel implements ComboBoxModel, PropertyChangeListener {
		...
		public Object getSelectedItem() {
			...
			/*
			 * The above statement is widely copied but it is incorrect. There is no code in
			 * JFileChooser that adds a filter to the list of choosable filters when it is
			 * set. Perhaps there was at one time? The effect of this code is to make the
			 * notion of a non-choosable filter difficult to realize. The problem being
			 * solved is a real one: if the file chooser is displaying a set of choosable
			 * filters and the application (unwisely) sets a filter that is not on the list,
			 * the combo box should display something other than one of the choosable
			 * filters. A better solution would add an item to the combo box model rather
			 * than to the file chooser's list of choosable filters. At this point, however,
			 * the broken behavior is probably depended upon by lots of code, so I guess it
			 * is too late to change except as a configurable option.
			 */
	...
	protected class DirectoryComboBoxAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			/*
			 * The reason for using invokeLater: we want to transfer focus to the proper
			 * column list. However, when QuaquaComboPopup is dismissed (which has not
			 * happened yet), it will transfer the focus to the previous focus owner. Our
			 * request must come after that.
			 */
	...
	/**
	 * This method is called when the user double clicks an item in the view or
	 * clicks at the approve button.
	 */
	private void maybeApproveSelection() {
		...
		/*
		 * In general, the file chooser selection should already match the view
		 * selection and should be legitimate. The primary exception is where the file
		 * chooser allows the user to provide a file name (e.g. a Save dialog), in which
		 * case we need to update the file chooser selection prior to approving the
		 * selection. There are also obscure exceptional cases only some of which are
		 * checked for here.
		 */
		...
		if (filename != null && selectedFiles.size() == 1) {
			...
			/*
			 * In a list view, it is possible to select a directory in the list without
			 * making it the current directory (the one whose name is in the combo box). It
			 * might be confusing to the user to enable the Save button in that situation.
			 * NSSavePanel enables the Save button, but the action switches to that
			 * directory rather than accepting the selection.
			 */
	...
	/**
	 * Is a directory selected as an item in the list view? This method is called
	 * only for a Save or Custom dialog.
	 * 
	...
	/**
	 * Return the selection from the active view normalized to be consistent with
	 * the file chooser configuration. If the actual view selection is not
	 * consistent with the file chooser configuration, the view selection will be
	 * updated to make it consistent. The returned paths are guaranteed to have an
	 * associated file.
	 */
	protected List<TreePath> getNormalizedUISelection() {

		/*
		 * Multiple paths are valid only if the chooser is configured for multiple
		 * selection. An unacceptable traversable file can be selected, but it must be
	 ...
	 * 
	 * @param useDefault If true and no files are selected in the view, return the
	 *                   display subtree root.
	...
	/**
	 * Return the file selection from the file chooser normalized to be consistent
	 * with the file chooser configuration. If the actual selection is not
	 * consistent with the file chooser configuration, the selection will be updated
	 * to make it consistent. The returned files are guaranteed to be absolute.
	 */
	protected List<File> getNormalizedChooserSelection() {

		/*
		 * Multiple files are valid only if the chooser is configured for multiple
		 * selection. An unacceptable traversable file can be selected, but it must be
		 * the unique selection.
		 */
	...
	/**
	 * Return the file selection from the file chooser. The returned files are
	...
	protected class NewFolderAction extends AbstractAction {
		...
		private String showNewFolderDialog() {
			JOptionPane optionPane = new JOptionPane(newFolderDialogPrompt, JOptionPane.PLAIN_MESSAGE,
			...
			// Setup Options
			optionPane.setOptions(new Object[] { UIManager.getString("FileChooser.createFolderButtonText"),
		...
		public void actionPerformed(ActionEvent actionevent) {
			...
			if (newFolderName != null) {
				...
				if (newFolder.exists()) {
					JOptionPane.showMessageDialog(fc, newFolderExistsErrorText, newFolderTitleText,
				...
				try {
					...
					JOptionPane.showMessageDialog(fc, newFolderErrorText, newFolderTitleText,
							JOptionPane.ERROR_MESSAGE);
				}
			}
		}
				...
				public void run() {
					...
					if (fileNameTextField.getText().length() != 0) {
						if (!node.isLeaf()) {
							...
							file = new File(fc.getFileSystemView().getParentDirectory(file),
					 ...
					 * because a save dialog does not allow selecting files and an open dialog does
					 * not display the file name text field.
					 */
	 ...
	 * JFileChooser. This is used to do validations (refreshes) of the tree model
	 * only, when the JFileChooser is showing.
	 */
	...
	public void ensureFileIsVisible(JFileChooser fc, final File f) {
		...
		/*
		 * This method is called by the file chooser when the selected file is set. It
		 * may also be called by the application in which case the file may not be
		 * selected. My reading is that the purpose of this method is to adjust scroll
		 * positions to reveal the file, not to change the file chooser selection or
		 * current directory.
		 */
