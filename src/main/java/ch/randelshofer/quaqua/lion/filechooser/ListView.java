...
public class ListView extends ch.randelshofer.quaqua.filechooser.ListView {
	...
	public ListView(JFileChooser fc) {
		...
		/*
		 * A fake tree model is needed to work around a problem. JTree maintains a cache
		 * of tree model nodes for layout purposes. The cache is not flushed when a new
		 * tree model is supplied. The result is that the model adapter in TreeTable
		 * tries to ask the new model whether a cached node is a leaf. This produces a
		 * ClassCastException in the new model.
		 */
		...
		/*
		 * Issue: I would like to get the Aqua table header background, but I would also
		 * like to control the font, indentation, and alignment of the column titles. Is
		 * this possible? Defining my own table header renderer inhibits the Aqua table
		 * header background.
		 */
	...
	public void setMultipleSelection(boolean b) {
		tree.getSelectionModel().setSelectionMode(
				b ? TreeSelectionModel.DISCONTIGUOUS_TREE_SELECTION : TreeSelectionModel.SINGLE_TREE_SELECTION);
	}
	...
	public List<TreePath> getSelection() {
		...
		return ps != null ? new ArrayList<>(Arrays.asList(ps)) : new ArrayList<>();
	}
	...
	protected class MyTableColumn extends TableColumn {
		...
		/*
		 * Seems like a bug in JTable that the column margin is not included in the
	...
	private class MyNameCellRenderer extends MyCellRenderer {
		...
		public Component getTreeTableCellRendererComponent(TreeTable treeTable, Object value, boolean selected,
			...
			if (c == null) {
				c = super.getTreeTableCellRendererComponent(treeTable, value, selected, hasFocus, row, column, expanded,
						leaf);
			}
