...
public class TIFFDirectory implements Serializable {
	 ...
	 * specifies which directory to read from the linked list present in the stream;
	 * directory 0 is normally read but it is possible to store multiple images in a
	 * single TIFF file by maintaing multiple directories.
	 *
	 ...
	 *                   current stream offset; zero indicates the IFD at the
	 *                   current offset.
	 */
	...
	/**
	 * Returns the value of a given tag as a TIFFField, or null if the tag is not
	 * present.
	 */
	 ...
	 * <p>
	 * Note: For efficiency reasons this method returns an internally used array. Do
	...
	/**
	 * Returns the value of a particular index of a given tag as a byte. The caller
	 * is responsible for ensuring that the tag is present and has type
	...
	/**
	 * Returns the value of a particular index of a given tag as a long. The caller
	 * is responsible for ensuring that the tag is present and has type TIFF_BYTE,
	 * TIFF_SBYTE, TIFF_UNDEFINED, TIFF_SHORT, TIFF_SSHORT, TIFF_SLONG or TIFF_LONG.
	 */
	...
	/**
	 * Returns the value of a particular index of a given tag as a float. The caller
	 * is responsible for ensuring that the tag is present and has numeric type (all
	...
	/**
	 * Returns a boolean indicating whether the byte order used in the the TIFF file
	 * is big-endian. That is, whether the byte order is from the most significant
