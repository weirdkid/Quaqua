...
public abstract class AbstractRed implements CachableRed {
	 ...
	 * @param props        this initializes the props Map.
	 */
	protected AbstractRed(List srcs, Rectangle bounds, ColorModel cm, SampleModel sm, int tileGridXOff,
			int tileGridYOff, Map props) {
	 ...
	 * @param props        Any properties you want to associate with the image.
	 */
	protected void init(List srcs, Rectangle bounds, ColorModel cm, SampleModel sm, int tileGridXOff, int tileGridYOff,
			Map props) {
...
//        }
	...
	public void copyToRaster(WritableRaster wr) {
		...
		if (!is_INT_PACK && (getSampleModel() instanceof PixelInterleavedSampleModel)) {
			...
			is_BYTE_RGBA_INTERLEAVED = pixi.getNumBands() == 4 && pixi.getDataType() == DataBuffer.TYPE_BYTE;
		} else {
