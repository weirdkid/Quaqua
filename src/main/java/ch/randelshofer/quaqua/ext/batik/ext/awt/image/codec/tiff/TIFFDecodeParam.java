...
/**
 * An instance of {@code ImageDecodeParam} for decoding images in the TIFF
 * format.
 *
 * <p>
 * To determine the number of images present in a TIFF file, use the
 * {@code getNumPages()} method on the {@code ImageDecoder} object that will be
 * used to perform the decoding. The desired page number may be passed as an
 * argument to the {@code ImageDecoder.decodeAsRaster)()} or
 ...
 * the {@code decode16BitsTo8Bits} method for each short value that needs to be
 * dithered. The method has the following implementation:
 * 
 * <pre>
 * 
 * byte b;
 ...
 * </pre>
 * 
 * If a different algorithm is to be used for the dithering, this class should
 * be subclassed and an appropriate implementation should be provided for the
 * {@code decode16BitsTo8Bits} method in the subclass.
 *
 * <p>
 ...
 * {@code decodeSigned16BitsTo8Bits} instead. The method has the following
 * implementation:
 * 
 * <pre>
 * 
 ...
 * </pre>
 * 
 * In order to use a different algorithm for the dithering, this class should be
 * subclassed and the method overridden.
 *
 * <p>
...
public class TIFFDecodeParam implements ImageDecodeParam {
	...
	/**
	 * If set, the entries in the palette will be decoded as shorts and no short to
	 * byte lookup will be applied to them.
	 */
	...
	/**
	 * Returns {@code true} if palette entries will be decoded as shorts, resulting
	 * in an output image with short datatype.
	 */
	 ...
	 * while Java's {@code short} datatype is a signed value. Therefore the Java
	 * {@code short} datatype cannot be used to store the TIFF specified short
	 * value. A Java {@code int} is used as input instead to this method. The method
	...
	/**
	 * Sets the offset in the stream from which to read the image. There must be an
	 * Image File Directory (IFD) at that position or an error will occur. If
	 * {@code setIFDOffset()} is never invoked then the decoder will assume that the
	 * TIFF stream is at the beginning of the 8-byte image header. If the directory
	 * offset is set and a page number is supplied to the TIFF {@code ImageDecoder}
	 * then the page will be the zero-relative index of the IFD in linked list of
	 * IFDs beginning at the specified offset with a page of zero indicating the
	...
	/**
	 * Returns the value set by {@code setIFDOffset()} or {@code null} if no value
	 * has been set.
	 */
	 ...
	 * The default value is {@code true}. This flag is ignored if the image data are
	 * not JPEG-compressed.
	 */
