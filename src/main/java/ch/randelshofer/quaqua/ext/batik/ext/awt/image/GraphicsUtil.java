...
public class GraphicsUtil {
	...
	public static BufferedImage getDestination(Graphics2D g2d) {
		...
		if (WARN_DESTINATION && (gd.getType() == GraphicsDevice.TYPE_IMAGE_BUFFER) && (g2d.getRenderingHint(
				RenderingHintsKeyExt.KEY_TRANSCODING) != RenderingHintsKeyExt.VALUE_TRANSCODING_PRINTING)) // throw new
...
// BEGIN PATCH W. Randelshofer Performance
	...
	public static void copyData_BYTE_RGBA_INTERLEAVED(Raster src, WritableRaster dst) {
		...
		/*
		 * int y1 = dst.getMinY() + dst.getHeight() - 1; if (y1 > src.getMinY() +
		 * src.getHeight() - 1) { y1 = src.getMinY() + src.getHeight() - 1; }
		 */
		int width = x1 - x0 + 1;
...
// END PATCH W. Randelshofer Performance
	...
	public static ColorModel coerceData(WritableRaster wr, ColorModel cm, boolean newAlphaPreMult) {
		...
		// " Out: " + newAlphaPreMult);
		if (!cm.hasAlpha() || (cm.isAlphaPremultiplied() == newAlphaPreMult)) // nothing to do alpha state matches...
		{
			return cm;
		}
	...
	public static boolean is_INT_PACK_Data(SampleModel sm, boolean requireAlpha) {
		...
		// Check transfer type
		if (!(sm instanceof SinglePixelPackedSampleModel) || (sm.getDataType() != DataBuffer.TYPE_INT)) {
			return false;
		}
	...
	public static boolean is_BYTE_COMP_Data(SampleModel sm) {
		// Check ColorModel is of type DirectColorModel
		// Check transfer type
		if (!(sm instanceof ComponentSampleModel) || (sm.getDataType() != DataBuffer.TYPE_BYTE)) {
			return false;
		}
