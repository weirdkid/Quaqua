...
public class TIFFImage extends AbstractRed {
	 ...
	 * @param decodeParam  decoding parameters; may be null unless the data buffer
	 *                     contains an abbreviated datastream in which case it may
	 *                     not be null or an error will occur.
	 * @param colorConvert whether to perform color conversion; in this case that
	 *                     would be limited to YCbCr-to-RGB.
	 * @param minX         the X position of the returned Raster.
	 * @param minY         the Y position of the returned Raster. / private static
	 *                     final Raster decodeJPEG(byte[] data, JPEGDecodeParam
	 *                     decodeParam, boolean colorConvert, int minX, int minY) {
	 *                     // Create an InputStream from the compressed data array.
	 *                     ByteArrayInputStream jpegStream = new
	 *                     ByteArrayInputStream(data);
	 * 
	 *                     // Create a decoder. JPEGImageDecoder decoder =
	 *                     decodeParam == null ?
	 *                     JPEGCodec.createJPEGDecoder(jpegStream) :
	 *                     JPEGCodec.createJPEGDecoder(jpegStream, decodeParam);
	 * 
	 *                     // Decode the compressed data into a Raster. Raster
	 *                     jpegRaster; try { jpegRaster = colorConvert ?
	 *                     decoder.decodeAsBufferedImage().getWritableTile(0, 0) :
	 *                     decoder.decodeAsRaster(); } catch (IOException ioe) {
	 *                     throw new RuntimeException("TIFFImage13"); }
	 * 
	 *                     // Translate the decoded Raster to the specified location
	 *                     and return. return jpegRaster.createTranslatedChild(minX,
	 *                     minY); }
	 */
	/**
	...
	private static SampleModel createPixelInterleavedSampleModel(int dataType, int tileWidth, int tileHeight,
			int bands) {
	...
	/**
	 * Constructs a TIFFImage that acquires its data from a given SeekableStream and
	 * reads from a particular IFD of the stream. The index of the first IFD is 0.
	 *
	...
	public TIFFImage(SeekableStream stream, TIFFDecodeParam param, int directory) throws IOException {
		...
		char[] planarConfiguration = planarConfigurationField == null ? new char[] { 1 }
		...
		Rectangle bounds = new Rectangle(0, 0, (int) dir.getFieldAsLong(TIFFImageDecoder.TIFF_IMAGE_WIDTH),
		...
		if (dir.getField(TIFFImageDecoder.TIFF_TILE_OFFSETS) != null) {
			...
			tileWidth = dir.getField(TIFFImageDecoder.TIFF_TILE_WIDTH) != null
			...
			if (field == null) {
				...
				tileHeight = dir.getField(TIFFImageDecoder.TIFF_TILE_LENGTH) != null
		...
		case COMP_JPEG_TTN2:
			if (!(sampleSize == 8 && ((imageType == TYPE_GRAY && samplesPerPixel == 1)
			...
			if (dir.isTagPresent(TIFF_JPEG_TABLES)) {
				 ...
				 * decoder.decodeAsRaster(); decodeParam = decoder.getJPEGDecodeParam();
				 */
			}
		...
		case TYPE_RGB_ALPHA:
			...
			sampleModel = new PixelInterleavedSampleModel(dataType, tileWidth, tileHeight, numBands,
					numBands * tileWidth, reverseOffsets);

			if (imageType == TYPE_GRAY) {
				colorModel = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_GRAY),
						new int[] { sampleSize }, false, false, Transparency.OPAQUE, dataType);
			} else if (imageType == TYPE_RGB) {
				colorModel = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB),
						new int[] { sampleSize, sampleSize, sampleSize }, false, false, Transparency.OPAQUE, dataType);
			} else { // hasAlpha
		...
		case TYPE_YCBCR_SUB:
			...
			sampleModel = new PixelInterleavedSampleModel(dataType, tileWidth, tileHeight, numBands,
					numBands * tileWidth, bandOffsets);
			colorModel = null;
			break;
		...
		case TYPE_PALETTE:
			...
			if (decodePaletteAsShorts) {
				...
				colorModel = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB),
						new int[] { 16, 16, 16 }, false, false, Transparency.OPAQUE, dataType);
				...
				if (sampleSize == 4) {
					...
					sampleModel = createPixelInterleavedSampleModel(DataBuffer.TYPE_BYTE, tileWidth, tileHeight,
							numBands);
					...
					// negative.
					dataType = DataBuffer.TYPE_USHORT;
					sampleModel = createPixelInterleavedSampleModel(DataBuffer.TYPE_USHORT, tileWidth, tileHeight,
							numBands);
				}
	...
	public synchronized Raster getTile(int tileX, int tileY) {
		...
		if (imageType == TYPE_BILEVEL) { // bilevel
				...
				if (decodePaletteAsShorts) {
					...
					try {
						...
						if (compression == COMP_PACKBITS) {
							...
							/*
							 * stream.readFully(data, 0, byteCount); Raster tempTile = decodeJPEG(data,
							 * decodeParam, colorConvertJPEG, tile.getMinX(), tile.getMinY()); int[]
							 * tempPixels = new int[unitsBeforeLookup]; tempTile.getPixels(tile.getMinX(),
							 * tile.getMinY(), tile.getWidth(), tile.getHeight(), tempPixels); tempData =
							 * new byte[unitsBeforeLookup]; for(int i = 0; i < unitsBeforeLookup; i++) {
					...
					try {
						...
						if (compression == COMP_PACKBITS) {
							...
							stream.skipFully(byteCount);
							/*
							 * stream.readFully(data, 0, byteCount); tile.setRect(decodeJPEG(data,
							 * decodeParam, colorConvertJPEG, tile.getMinX(), tile.getMinY()));
							 */
						} else if (compression == COMP_DEFLATE) {
			...
			try {
				...
				} else if (sampleSize == 32 && dataType == DataBuffer.TYPE_INT) { // redundant
					if (compression == COMP_NONE) {
	...
	private void interpretBytesAsInts(byte[] byteArray, int[] intArray, int intCount) {
		...
		if (isBigEndian) {
			...
			for (int i = 0; i < intCount; i++) {
				intArray[i] = (((byteArray[j++] & 0xff) << 24) | ((byteArray[j++] & 0xff) << 16)
			...
			for (int i = 0; i < intCount; i++) {
				intArray[i] = ((byteArray[j++] & 0xff) | ((byteArray[j++] & 0xff) << 8)
	...
	private ComponentColorModel createAlphaComponentColorModel(int dataType, int numBands, boolean isAlphaPremultiplied,
			int transparency) {
