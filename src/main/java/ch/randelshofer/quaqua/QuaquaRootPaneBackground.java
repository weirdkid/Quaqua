...
public class QuaquaRootPaneBackground extends PaintableColor {
	 ...
	 * internally without copying them. Do not modify these arrays after invoking
	 * this method.
	 */
	...
	public Paint getPaint(Component c, int x, int y, int width, int height) {
		...
		if (isTextured) {
			...
			return new LinearGradientPaint(new Point2D.Float(x, rootOffset),
					new Point2D.Float(x, rootOffset + rootHeight),
					new float[] { 0f, 1f / rootHeight, 81f / rootHeight, (rootHeight - 41f) / rootHeight,
