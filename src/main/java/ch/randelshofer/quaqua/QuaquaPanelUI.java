...
public class QuaquaPanelUI extends BasicPanelUI {
	...
	public void paint(Graphics gr, JComponent c) {
		...
		if (backgroundBorder != null) {
			backgroundBorder.paintBorder(c, bg, insets.left, insets.top, c.getWidth() - insets.left - insets.right,
	...
	/**
	 * Set a clipping region as appropriate for a titled border so that it does not
	...
	private void setTitledBorderClip(Graphics2D g, JComponent c, TitledBorder tb) {
		...
		path.append(new Rectangle(bounds.x + bounds.width + 2, bounds.y, c.getWidth() - bounds.width - bounds.x - 2,
				bounds.height), false);
		path.append(new Rectangle(0, bounds.y + bounds.height, c.getWidth(), c.getHeight() - bounds.height - bounds.y),
				false);
		g.clip(path);
	}

	/**
	 * Figure out where a TitledBorder would paint its title. This method basically
	 * attempts to simulate the behavior of TitledBorder. There is no guarantee that
	 * TitledBorder won't change its behavior or be overridden.
	 */
	...
	/**
	 * Figure out where a TitledBorder would paint its border. This method basically
	 * attempts to simulate the behavior of TitledBorder. There is no guarantee that
	 * TitledBorder won't change its behavior or be overridden.
	 */
	...
	private Insets getTitledBorderBorderPaintingInsets(Component c, TitledBorder tb) {
		...
		/*
		 * The border insets of the TitledBorder is larger than the insets of the
		 * painted border to make room for the label and to provide a margin around the
		 * border and a margin between the border and the contents. The left and right
		 * insets are based only on the margins, so we can figure out the total margin
		 * from that.
		 */
		...
		/*
		 * The remaining margin provides a place for the title, depending upon the title
		 * position option.
		 */
