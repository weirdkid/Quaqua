...
public class QuaquaOptionPaneUI extends BasicOptionPaneUI {
	...
	protected void installComponents() {
		...
		Methods.invokeIfExists(optionPane, "applyComponentOrientation", ComponentOrientation.class,
	 ...
	 * the message. The icon is the created by calling <code>addIcon</code>.
	 */
	@Override
	...
	protected int getMaxCharactersPerLineCount() {
		return Math.min(optionPane.getMaxCharactersPerLineCount(),
	 ...
	 * <code>buttons</code> and adds it to <code>container</code>. This differs from
	 * addMessageComponents in that it will recurse on <code>buttons</code> and that
	 * if button is not a Component it will create an instance of JButton.
	 */
	@Override
	...
	/**
	 * Creates the appropriate object to represent <code>msg</code> and places it
	 * into <code>container</code>. If <code>msg</code> is an instance of Component,
	 * it is added directly, if it is an Icon, a JLabel is created to represent it,
	 * otherwise a JLabel is created for the string, if <code>d</code> is an
	 * Object[], this method will be recursively invoked for the children.
	 * <code>internallyCreated</code> is true if Objc is an instance of Component
	 * and was created internally by this method (this is used to correctly set
...
// We use a plain font for HTML messages to make the examples in the
		...
		Font messageFont = (isHTML) ? UIManager.getFont("OptionPane.htmlMessageFont")
		...
		if (optionPane != null) {
			...
			if (suppliedOptions == null) {
				...
				if (type == JOptionPane.YES_NO_OPTION) {
					...
					defaultOptions[0] = new ButtonFactory(UIManager.getString("OptionPane.yesButtonText"),
							getMnemonic("OptionPane.yesButtonMnemonic", l), (Icon) UIManager.get("OptionPane.yesIcon"));
					defaultOptions[1] = new ButtonFactory(UIManager.getString("OptionPane.noButtonText"),
							getMnemonic("OptionPane.noButtonMnemonic", l), (Icon) UIManager.get("OptionPane.noIcon"));
					...
					defaultOptions[2] = new ButtonFactory(UIManager.getString("OptionPane.cancelButtonText"),
							...
							(Icon) UIManager.get("OptionPane.cancelIcon"));
				} else if (type == JOptionPane.OK_CANCEL_OPTION) {
					defaultOptions = new ButtonFactory[2];
					defaultOptions[0] = new ButtonFactory(UIManager.getString("OptionPane.okButtonText"),
							getMnemonic("OptionPane.okButtonMnemonic", l), (Icon) UIManager.get("OptionPane.okIcon"));
	...
	protected Object getMessage() {
		...
		if (optionPane != null) {
			if (optionPane.getWantsInput()) {
				/*
				 * Create a user component to capture the input. If the selectionValues are non
				 * null the component and there are < 20 values it'll be a combobox, if non null
				 * and >= 20, it'll be a list, otherwise it'll be a textfield.
				 */
				...
				if (sValues != null) {
					...
					if (optionPane
							.getClientProperty("PrivateQuaqua.OptionPane.InputFieldDocument") instanceof Document) {
						tf.setDocument(
								(Document) optionPane.getClientProperty("PrivateQuaqua.OptionPane.InputFieldDocument"));
					}
	 ...
	 * <code>FlowLayout</code>. It lays out all components from left to right. If
	 * <code>syncAllWidths</code> is true, the widths of each component will be set
	 * to the largest preferred size width.
	 *
	 * This inner class is marked &quot;public&quot; due to a compiler bug. This
	 * class should be treated as a &quot;protected&quot; inner class. Instantiate
	 * it only within subclasses of BasicOptionPaneUI.
	 */
	public static class QuaquaButtonAreaLayout extends BasicOptionPaneUI.ButtonAreaLayout {

		/**
		 * The destructive option is placed at least 24 pixels further away from the
		 * non-destructive options. Set this to -1 to specify that there is no
		 * destructive option.
		 */
		protected int destructiveOption = -1;
		/*
		 * protected boolean syncAllWidths; protected int padding; /** If true, children
		 * are lumped together in parent. * / protected boolean centersChildren;
		 */

		public QuaquaButtonAreaLayout(boolean syncAllWidths, int padding) {
			super(syncAllWidths, padding);
			/*
			 * this.syncAllWidths = syncAllWidths; this.padding = padding; centersChildren =
			 * true;
			 */
				...
				if (syncAllWidths) {
					...
					if (getCentersChildren()) {
						xLocation = (container.getSize().width - insets.left - insets.right
								- (maxWidth * numChildren + (numChildren - 1) * padding)) / 2;
					} else {
						if (ltr) {
					...
					if (ltr) {
						if (numChildren > 1) {
							xLocation += maxWidth * (numChildren - 1) + (numChildren - 1) * padding;
						}
						xOffset = -xOffset;
					}
					...
					for (counter = 0; counter < numChildren; counter++) {
						sizes[counter] = children[counter].getPreferredSize();
						sizes[counter].width = Math.max(MIN_BUTTON_WIDTH,
								sizes[counter].width + HORIZONTAL_BUTTON_PADDING);
					...
					if (cc) {
						xLocation = insets.left
								+ (container.getSize().width - insets.left - insets.right - totalWidth) / 2;
					} else {
						if (ltr) {
					...
					if (ltr) {
						...
						for (counter = 0; counter < numChildren; counter++) {
							children[counter].setBounds(xLocation, yLocation, sizes[counter].width,
				...
				if (children != null && children.length > 0) {
					...
					if (syncAllWidths) {
						...
						return new Dimension(extraWidth + (maxWidth * numChildren) + (numChildren - 1) * padding
								+ ((destructiveOption != -1) ? 14 : 0), extraHeight + height);
					} else {
						int totalWidth = 0;
						...
						totalWidth += ((numChildren - 1) * padding);
						return new Dimension(extraWidth + totalWidth + ((destructiveOption != -1) ? 14 : 0),
								extraHeight + height);
					}
				}
			}
	...
	/**
	 * This class is used to create the default buttons. This indirection is used so
	 * that addButtonComponents can tell which Buttons were created by us vs
	 * subclassers or from the JOptionPane itself.
	 */
		...
		public void propertyChange(PropertyChangeEvent e) {
			if (e.getSource() == optionPane) {
				...
				if ("ancestor" == e.getPropertyName()) {
					...
					/*
					 * switch (op.getMessageType()) { case JOptionPane.PLAIN_MESSAGE: if
					 * (isComingUp) { QuaquaLookAndFeel.playSound(optionPane,
					 * "OptionPane.informationSound"); } break; case JOptionPane.QUESTION_MESSAGE:
					 * if (isComingUp) { QuaquaLookAndFeel.playSound(optionPane,
					 * "OptionPane.questionSound"); } break; case JOptionPane.INFORMATION_MESSAGE:
					 * if (isComingUp) { QuaquaLookAndFeel.playSound(optionPane,
					 * "OptionPane.informationSound"); } break; case JOptionPane.WARNING_MESSAGE: if
					 * (isComingUp) { QuaquaLookAndFeel.playSound(optionPane,
					 * "OptionPane.warningSound"); } break; case JOptionPane.ERROR_MESSAGE: if
					 * (isComingUp) { QuaquaLookAndFeel.playSound(optionPane,
					 * "OptionPane.errorSound"); } break; default:
				...
				if (changeName == JOptionPane.OPTIONS_PROPERTY || changeName == JOptionPane.INITIAL_VALUE_PROPERTY
						|| changeName == JOptionPane.ICON_PROPERTY || changeName == JOptionPane.MESSAGE_TYPE_PROPERTY
						|| changeName == JOptionPane.OPTION_TYPE_PROPERTY || changeName == JOptionPane.MESSAGE_PROPERTY
					...
					if (buttonAreaLayout != null) {
						buttonAreaLayout.setDestructiveOption((value == null) ? -1 : value.intValue());
					}
				}
			}
		}
