...
public class QuaquaTableHeaderUI extends BasicTableHeaderUI {
	...
	public void installDefaults() {
		...
		if (header.getDefaultRenderer() instanceof JLabel) {
			((JLabel) header.getDefaultRenderer()).setHorizontalAlignment(SwingConstants.LEADING);
	...
	public void paint(Graphics g, JComponent c) {
		 ...
		 * // Replace this with the index of the last column. if (cMax == -1) { cMax =
		 * cm.getColumnCount()-1; }
		 */
		// If the table does not have enough columns to fill the view we'll get -1.
	...
	private class ColumnHandler implements TableColumnModelListener {
		...
		private void updateViewport() {
			...
			if (property != null && property.equals("striped") && table.getParent() instanceof JViewport) {
				JViewport viewport = (JViewport) table.getParent();
				if (viewport.getHeight() > table.getHeight()) {
					viewport.repaint(0, table.getHeight(), viewport.getWidth(),
							viewport.getHeight() - table.getHeight());
				}
			}
		}
