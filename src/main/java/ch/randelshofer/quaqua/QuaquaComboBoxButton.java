...
public class QuaquaComboBoxButton extends JButton {
	...
	private static Border getFocusRing() {
		if (focusRing == null) {
			focusRing = QuaquaBorderFactory.create(
					...
					new Insets(4, 6, 4, 6), new Insets(0, 0, 0, 0), false);
		}
		return focusRing;
	}
	...
	public QuaquaComboBoxButton(QuaquaComboBoxUI ui, JComboBox cb, Icon i, boolean onlyIcon, CellRendererPane pane,
			JList list) {
		super("");
	...
	public void paintComponent(Graphics g) {
		...
		if (!iconOnly && comboBox != null) {
			...
			if (leftToRight) {
				rendererPane.paintComponent(g, c, this, cellBounds.x - getX(), cellBounds.y - getY(), cellBounds.width,
						cellBounds.height, shouldValidate);
			} else {
				rendererPane.paintComponent(g, c, this, cellBounds.x, cellBounds.y, cellBounds.width, cellBounds.height,
						shouldValidate);
			}
			if (c instanceof JComponent) {
