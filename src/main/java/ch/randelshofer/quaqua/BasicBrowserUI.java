...
public class BasicBrowserUI extends BrowserUI {
	...
	protected void installDefaults() {
		if (browser.getColumnCellRenderer() == null || (browser.getColumnCellRenderer() instanceof UIResource)) {
	...
	static class BrowserTransferHandler extends TransferHandler implements UIResource/* , Comparator */ {
		...
		/*
		 * public int compare(Object o1, Object o2) { int row1 =
		 * browser.getRowForPath((TreePath)o1); int row2 =
		 * browser.getRowForPath((TreePath)o2); return row1 - row2; }
		 */
		...
		/**
		 * Selection paths are in selection order. The conversion to HTML requires
		 * display order. This method resorts the paths to be in the display order.
		 */
		TreePath[] getDisplayOrderPaths(TreePath[] paths) {
			// sort the paths to display order rather than selection order
			ArrayList<TreePath> selOrder = new ArrayList<>();
