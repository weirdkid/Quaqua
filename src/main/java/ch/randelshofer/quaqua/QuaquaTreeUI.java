 ...
 * QuaquaTreeUI.
 *
 * XXX - Without copying a substantial amount of code from BasicTreeUI, we can't
 * implement the proper selection behavior for a JTree.
 *
...
public class QuaquaTreeUI extends BasicTreeUI {
	...
	/**
	 * Last width the tree was at when painted. This is used when !leftToRigth to
	 * notice the bounds have changed so that we can instruct the TreeState to
	 * relayout.
	 */
	...
	protected void installKeyboardActions() {
		...
		QuaquaLazyActionMap.installLazyActionMap(tree, QuaquaTreeUI.class, "Tree.actionMap");
	}
	...
	protected TreeCellEditor createDefaultCellEditor() {
		if (currentCellRenderer != null && (currentCellRenderer instanceof DefaultTreeCellRenderer)) {
			DefaultTreeCellEditor editor = new QuaquaTreeCellEditor(tree,
	...
	protected boolean isToggleSelectionEvent(MouseEvent event) {
		return event.getID() == MouseEvent.MOUSE_PRESSED && SwingUtilities.isLeftMouseButton(event)
				&& event.isMetaDown();
	}
...
//        return leadRow;
	...
	/**
	 * Determines whether the node handles are to be displayed. Regardless of what
	 * value you specify here, the Quaqua look and feel always shows the root
	 * handles.
	 */
	@Override
	...
	/*
	 * private int getLeadSelectionRow() { TreePath leadPath =
	 * tree.getLeadSelectionPath(); return (leadPath == null) ? -1 :
	 * getRowForPath(tree, leadPath); }
	 */
	...
	protected void paintExpandControl(Graphics g, Rectangle clipBounds, Insets insets, Rectangle bounds, TreePath path,
		...
		if (!isLeaf) {
			...
			if (treeIcon != null) {
				drawCentered(tree, g, treeIcon, middleXOfKnob, middleYOfKnob);
			}
		}
	}
	...
	private Icon getTreeIcon(boolean isExpanded, boolean isSelected) {
		...
		if (icons == null) {
			...
			/*
			 * if (!isSideBar && !QuaquaUtilities.isOnActiveWindow(tree)) { index += 2; }
			 * else { if (isSelected && QuaquaUtilities.isFocused(tree)) { index++; } else
			 * if (!tree.isEnabled()) { index += 2; } }
			 */
			return icons[index];
		}
	}
...
//
	...
	public void paint(Graphics gr, JComponent c) {
		...
		boolean isSideBar = isSideBar();
		Color[] stripes = { UIManager.getColor("Tree.alternateBackground.0"),
				UIManager.getColor("Tree.alternateBackground.1") };
...
//than ferret out problem at the source, this compensates.
					...
					paintRow(g, paintBounds, insets, bounds, path, row, isExpanded, hasBeenExpanded, isLeaf, isEnabled,
							isFocused, isActive);
					if ((bounds.y + bounds.height) >= endY) {
						done = true;
					}
	...
	protected void paintVerticalPartOfLeg(Graphics g, Rectangle clipBounds, Insets insets, TreePath path) {
		/*
		 * Never draw lines if (QuaquaManager.getBoolean("Tree.paintLines")) {
	...
	protected void paintHorizontalPartOfLeg(Graphics g, Rectangle clipBounds, Insets insets, Rectangle bounds,
		...
		/*
		 * Never draw lines if (QuaquaManager.getBoolean("Tree.paintLines")) {
		 * super.paintHorizontalPartOfLeg(g, clipBounds, insets, bounds, leadPath, row,
	...
	protected void paintVerticalLine(Graphics g, JComponent c, int x, int top, int bottom) {
		/*
		 * if (QuaquaManager.getBoolean("Tree.paintLines")) { super.paintVerticalLine(g,
		 * c, x, top, bottom); }
		 */
	}
	...
	protected void paintRow(Graphics g, Rectangle clipBounds, Insets insets, Rectangle bounds, TreePath path, int row,
			boolean isExpanded, boolean hasBeenExpanded, boolean isLeaf, boolean isEnabled, boolean isFocused,
			boolean isActive) {
		...
		component = currentCellRenderer.getTreeCellRendererComponent(tree, path.getLastPathComponent(), isRowSelected,
		...
		if (isSideBar && component instanceof JLabel) {
			...
			label.putClientProperty("Quaqua.Label.style",
					isTopLevel
							? isRowSelected ? (isActive ? "categorySelected" : "categoryInactiveSelected")
									: (isActive ? "category" : "categoryInactive")
							: isRowSelected ? (isActive ? "rowSelected" : "rowInactiveSelected")
	...
	protected class Handler implements CellEditorListener, FocusListener, KeyListener, MouseListener,
				...
				if (name != null && name.equals("Frame.active")) {
					if (tree.getClientProperty("Quaqua.Tree.style") != null //
							&& (tree.getClientProperty("Quaqua.Tree.style").equals("sideBar")
									|| tree.getClientProperty("Quaqua.Tree.style").equals("sourceList"))) {
						tree.repaint();
					}
			...
			if (tree.isEnabled()) {
				...
				if (index != -1) {
					...
					if (isRowAtIndexSelected && e.isPopupTrigger()) {
						...
						if ((e.getModifiersEx() & (InputEvent.META_DOWN_MASK | InputEvent.BUTTON2_DOWN_MASK
						...
						} else if ((e.getModifiersEx() & (InputEvent.SHIFT_DOWN_MASK | InputEvent.BUTTON2_DOWN_MASK
								| InputEvent.BUTTON3_DOWN_MASK)) == InputEvent.SHIFT_DOWN_MASK && anchorIndex != -1) {
							...
							if (isRowAtIndexSelected) {
								...
								if (tree.getDragEnabled() && getPathBounds(tree, path).contains(e.getPoint())) {
									isDragRecognitionOngoing = QuaquaDragRecognitionSupport.mousePressed(e);
									mouseDragSelects = mouseReleaseDeselects = false;
									isMouseReleaseStartsEditing = false;
								} else {
		...
		public void mouseDragged(MouseEvent e) {
			if (tree.isEnabled()) {
				...
				TreePath leadPath = getClosestPathForLocation(tree, e.getX(), e.getY());
	...
	private static class Actions extends QuaquaUIAction {
		...
		private static final String MOVE_SELECTION_TO_PARENT = "moveSelectionToParent"; // add the lead item to the
																						// selection without changing
																						// lead or anchor
		private static final String ADD_TO_SELECTION = "addToSelection"; // toggle the selected state of the lead item
																			// and move the anchor to it
		private static final String TOGGLE_AND_ANCHOR = "toggleAndAnchor"; // extend the selection to the lead item
		private static final String EXTEND_TO = "extendTo"; // move the anchor to the lead and ensure only that item is
															// selected
		...
		private void scrollChangeSelection(JTree tree, QuaquaTreeUI ui, int direction, boolean addToSelection,
		...
		private void increment(JTree tree, QuaquaTreeUI ui, int direction, boolean addToSelection,
				boolean changeSelection) {
	...
	public class TreeTraverseAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			if (tree != null) {
				SHARED_ACTION.traverse(tree, QuaquaTreeUI.this, direction, changeSelection);
			}
		}
	...
	public class TreePageAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			if (tree != null) {
				SHARED_ACTION.page(tree, QuaquaTreeUI.this, direction, addToSelection, changeSelection);
			}
		}
	...
	public class TreeIncrementAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			if (tree != null) {
				SHARED_ACTION.increment(tree, QuaquaTreeUI.this, direction, addToSelection, changeSelection);
			}
		}
	...
	public class TreeHomeAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			if (tree != null) {
				SHARED_ACTION.home(tree, QuaquaTreeUI.this, direction, addToSelection, changeSelection);
			}
		}
