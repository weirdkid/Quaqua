...
/**
 * A replacement for the AquaTabbedPaneUI for Mac OS X 10.2 Jaguar. Tabs of
 * tabbed panes are stacked instead of moved into a popup menu, if not enough
 * space is available to render all tabs in a single line.
 * <p>
 * Supports the following client properties on the children of the JTabbedPane:
 * <code>Quaqua.TabbedPaneChild.contentBackground</code> specifies the
 * background Color to be used to fill the content border.
 * <code>Quaqua.TabbedPaneChild.contentInsets</code> specifies the insets to be
 * used to lay out the child component inside the JTabbedPane.
 *
...
public class QuaquaJaguarTabbedPaneUI extends BasicTabbedPaneUI implements VisuallyLayoutable, NavigatableTabbedPaneUI {
	...
	/**
	 * This variable is set in method paint() and used in all subsequent paint...()
	 * methods.
	 */
	private boolean isFrameActive;

	/**
	 * This is the border painted around the content area. Don't cache this border
	 * because it usually encompasses a huge are on a panel.
	 */
	...
	private static Border createNonCachedImageBorder(String name, Insets insets) {
		return QuaquaBorderFactory.create(Images.createImage(QuaquaJaguarTabbedPaneUI.class.getResource(name)), insets,
				insets, true, null, false);
	}

	/**
	 * This is the bar used when the tabs are at the top.
	 *
	 * Indices 0: active 1: inactive 2: disabled
	 */
	 ...
	 *
	 * Indices 0: enabled 1: selected 2: inactive 3: disabled 4: disabled selected
	 */
	...
	private Border getTabTopBorder(int i) {
		if (tabTopBorder == null) {
			...
			tabTopBorder = new Border[] { createImageBorder("images/TabbedPane.tabTop.png", insets),
	...
	private Border getTabBottomBorder(int i) {
		if (tabBottomBorder == null) {
			...
			tabBottomBorder = new Border[] { createImageBorder("images/TabbedPane.tabBottom.png", insets),
	...
	private Border getTabRightBorder(int i) {
		if (tabRightBorder == null) {
			...
			tabRightBorder = new Border[] { createImageBorder("images/TabbedPane.tabRight.png", insets),
	...
	private Border getTabLeftBorder(int i) {
		if (tabLeftBorder == null) {
			...
			tabLeftBorder = new Border[] { createImageBorder("images/TabbedPane.tabLeft.png", insets),
	...
	protected void paintTabBorderTop(int tabIndex, Graphics g, int x, int y, int w, int h, int btm, int rght,
			boolean isSelected) {
		/*
		 * Indices 0: enabled 1: selected 2: inactive 3: disabled 4: disabled selected
		 */
		...
		if (isSelected) {
			...
			if (tabPane.isEnabled()) {
				if (isFrameActive) {
					...
					// border = 3;
					border = 0; /*
								 * click-through behavior:use same border for inactive frames, we have got click
								 */
				}
			} else {
				border = 3;
	...
	protected void paintTabBorderLeft(int tabIndex, Graphics g, int x, int y, int w, int h, int btm, int rght,
			boolean isSelected) {
		/*
		 * Indices 0: enabled 1: selected 2: inactive 3: disabled 4: disabled selected
		 */
	...
	protected void paintTabBorderBottom(int tabIndex, Graphics g, int x, int y, int w, int h, int btm, int rght,
			boolean isSelected) {
		/*
		 * Indices 0: enabled 1: selected 2: inactive 3: disabled 4: disabled selected
		 */
	...
	protected void paintTabBorderRight(int tabIndex, Graphics g, int x, int y, int w, int h, int btm, int rght,
			boolean isSelected) {
		/*
		 * Indices 0: enabled 1: selected 2: inactive 3: disabled 4: disabled selected
		 */
	...
	/**
	 * Overridden to return specific shift values for the Quaqua L&amp;F. FIXME We
	 * should find another way to align the labels properly.
	 */
	@Override
	...
	public void paint(Graphics gr, JComponent c) {
		...
		if (tabPane.isOpaque()) {
			...
			/*
			 * switch ( tabPlacement ) { case LEFT: g.fillRect( insets.left, insets.top,
			 * calculateTabAreaWidth( tabPlacement, runCount, maxTabWidth ), size.height -
			 * insets.bottom - insets.top ); break; case BOTTOM: int totalTabHeight =
			 * calculateTabAreaHeight( tabPlacement, runCount, maxTabHeight ); g.fillRect(
			 * insets.left, size.height - insets.bottom - totalTabHeight, size.width -
			 * insets.left - insets.right, totalTabHeight ); break; case RIGHT: int
			 * totalTabWidth = calculateTabAreaWidth( tabPlacement, runCount, maxTabWidth );
			 * g.fillRect( size.width - insets.right - totalTabWidth, insets.top,
			 * totalTabWidth, size.height - insets.top - insets.bottom ); break; case TOP:
			 * default: g.fillRect( insets.left, insets.top, size.width - insets.right -
	...
	protected void paintContentBorderTopEdge(Graphics g, int tabPlacement, int selectedIndex, int x, int y, int w,
		...
		getContentBorder().paintBorder(tabPane, g, x, y - contentBorderInsets.top + 1, w,
				h + contentBorderInsets.top - 1);
		/*
		 * Indices 0: active 1: inactive 2: disabled
		 */
		int bar;
		...
		getBarTopBorder(bar).paintBorder(tabPane, g, x + contentBorderInsets.left, y,
				w - contentBorderInsets.left - contentBorderInsets.right, 7);
	}
	...
	protected void paintContentBorderBottomEdge(Graphics g, int tabPlacement, int selectedIndex, int x, int y, int w,
		...
		getBarBottomBorder(bar).paintBorder(tabPane, g, x + contentBorderInsets.left, y + h - 6,
				w - contentBorderInsets.left - contentBorderInsets.right, 6);
	}
	...
	protected int getTabRunOverlay(int tabPlacement) {
		...
		/*
		 * // Tab runs laid out vertically should overlap // at least as much as the
		 * largest slant if ( tabPlacement == LEFT || tabPlacement == RIGHT ) { int
		 * maxTabHeight = calculateMaxTabHeight(tabPlacement); return maxTabHeight / 2;
		 * } return 0;
		 */
	}
	...
	protected void paintText(Graphics g, int tabPlacement, Font font, FontMetrics metrics, int tabIndex, String title,
		...
		if (v != null) {
			...
			if (tabPane.isEnabled() && tabPane.isEnabledAt(tabIndex)) {
				...
				QuaquaUtilities.drawStringUnderlineCharAt(g, title, mnemIndex, textRect.x,
	 ...
	 * it only within subclasses of MetalTabbedPaneUI.
	 */
	public class TabbedPaneLayout extends BasicTabbedPaneUI.TabbedPaneLayout {
		...
		protected void calculateTabRects(int tabPlacement, int tabCount) {
			...
			int maxPad = 0;
			/*
			 * for (i = runCount - 1; i >= 0; i--) { int start = tabRuns[i]; int next =
			 * tabRuns[i == (runCount - 1)? 0 : i + 1]; int end = (next != 0? next - 1 :
			 * tabCount - 1);
			 * 
	...
	private static class TabSelectionHandler implements ChangeListener {
		...
		public void stateChanged(ChangeEvent e) {
			...
			tabPane.repaint();
			/*
			 * if (tabPane.getTabLayoutPolicy() == JTabbedPane.SCROLL_TAB_LAYOUT) { int
			 * index = tabPane.getSelectedIndex(); if (index < rects.length && index != -1)
