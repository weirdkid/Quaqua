 ...
 * the one provided with the native Aqua user interface on Mac OS X 10.2
 * (Jaguar).
 *
...
public class QuaquaJaguarFileChooserUI extends BasicFileChooserUI {
	...
	public void installComponents(JFileChooser fc) {
		...
		Component[] dropComponents = { fc, accessoryPanel, approveButton, browser, browserScrollPane, buttonPanel,
				cancelButton, directoryComboBox, fileNameLabel, fileNameTextField, filesOfTypeLabel, filterComboBox,
				formatPanel, formatPanel2, fromPanel, lookInLabel, newFolderButton, separatorPanel, separatorPanel1,
				separatorPanel2, strutPanel1, strutPanel2 };
	...
	protected void installStrings(JFileChooser fc) {
		...
		/// newFolderButtonText = getString("FileChooser.newFolderButtonText", l,
		/// "New");
	...
	public Dimension getPreferredSize(JComponent c) {
		...
		if (d != null) {
			return new Dimension(Math.max(d.width, PREF_SIZE.width), Math.max(d.height, PREF_SIZE.height));
		} else {
	...
	private void updateSeparatorPanelVisibility() {
		...
		separatorPanel.setVisible(isSave
				&& (fc.getControlButtonsAreShown() || !fc.isAcceptAllFileFilterUsed() || fc.getAccessory() != null));
	}
	...
	class DirectoryComboBoxRenderer extends DefaultListCellRenderer {
		...
		public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected,
			 ...
			 * getFileChooser().getIcon(root); ii.icon =
			 * UIManager.getIcon("FileView.computerIcon"); } else {
			 */
	...
	private void maybeApproveSelection() {
		...
		if (fc.isMultiSelectionEnabled()) {
			...
			selectedFile = ((FileSystemTreeModel.Node) browser.getSelectionPath().getLastPathComponent())
					.getResolvedFile();
			if (filename != null) {
				selectedFile = new File(selectedFile.isDirectory() && fc.isTraversable(selectedFile) ? selectedFile
						: fc.getFileSystemView().getParentDirectory(selectedFile), filename);
			}
			if (fc.getFileSelectionMode() == JFileChooser.FILES_ONLY && selectedFile.isDirectory()
					&& fc.isTraversable(selectedFile)) {
	...
	protected class NewFolderAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent actionevent) {
			...
			if (newFolderName != null) {
				...
				File newFolder;
				File currentFile = ((FileSystemTreeModel.Node) browser.getSelectionPath().getLastPathComponent())
						.getResolvedFile();
	...
	protected class SaveTextDocumentListener implements DocumentListener {
		...
		private void textChanged() {
			...
			SwingUtilities.invokeLater(new Runnable() {
				...
				public void run() {
					JFileChooser fc = getFileChooser();
					File file = ((FileSystemTreeModel.Node) browser.getSelectionPath().getLastPathComponent())
							.getResolvedFile();
