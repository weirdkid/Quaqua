...
public class QuaquaPopupMenuUI extends BasicPopupMenuUI implements QuaquaMenuPainterClient {
	 ...
	 * Toolkit.addAWTEventListener. Copied here, because
	 * sun.awt.SunToolkit.GRAB_EVENT_MASK is only available from J2SE6 onwards.
	 */
	...
	/**
	 * Returns the <code>Popup</code> that will be responsible for displaying the
	 ...
	 *//*
		 ...
		 * PopupFactory popupFactory = PopupFactory.getSharedInstance(); return
		 * popupFactory.getPopup(popup.getInvoker(), popup, x, y); }
		 */
	...
	static class MouseGrabber implements ChangeListener, AWTEventListener, ComponentListener, WindowListener {
				...
				public Object run() {
					tk.addAWTEventListener(MouseGrabber.this,
							AWTEvent.MOUSE_EVENT_MASK | AWTEvent.MOUSE_MOTION_EVENT_MASK
									| AWTEvent.MOUSE_WHEEL_EVENT_MASK | AWTEvent.WINDOW_EVENT_MASK | GRAB_EVENT_MASK);
					return null;
				}
			});
		...
		public void eventDispatched(AWTEvent ev) {
			...
			case MouseEvent.MOUSE_PRESSED:
				...
				if (!(src instanceof JComponent) || !(((JComponent) src).getClientProperty("doNotCancelPopup") != null
						&& ((JComponent) src).getClientProperty("doNotCancelPopup").toString()
								.equals(QuaquaComboBoxUI.HIDE_POPUP_KEY.toString()))//
				) {
	...
	/**
	 * This helper is added to MenuSelectionManager as a ChangeListener to listen to
	 * menu selection changes. When a menu is activated, it passes focus to its
	 * parent JRootPane, and installs an ActionMap/InputMap pair on that JRootPane.
	 * Those maps are necessary in order for menu navigation to work. When menu is
	 * being deactivated, it restores focus to the component that has had it before
	 * menu activation, and uninstalls the maps. This helper is also installed as a
	 * KeyListener on root pane when menu is active. It forwards key events to
	 * MenuSelectionManager for mnemonic keys handling.
	 */
	static class MenuKeyboardHelper implements ChangeListener, KeyListener {
		...
		/*
		 * Fix for 4213634 If this is false, KEY_TYPED and KEY_RELEASED events are NOT
		 * processed. This is needed to avoid activating a menuitem when the menu and
		 * menuitem share the same mnemonic.
		 */
		...
		/**
		 * Return the last JPopupMenu in <code>path</code>, or <code>null</code> if none
		 * found
		 */
	...
	private static class Actions extends UIAction {
		...
		private void selectParentChild(boolean direction) {
			...
			if (direction == PARENT) {
				...
				if (len > 2 && // check if we have an open submenu. A submenu item may or
		...
		private void cancel() {
			...
			if (path.length > 4) { /*
									 * PENDING(arnaud) Change this to 2 when a mouse grabber is available for
									 * MenuBar
									 */
