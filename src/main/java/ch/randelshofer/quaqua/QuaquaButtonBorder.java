...
/**
 * QuaquaButtonBorder. This border uses client properties and font sizes of a
 * JComponent to determine which style the border shall have. For some styles,
 * the JComponent should honour size constrictions.
 * <p>
 ...
 * <li><code>placard</code> or <code>gradient</code> Placard button. No size
 * constraints.</li>
 * <li><code>colorWell</code> Square button with color area in the center. No
 * size constraints.</li>
 * <li><code>bevel</code> Rounded Bevel button. No size constraints.</li>
 * <li><code>toggle</code> or <code>segmented</code> Toggle button. Maximum
 * height of the JComponent shall be constrained to its preferred height.</li>
 * <li><code>toggleWest</code> West Toggle button. Maximum height of the
 * JComponent shall be constrained to its preferred height.</li>
 * <li><code>toggleEast</code> East Toggle button. Maximum height of the
 * JComponent shall be constrained to its preferred height.</li>
 * <li><code>toggleCenter</code> Center Toggle button. Maximum height of the
 * JComponent shall be constrained to its preferred height.</li>
 ...
 * <li><code>toolBarRollover</code> ToolBar button with rollover effect. No size
 * constraints.</li>
 * </ul>
 * If the <code>Quaqua.Button.style</code> property is missing, then the
 * following values of the client property <code>JButton.buttonType</code> are
 * supported:
 * <ul>
...
public class QuaquaButtonBorder implements Border, PressedCueBorder, UIResource {
	...
	/**
	 * The default client property value to be used, when no client property has
	 * been specified for the JComponent.
	 */
	...
	public Border getActualBorder(Component c) {
		...
		if ("text".equals(style) || "push".equals(style)) {
			...
			if (toolBarBorder == null) {
				toolBarBorder = new CompositeVisualMarginBorder(
						new CompoundBorder(new EmptyBorder(-1, -1, -1, -2),
								new QuaquaToolBarButtonStateBorder(
										Images.createImage(
												QuaquaButtonBorder.class.getResource("images/Toggle.borders.png")),
										10, true, new Insets(8, 10, 15, 10), new Insets(4, 6, 4, 6), true, false)), //
						0, 0, 0, 0);
			}
			b = toolBarBorder;
		} else if ("toolBarRollover".equals(style)) {
			if (toolBarRolloverBorder == null) {
				toolBarRolloverBorder = new CompositeVisualMarginBorder(
						new CompoundBorder(new EmptyBorder(-1, -1, -1, -2),
								new QuaquaToolBarButtonStateBorder(
										Images.createImage(
												QuaquaButtonBorder.class.getResource("images/Toggle.borders.png")),
										10, true, new Insets(8, 10, 15, 10), new Insets(4, 6, 4, 6), true, true)), //
						0, 0, 0, 0);
			}
			...
			if (colorWellBorder == null) {
				colorWellBorder = new CompositeVisualMarginBorder(
						new OverlayBorder(new QuaquaColorWellBorder() /* ) */, new CompoundBorder(
								new EmptyBorder(-2, -2, -2, -2),
								new FocusBorder(QuaquaBorderFactory.create(
										Images.createImage(
												QuaquaButtonBorder.class.getResource("images/Square.focusRing.png")),
			...
			if (bevelBorder == null) {
				...
				Border focusBorder = new FocusBorder(QuaquaBorderFactory.create(
				...
				bevelBorder = new CompositeVisualMarginBorder(
						new CompoundBorder(new EmptyBorder(-3, -2, -2, -2),
								new OverlayBorder(
										new ButtonStateBorder(
												Images.createImage(QuaquaButtonBorder.class
														.getResource("images/RoundedBevel.borders.png")),
												10, true, new Insets(10, 9, 10, 8), borderInsets, true),
										new CompoundBorder(new EmptyBorder(0, -1, 0, -1), focusBorder))), //
						0, 0, 0, 0);
			}
			b = bevelBorder;
		} else if ("only".equals(segpos)
				&& ("toggle".equals(style) || "segmented".equals(style) || "segmentedRoundRect".equals(style)
						|| "segmentedCapsule".equals(style) || style.contains("segmentedTextured"))) {
			if (toggleBorder == null) {
				Insets borderInsets = new Insets(3, 5, 3, 5);
				toggleBorder = new CompositeVisualMarginBorder(new OverlayBorder(
						new ButtonStateBorder(
								Images.createImage(QuaquaButtonBorder.class.getResource("images/Toggle.borders.png")),
								10, true, new Insets(8, 10, 15, 10), borderInsets, true),
						new FocusBorder(QuaquaBorderFactory.create(
			...
			if (toggleEastBorder == null) {
				...
				toggleEastBorder = new CompositeVisualMarginBorder(
						new OverlayBorder(
								new ButtonStateBorder(
										Images.createImage(
												QuaquaButtonBorder.class.getResource("images/Toggle.east.borders.png")),
										10, true, new Insets(8, 1, 15, 10), borderInsets, true),
								new FocusBorder(QuaquaBorderFactory.create(
										Images.createImage(QuaquaButtonBorder.class
												.getResource("images/Toggle.east.focusRing.png")),
			...
			if (toggleCenterBorder == null) {
				...
				toggleCenterBorder = new CompositeVisualMarginBorder(
						new OverlayBorder(
								new ButtonStateBorder(
										Images.createImage(QuaquaButtonBorder.class
												.getResource("images/Toggle.center.borders.png")),
										10, true, new Insets(8, 0, 15, 1), borderInsets, true),
								new FocusBorder(QuaquaBorderFactory.create(
										Images.createImage(QuaquaButtonBorder.class
												.getResource("images/Toggle.center.focusRing.png")),
			...
			if (toggleWestBorder == null) {
				...
				toggleWestBorder = new CompositeVisualMarginBorder(
						new OverlayBorder(
								new ButtonStateBorder(
										Images.createImage(
												QuaquaButtonBorder.class.getResource("images/Toggle.west.borders.png")),
										10, true, new Insets(8, 10, 15, 1), borderInsets, true),
								new FocusBorder(QuaquaBorderFactory.create(
										Images.createImage(QuaquaButtonBorder.class
												.getResource("images/Toggle.west.focusRing.png")),
	...
	private Border getRegularPushButtonBorder() {
		if (regularPushButtonBorder == null) {
			...
			BufferedImage[] imageFrames = Images.split(
					Images.createImage(QuaquaButtonBorder.class.getResource("images/Button.default.png")), 12, true);
			Border[] borderFrames = new Border[12];
			for (int i = 0; i < 12; i++) {
				borderFrames[i] = QuaquaBorderFactory.create(imageFrames[i], new Insets(11, 13, 13, 13), borderInsets,
						true);
			}
			ButtonStateBorder buttonStateBorder = new ButtonStateBorder(
					Images.split(Images.createImage(QuaquaButtonBorder.class.getResource("images/Button.borders.png")),
							10, true),
					new Insets(11, 13, 13, 13), borderInsets, true);

			buttonStateBorder.setBorder(ButtonStateBorder.DEFAULT, new AnimatedBorder(borderFrames, 100));

			regularPushButtonBorder = new CompositeVisualMarginBorder(new OverlayBorder(buttonStateBorder,
					new FocusBorder(QuaquaBorderFactory.create(
							Images.createImage(QuaquaButtonBorder.class.getResource("images/Button.focusRing.png")),
							new Insets(12, 13, 12, 13), borderInsets, false))),
					2, 4, 2, 4);
		}
	...
	private Border getSquareBorder() {
		if (squareBorder == null) {
			squareBorder = new CompositeVisualMarginBorder(new OverlayBorder(
	...
	private Border getPlacardBorder() {
		if (placardBorder == null) {
			placardBorder = new CompositeVisualMarginBorder(new OverlayBorder(
					new CompoundBorder(new EmptyBorder(-1, 0, -1, 0), QuaquaBorderFactory.createPlacardButtonBorder()),
	...
	private Border getTableHeaderBorder() {
		if (tableHeaderBorder == null) {
			tableHeaderBorder = new CompositeVisualMarginBorder(new ButtonStateBorder(
					Images.createImage(QuaquaButtonBorder.class.getResource("images/TableHeader.borders.png")), 4, true,
					new Insets(7, 1, 8, 1), new Insets(1, 2, 1, 2), true), 0, 0, 0, 0);
		}
	...
	private Border getSmallPushButtonBorder() {
		if (smallPushButtonBorder == null) {
			...
			BufferedImage[] imageFrames = Images.split(
					Images.createImage(QuaquaButtonBorder.class.getResource("images/Button.small.default.png")), 12,
					true);
			Border[] borderFrames = new Border[12];
			for (int i = 0; i < 12; i++) {
				borderFrames[i] = QuaquaBorderFactory.create(imageFrames[i], new Insets(9, 13, 12, 13), borderInsets,
						true);
			}
			ButtonStateBorder buttonStateBorder = new ButtonStateBorder(Images.split(
					Images.createImage(QuaquaButtonBorder.class.getResource("images/Button.small.borders.png")), 10,
					true), new Insets(9, 13, 12, 13), borderInsets, true);
			buttonStateBorder.setBorder(ButtonStateBorder.DEFAULT, new AnimatedBorder(borderFrames, 100));

			smallPushButtonBorder = new CompositeVisualMarginBorder(new CompoundBorder(new EmptyBorder(-2, -3, -2, -3),
					new OverlayBorder(buttonStateBorder,
							new FocusBorder(QuaquaBorderFactory.create(
									Images.createImage(
											QuaquaButtonBorder.class.getResource("images/Button.small.focusRing.png")),
									new Insets(9, 14, 12, 14), borderInsets, false)))),
					0, 0, 0, 0);
		}
	...
	/**
	 * Returns true, if this border has a visual cue for the disabled state of the
	 * button. If the border has no visual cue, then the ButtonUI has to provide it
	 * by some other means. / public boolean hasDisabledCue(JComponent c) { return
	 * false; }
	 */
	@Override
	public Insets getBorderInsets(Component c) {
		...
		if (!isBorderPainted) {
			...
			insets = getActualBorder(c).getBorderInsets(c);
		}
