...
public class QuaquaUtilities extends BasicGraphicsUtils implements SwingConstants {
	...
	/**
	 * Draw a string with the graphics {@code g} at location ({@code x}, {@code y})
	 * just like {@code g.drawString} would. The character at index
	 * {@code underlinedIndex} in text will be underlined. If {@code index} is
	 * beyond the bounds of {@code text} (including &lt; 0), nothing will be
	 * underlined.
	 *
	...
	/**
	 * Returns index of the first occurrence of {@code mnemonic} within string
	 * {@code text}. Matching algorithm is not case-sensitive.
	 *
	...
	/**
	 * Returns true if the component is on a Dialog or a Frame, which is active, or
	 * if it is on a Window, which is focused. Always returns true, if the component
	 ...
	 * @param isActiveWhenSheetIsActive Set this to true, when the window should be
	 *                                  considered as active when its sheet dialog
	 *                                  is active.
	 */
	public static boolean isOnActiveWindow(Component c, boolean isActiveWhenSheetIsActive) {
		...
		if (window == null) {
			...
			isOnActiveWindow = window.isActive() || window.getName() == "###focusableSwingPopup###";// literal strings
																									// get interned
	...
	/**
	 * Returns a Mac OS X specific String describing the modifier key(s), such as
	 * "Shift", or "Ctrl+Shift".
	 *
	 * @return string a text description of the combination of modifier keys that
	...
	/**
	 * Returns true, if the specified component is focus owner or permanent focus
	 * owner and if the component is on an the active window.
	 */
	...
	public static int getLeftSideBearing(Font f, String string) {
		return ((Integer) Methods.invokeStatic(getSwingUtilities2(), "getLeftSideBearing",
				new Class[] { Font.class, String.class }, new Object[] { f, string }, 0)).intValue();
	}

	/**
	 * Invoked when the user attempts an invalid operation, such as pasting into an
	 * uneditable {@code JTextField} that has focus. The default implementation
	 * beeps. Subclasses that wish different behavior should override this and
	 * provide the additional feedback.
	 *
	 * @param component Component the error occured in, may be null indicating the
	...
	public static BufferedImage createBufferedImage(URL location) {
		...
		if (image instanceof BufferedImage) {
			...
			loadImage(image);
			// buf = new BufferedImage(image.getWidth(null), image.getHeight(null),
			// BufferedImage.TYPE_INT_RGB);
			buf = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration()
	 ...
	 * layoutCompoundLabel() instead.
	 *
	 * This is the same as SwingUtilities.layoutCompoundLabelImpl, except for the
	 * algorithm for clipping the text. If a text is too long, "..." are inserted at
	 * the middle of the text instead of at the end.
	 */
	...
	/**
	 * Uses some unsupported (dangerous) API calls on the native peers to make a
	 * window translucent. If the API is not found, this method leaves the window
	 * opaque.
	 *
	 * @param w     The Window.
		...
		try {
			if (component instanceof Container) {
				...
				if (Methods.invokeGetter(container, "isFocusCycleRoot", false)) {
					...
					Component comp = (Component) Methods.invoke(policy, "getDefaultComponent", Container.class,
							container);
					if (comp != null) {
						comp.requestFocus();
						return comp;
				...
				if (rootAncestor != null) {
					...
					Component comp = (Component) Methods.invoke(policy, "getComponentAfter",
							new Class[] { Container.class, Component.class }, new Object[] { rootAncestor, container });
	 ...
	 * value on a component if that property has not already been set by the client
	 * program. This method is intended to be used by UI delegate instances that
	 * need to specify a default value for a property of primitive type (boolean,
	 * int, ..), but do not wish to override a value set by the client. Since
	 * primitive property values cannot be wrapped with the UIResource marker, this
	 * method uses private state to determine whether the property has been set by
	 * the client.
	 * 
	 * @throws IllegalArgumentException if the specified property is not one which
	...
	/**
	 * Returns the ui that is of type {@code klass}, or null if one can not be
	 * found.
	 */
	...
	static boolean shouldIgnore(MouseEvent e, JComponent tree) {
		...
		try {
			return ((Boolean) Methods.invokeStatic(getSwingUtilities2(), "shouldIgnore",
					new Class[] { MouseEvent.class, JComponent.class }, new Object[] { e, tree })).booleanValue();
		} catch (NoSuchMethodException ex) {
			return false;
		}
	}
	 ...
	 * <p>
	 * The default size variant is "regular". If a component is a cell renderer, the
	 * default size variant is "small".
	 *
	 * @param c
	 * @return size variant.
	 */
	public static SizeVariant getSizeVariant(Component c) {
		...
		if (sv == null) {
			...
			if ((c instanceof TableCellRenderer) || (c instanceof TableCellEditor)
	...
	public static int convertModifiersToDropAction(final int modifiers, final int supportedActions) {
		...
		/*
		 * Issue: Shift-Drag means Link in Motif, but not in Aqua. Probably preserving
		 * this behavior because it is assumed by some Java applications. Perhaps should
		 * be configurable.
		 */

		/*
		 * Fix for 4285634. Calculate the drop action to match Motif DnD behavior. If
		 * the user selects an operation (by pressing a modifier key), return the
		 * selected operation or ACTION_NONE if the selected operation is not supported
		 * by the drag source. If the user doesn't select an operation search the set of
		 * operations supported by the drag source for ACTION_MOVE, then for
		 * ACTION_COPY, then for ACTION_LINK and return the first operation found.
		 */
	...
	/**
	 * Return the child {@code Component} of the specified {@code Component} that is
	 * the focus owner, if any.
	 *
	 * @param c the root of the {@code Component} hierarchy to search for the focus
	 *          owner
	 * @return the focus owner, or {@code null} if there is no focus owner, or if
	 *         the focus owner is not {@code comp}, or a descendant of {@code comp}
	 *
	...
	/**
	 * Returns the visual bounds of the component given in the local coordinate
