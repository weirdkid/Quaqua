...
//    private static HashSet<AbstractButton> animatedComponents = new HashSet<AbstractButton>();
	...
	/**
	 * Push Button. Preferred spacing between buttons and other components. /
	 ...
	 * private final static Insets pushMiniSpacing = new Insets(8,8,8,8); /** Metal
	 * Button. Preferred spacing between buttons and other components. / private
	 * final static Insets metalRegularSpacing = new Insets(12,12,12,12); private
	 * final static Insets metalSmallSpacing = new Insets(8,8,8,8); /** Bevel
	 * Button. Preferred spacing between buttons and other components.
	 *
	 * Large Spacing is used, if the bevel button contains an icon that is 24 x 24
	 * pixels or larger. / private final static Insets bevelLargeSpacing = new
	 * Insets(8,8,8,8); private final static Insets bevelRegularSpacing = new
	 * Insets(0,0,0,0); /** Square Button. Preferred spacing between buttons and
	 * other components. / private final static Insets squareSpacing = new
	 * Insets(0,0,0,0); /** Icon Button (buttons without border and an icon, and,
	 * optionally, text) Preferred spacing between buttons and other components.
	 *
	 * Large Spacing is used, if the icon button contains an icon that is 24 x 24
	 * pixels or larger. / private final static Insets iconLargeSpacing = new
	 * Insets(8,8,8,8); private final static Insets iconRegularSpacing = new
	 * Insets(0,0,0,0); /** Round Button and Help Button. / private final static
	 * Insets roundSpacing = new Insets(12,12,12,12);
	 */
	// ********************************
	// Create PLAF
	// ********************************
	...
	protected void paintText(Graphics g, AbstractButton b, Rectangle textRect, String text) {
		...
		if (model.isPressed() && model.isArmed() && !borderHasPressedCue) {
			...
			QuaquaUtilities.drawStringUnderlineCharAt(g, text, mnemonicIndex, textRect.x + getTextShiftOffset(),
	...
	protected void paintIcon(Graphics g, JComponent c, Rectangle iconRect) {
		...
		if (model.isPressed() && model.isArmed()) {
			if (shadowIcon != null) {
				shadowIcon.paintIcon(c, g, iconRect.x + getTextShiftOffset(), iconRect.y + getTextShiftOffset() + 1);
			}
	...
	public Dimension getMinimumSize(JComponent c) {
		...
		if (style == null) {
			if (b.getBorder() instanceof UIResource && b.isBorderPainted()) {
				style = "push";
			} else {
				style = "plain";
			}
	...
	protected static String getStyle(Component c, String defaultStyle) {
		...
		if (s == null || "segmented".equals(s) || "toggle".equals(s) || "segmentedRoundRect".equals(s)
				|| "segmentedCapsule".equals(s) || s.contains("segmentedTextured")) {
	...
	public static Dimension getPreferredSize(AbstractButton b) {
		...
		if ("help".equals(style)) {
			...
			return new Dimension(helpIcon.getIconWidth() + insets.left + insets.right,
		...
		/*
		 * The preferred size of the button is the size of the text and icon rectangles
		 * plus the buttons insets.
		 */
		...
		// }
		if (sv == QuaquaUtilities.SizeVariant.REGULAR && style.equals("push") && b.getIcon() == null
				&& b.getText() != null) {
	 ...
	 */
	protected String layoutCL(AbstractButton c, FontMetrics fontMetrics, String text, Icon icon, Rectangle viewR,
			Rectangle iconR, Rectangle textR) {
		return SwingUtilities.layoutCompoundLabel(c, fontMetrics, text, icon, c.getVerticalAlignment(),
				c.getHorizontalAlignment(), c.getVerticalTextPosition(), c.getHorizontalTextPosition(), viewR, iconR,
				textR, Methods.invokeGetter(c, "getIconTextGap", 4));
	}
	...
	public Rectangle getVisualBounds(JComponent c, int type, int width, int height) {
		...
		if (type == VisuallyLayoutable.COMPONENT_BOUNDS && b.getBorder() != null && b.isBorderPainted()) {
	...
	/*
	 * private static void animateButton(final AbstractButton b) { if
	 * (animatedComponents.add(b)) { Timer animationTimer; long sleep = 1000 / 30;
	 * // 30 fps animationTimer = new Timer((int) sleep, new ActionListener() {
	 * 
	 * public void actionPerformed(ActionEvent event) {
	 * animatedComponents.remove(b); b.repaint(); } }); animationTimer.start(); } }
	 */
}
