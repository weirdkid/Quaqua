...
public class QuaquaScrollBarUI extends BasicScrollBarUI {
	...
	private boolean isSmall() {
		...
		return sv == QuaquaUtilities.SizeVariant.SMALL || sv == QuaquaUtilities.SizeVariant.MINI;
	}

	/**
	 * Return the smallest acceptable size for the thumb. If the scrollbar becomes
	 * so small that this size isn't available, the thumb will be hidden.
	 * <p>
	 * <b>Warning </b>: the value returned by this method should not be be modified,
	 * it's a shared static constant.
	 *
	...
	protected Dimension getMinimumThumbSize() {
		return isSmall() ? smallMinimumThumbSize : minimumThumbSize;
	}
	...
	public Dimension getPreferredSize(JComponent c) {
		...
		return (scrollbar.getOrientation() == Adjustable.VERTICAL) ? (Dimension) dim.clone()
				: new Dimension(dim.height, dim.width);
	}

	/**
	 * Return the largest acceptable size for the thumb. To create a fixed size
	 * thumb one make this method and <code>getMinimumThumbSize</code> return the
	 * same value.
	 * <p>
	 * <b>Warning </b>: the value returned by this method should not be be modified,
	 * it's a shared static constant.
	...
	protected void paintThumb(Graphics gr, JComponent c, Rectangle thumbBounds) {
		...
		if (!scrollbar.isEnabled() || !QuaquaUtilities.isOnActiveWindow(scrollbar)) {
			Border thumbBorder;
			if (scrollbar.getOrientation() == Adjustable.VERTICAL) {
				thumbBorder = UIManager
						.getBorder(isSmall ? "ScrollBar.thumb.vInactive.small" : "ScrollBar.thumb.vInactive");
			} else {
				thumbBorder = UIManager
						.getBorder(isSmall ? "ScrollBar.thumb.hInactive.small" : "ScrollBar.thumb.hInactive");
			}
			if (thumbBorder != null) {
				thumbBorder.paintBorder(c, g, thumbBounds.x, thumbBounds.y, thumbBounds.width, thumbBounds.height);
			}
		} else {
			Border thumbBorder;
			if (scrollbar.getOrientation() == Adjustable.VERTICAL) {
			...
			if (scrollbar.getOrientation() == Adjustable.VERTICAL) {
				Icon[] icons = ((Icon[]) UIManager
						.get(isSmall ? "ScrollBar.thumb.vFirst.small" : "ScrollBar.thumb.vFirst"));
				if (icons != null) {
					Icon thumbBegin = icons[thumbBounds.y % 5];
					thumbBegin.paintIcon(c, g, thumbBounds.x, thumbBounds.y);
					Icon thumbEnd = ((Icon[]) UIManager
							.get(isSmall ? "ScrollBar.thumb.vLast.small" : "ScrollBar.thumb.vLast"))[(thumbBounds.y
									+ thumbBounds.height) % 5];
					thumbEnd.paintIcon(c, g, thumbBounds.x,
							thumbBounds.y + thumbBounds.height - thumbEnd.getIconHeight());
					BufferedImage img = (BufferedImage) UIManager
							.get(isSmall ? "ScrollBar.thumb.vMiddle.small" : "ScrollBar.thumb.vMiddle");
					TexturePaint paint = new TexturePaint(img,
							new Rectangle(thumbBounds.x, 0, img.getWidth(), img.getHeight()));
					g.setPaint(paint);
					g.fillRect(thumbBounds.x, thumbBounds.y + thumbBegin.getIconHeight(), getPreferredSize(c).width,
							thumbBounds.height - thumbBegin.getIconHeight() - thumbEnd.getIconHeight());
				}
			} else {
				Icon[] icons = ((Icon[]) UIManager
						.get(isSmall ? "ScrollBar.thumb.hFirst.small" : "ScrollBar.thumb.hFirst"));
				if (icons != null) {
					Icon thumbBegin = icons[thumbBounds.x % 5];
					thumbBegin.paintIcon(c, g, thumbBounds.x, thumbBounds.y);
					Icon thumbEnd = ((Icon[]) UIManager
							.get(isSmall ? "ScrollBar.thumb.hLast.small" : "ScrollBar.thumb.hLast"))[(thumbBounds.x
									+ thumbBounds.width) % 5];
					thumbEnd.paintIcon(c, g, thumbBounds.x + thumbBounds.width - thumbEnd.getIconWidth(),
							thumbBounds.y);
					BufferedImage img = (BufferedImage) UIManager
							.get(isSmall ? "ScrollBar.thumb.hMiddle.small" : "ScrollBar.thumb.hMiddle");
					TexturePaint paint = new TexturePaint(img,
							new Rectangle(0, thumbBounds.y, img.getWidth(), img.getHeight()));
					g.setPaint(paint);
					g.fillRect(thumbBounds.x + thumbBegin.getIconWidth(), thumbBounds.y,
							thumbBounds.width - thumbBegin.getIconWidth() - thumbEnd.getIconWidth(),
							getPreferredSize(c).height);
				}
			}
		}
	}
	...
	protected void paintTrack(Graphics g, JComponent c, Rectangle trackBounds) {
		...
		Rectangle contentBounds = new Rectangle(sbInsets.left, sbInsets.top,
				sbSize.width - sbInsets.left - sbInsets.right, sbSize.height - sbInsets.top - sbInsets.bottom);
		...
		if (scrollbar.getOrientation() == Adjustable.VERTICAL) {
			if (isPlaceButtonsTogether() && contentBounds.height < tbInsets.top + tbInsets.bottom) {
				...
				Graphics clipped = g.create(contentBounds.x, contentBounds.y, contentBounds.width,
						contentBounds.height);
				int buttonsH = incrButton.getPreferredSize().height * 2;
				trackAndButtons.paintBorder(c, g, contentBounds.x,
						contentBounds.y + contentBounds.height - tbInsets.top - tbInsets.bottom
								+ Math.max(0, (buttonsH - contentBounds.height) / 2),
						contentBounds.width, tbInsets.top + tbInsets.bottom);
				clipped.dispose();
			} else {
				trackAndButtons.paintBorder(c, g, contentBounds.x, contentBounds.y, contentBounds.width,
						contentBounds.height);
			}
		} else {
			if (isPlaceButtonsTogether() && contentBounds.width < tbInsets.left + tbInsets.right) {
				// Special treatment, if track and buttons do
				// not fit into available space
				// trackAndButtons.paintBorder(c, g, contentBounds.x, contentBounds.y,
				// contentBounds.width, contentBounds.height);
				Graphics clipped = g.create(contentBounds.x, contentBounds.y, contentBounds.width,
						contentBounds.height);
				int buttonsW = incrButton.getPreferredSize().width * 2;
				trackAndButtons.paintBorder(c, g,
						contentBounds.x + contentBounds.width - tbInsets.left - tbInsets.right
								+ Math.max(0, (buttonsW - contentBounds.width) / 2),
						contentBounds.y, tbInsets.left + tbInsets.right, contentBounds.height);
				clipped.dispose();
			} else {
				trackAndButtons.paintBorder(c, g, contentBounds.x, contentBounds.y, contentBounds.width,
						contentBounds.height);
	...
	/**
	 * We use a border to fill the background of the scroll bar. The border contains
	 * the track and the buttons in their current visual representation depending on
	 * the various states of the scroll bar ('enabled','pressed','inactive').
	 */
	protected Border getTrackAndButtonsBorder() {
		char vh = (scrollbar.getOrientation() == Adjustable.VERTICAL) ? 'v' : 'h';
		boolean isSmall = isSmall();
		...
		if (isPlaceButtonsTogether()) {
			borders = (Border[]) UIManager
					.get(isSmall ? "ScrollBar.buttons." + vh + "Tog.small" : "ScrollBar.buttons." + vh + "Tog");
		} else {
			borders = (Border[]) UIManager
					.get(isSmall ? "ScrollBar.buttons." + vh + "Sep.small" : "ScrollBar.buttons." + vh + "Sep");
		}
		return (borders == null) ? UIManager.getBorder("ScrollBar.track." + vh) : borders[buttonsIndex];
	}

	/**
	 * Indicates whether the user can absolutely position the offset with a mouse
	 * click (depending on the settings in "Appearance" panel of " the "System
	...
	protected void layoutVScrollbar(JScrollBar sb) {
		...
		/**
		 * Hide thumb and buttons if the complete extent is shown. Note that setting the
		...
		incrButtonH = decrButtonH = UIManager
				.getInt((isSmall) ? "ScrollBar.buttonHeight.small" : "ScrollBar.buttonHeight");
		...
		if (isPlaceButtonsTogether()) {
			...
			trackY = sbInsets.top + UIManager
					.getInsets((isSmall) ? "ScrollBar.trackInsets.tog.small" : "ScrollBar.trackInsets.tog").top;
			trackH = decrButtonY - trackY + UIManager
					.getInsets((isSmall) ? "ScrollBar.trackInsets.tog.small" : "ScrollBar.trackInsets.tog").bottom;
		} else {
			decrButtonY = sbInsets.top;
			incrButtonY = sbSize.height - (sbInsets.bottom + incrButtonH);
		 ...
		 * fits into the track. Note that setting the thumbs bounds will cause a
		 * repaint.
		 */
		if (thumbH >= trackH) {
			setThumbBounds(0, 0, 0, 0);
	...
	protected void layoutHScrollbar(JScrollBar sb) {
		...
		leftButtonW = rightButtonW = UIManager
				.getInt((isSmall) ? "ScrollBar.buttonHeight.small" : "ScrollBar.buttonHeight");
		...
		if (isPlaceButtonsTogether()) {
			...
			trackX = sbInsets.left + UIManager
					.getInsets((isSmall) ? "ScrollBar.trackInsets.tog.small" : "ScrollBar.trackInsets.tog").left;
			trackW = leftButtonX - trackX + UIManager
					.getInsets((isSmall) ? "ScrollBar.trackInsets.tog.small" : "ScrollBar.trackInsets.tog").right;
		} else {
		 ...
		 * specially to avoid numerical problems in computing thumbX. If the thumb
		...
		/*
		 * Make sure the thumb fits between the buttons. Note that setting the thumbs
		 * bounds causes a repaint.
		 */
		if (thumbW >= trackW) {
			setThumbBounds(0, 0, 0, 0);
		} else {
	...
	/*
	 * Method for scrolling by a block increment. Added for mouse wheel scrolling
	 * support, RFE 4202656.
	 */
	...
	protected class QuaquaTrackListener extends TrackListener {
		 ...
		 * If there is no thumb then page up if the mouse is in the upper half of the
		 * track.
		 */
		@Override
		 ...
		 * left-to-right/right-to-left scrollbar relative to the origin of the track.
		 */
		@Override
		...
		private void setValueFrom(MouseEvent e) {
			...
			if (scrollbar.getOrientation() == Adjustable.VERTICAL) {
				...
				/*
				 * if (scrollbar.getComponentOrientation().isLeftToRight()) { thumbMin =
				 * decrButton.getX() + decrButton.getWidth(); thumbMax = incrButton.getX() -
				 * thumbR.width; } else { thumbMin = incrButton.getX() + incrButton.getWidth();
			...
			/*
			 * Set the scrollbars value. If the thumb has reached the end of the scrollbar,
			 * then just set the value to its maximum. Otherwise compute the value as
		...
		private void startScrollTimerIfNecessary() {
			...
			case Adjustable.VERTICAL:
				if (direction > 0) {
					if (getThumbBounds().y
							+ getThumbBounds().height < ((QuaquaTrackListener) trackListener).currentMouseY) {
						scrollTimer.start();
					}
			...
			case Adjustable.HORIZONTAL:
				if (direction > 0) {
					if (getThumbBounds().x
							+ getThumbBounds().width < ((QuaquaTrackListener) trackListener).currentMouseX) {
						scrollTimer.start();
					}
	...
	protected class QuaquaArrowButtonListener extends ArrowButtonListener {
		...
		protected void repaintButtons() {
			...
			if (isPlaceButtonsTogether()) {
				...
				if (scrollbar.getOrientation() == Adjustable.VERTICAL) {
					scrollbar.repaint(contentBounds.x, contentBounds.y + contentBounds.height - tbInsets.bottom,
							contentBounds.width, tbInsets.bottom);
				} else {
					scrollbar.repaint(contentBounds.x + contentBounds.width - tbInsets.right, contentBounds.y,
							tbInsets.right, contentBounds.height);
				}
			} else {
				scrollbar.repaint();
			}
	...
	protected class QuaquaScrollListener extends ScrollListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (useBlockIncrement) {
				...
				if (scrollbar.getOrientation() == Adjustable.VERTICAL) {
					if (direction > 0) {
						if (getThumbBounds().y + getThumbBounds().height >= trackListener.getCurrentMouseY()) {
					...
					if (direction > 0) {
						if (getThumbBounds().x + getThumbBounds().width >= trackListener.getCurrentMouseX()) {
							((Timer) e.getSource()).stop();
						}
