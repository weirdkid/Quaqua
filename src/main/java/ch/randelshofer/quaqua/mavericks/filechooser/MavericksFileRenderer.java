...
public class MavericksFileRenderer extends JLabel implements ListCellRenderer, CellRenderer {
	...
	public MavericksFileRenderer(JFileChooser fileChooser, Icon expandingIcon, Icon expandedIcon,
			Icon selectedExpandingIcon, Icon selectedExpandedIcon, Icon focusedSelectedExpandingIcon,
			Icon focusedSelectedExpandedIcon) {
	...
	@Override
	public Component getCellRendererComponent(JComponent container, Object value, boolean isSelected,
			boolean cellHasFocus) {
		return getCellRendererComponent(container, value, isSelected, cellHasFocus, true);
	}

	protected Component getCellRendererComponent(JComponent container, Object value, boolean isSelected,
			boolean cellHasFocus, boolean isListView) {
		...
		if (!isListView) {
			boolean useUnselectedArrow = UIManager
					.getBoolean("FileChooser.browserUseUnselectedExpandIconForLabeledFile");
			...
			/*
			 * Special case: no arrow is displayed for a package even if the package is
			 * traversable (an option).
			 */
		...
		setBorder(isListView ? null
				: (cellHasFocus)
						? UIManager.getBorder(isGrayed ? "FileChooser.browserCellFocusBorderGrayed"
	...
	protected void paintComponent(Graphics gr) {
		...
		String clippedText = layoutRenderer(textFM, text, icon, arrowIcon, viewRect, iconRect, textRect, arrowIconRect,
				labelRect, text == null ? 0 : textIconGap, textArrowIconGap);
	...
	/**
	 * The following variables are used for laying out the renderer. This variables
	 ...
	 * where a FileRenderer instance enters a method of another FileRenderer
	 * instance.
	 */
	...
	public Dimension getPreferredSize() {
		...
		layoutRenderer(textFM, text, icon, arrowIcon, viewRect, iconRect, textRect, arrowIconRect, labelRect,
	...
	private String layoutRenderer(FontMetrics textFM, String text, Icon icon, Icon arrowIcon, Rectangle viewRect,
			Rectangle iconRect, Rectangle textRect, Rectangle arrowIconRect, Rectangle labelRect, int textIconGap,
		...
		text = QuaquaUtilities.layoutCompoundLabel(this, textFM, text, icon, SwingConstants.CENTER, SwingConstants.LEFT,
