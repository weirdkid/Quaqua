...
public class QuaquaIconFactory {
	...
	/**
	 * Lazy option pane icon. The creation of an option pane icon is a potentially
	 * slow operation. This icon class will load the icon image in a worker thread
	 * and paint it, when it is ready.
	 */
	...
	public static Icon[] createIcons(String location, Rectangle subimage, int count, boolean horizontal) {
		...
		BufferedImage[] images = Images.split(img, count, horizontal);
	...
	public static Icon createButtonStateIcon(String location, int states) {
		return new ButtonStateIcon(createImage(location), states, true);
	}

	public static Icon createButtonStateIcon(String location, int states, Point shift) {
		return new ShiftedIcon(new ButtonStateIcon(createImage(location), states, true), shift);
	}
	...
	public static Icon createFrameButtonStateIcon(String location, int states) {
		return new FrameButtonStateIcon(createImage(location), states, true);
	}

	/**
	 * Creates a button state icon overlaid with a button focus icon.
	 */
	public static Icon createOverlaidButtonStateIcon(String location1, int states1, String location2, int states2,
			Rectangle layoutRect) {
		return new IconUIResource(new VisuallyLayoutableIcon(
				new OverlayIcon(createButtonStateIcon(location1, states1), createButtonFocusIcon(location2, states2)),
				layoutRect));
	}

	public static Icon createButtonFocusIcon(String location, int states) {
		return new ButtonFocusIcon(createImage(location), states, true);
	}
	...
	public static Icon createIcon(Class baseClass, String location, Point shift) {
		return new ShiftedIcon(new ImageIcon(createImage(baseClass, location)), shift);
	}

	public static Icon createIcon(Class baseClass, String location, Rectangle shiftAndSize) {
		return new ShiftedIcon(new ImageIcon(createImage(baseClass, location)), shiftAndSize);
	}
	...
	public static Icon createNativeSidebarIcon(String path, int width, int height, Color color, Color selectedColor) {
		try {
			...
			// BandCombineOp bop=new BandCombineOp(new
			// float[][]{{1f,0f,0f,0f,0f},{0f,1f,0f,0f,0f},{0f,0f,1f,0f,0f},{1f,0f,0f,0f,0f}},null);
			g.drawImage(img, rop, 0, 0);
			g.setComposite(AlphaComposite.SrcIn);
	...
	public static Icon createNativeButtonStateIcon(OSXAquaPainter.Widget widget, int xoffset, int yoffset, int width,
			int height, boolean drawFocusRing) {
		Icon icon = new QuaquaNativeButtonStateIcon(widget, xoffset, yoffset, width, height);
		if (drawFocusRing) {
	...
	private static ImageIcon createApplicationIcon() {
		...
		/*
		 * BufferedImage image = getApplicationIconImage(); return new
		 * ImageIcon(image.getScaledInstance(64, 64, Image.SCALE_SMOOTH));
		 */
		/*
		 * BufferedImage result = new BufferedImage(64, 64,
		 * BufferedImage.TYPE_INT_ARGB); Graphics2D g = result.createGraphics();
		 * g.drawImage(getApplicationIconImage(), 0, 0, 64, 64, null); g.dispose();
	...
	private static ImageIcon composeOptionPaneIcon(String resource) {
		...
		BufferedImage warningImage = Images
				.toBufferedImage(Images.createImage(QuaquaIconFactory.class.getResource(resource)));
	...
	/**
	 * Gets the application image. This is a buffered image of size 64x64. If the
	 * Quaqua JNI code and the ImageIO API are present, this will get the image from
	 * the OS X application bundle. In all other cases this will return a default
	 * application image.
	 */
